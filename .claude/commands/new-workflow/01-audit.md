...
Comprehensive Linear issue validation system that performs unbiased investigation, validates root causes, and provides actionable recommendations. Features sequential multi-agent analysis optimized for batching, historical context validation, fresh investigation capabilities, and intelligent workflow routing based on informed complexity assessment.
...
```
01-audit - Step 1: Comprehensive Linear issue validation and intelligent workflow routing

USAGE:
  01-audit RR-123                  Audit single issue with workflow routing
...
FEATURES:
  ...
  • Intelligent workflow routing based on informed complexity assessment
...
WORKFLOW ROUTING:
  After comprehensive analysis, routes to appropriate workflow:
  STANDARD (6 steps): 01-audit → 02-plan → 04-implement → 06-test → 08-commit
  COMPLEX (8 steps): 01-audit → 02-plan → 03-stage → 04-implement → 05-review → 06-test → 07-document → 08-commit
...
- **Memory Lookup**: Use `mcp__serena__read_memory` to check issue_learnings_consolidated.md and completed_issue_implementations.md for similar issue patterns and gotchas
...
### Step 4.5: Evidence-Based Validation
...
### Step 4.6: Alternative Root Cause Analysis
...
## Phase 8: Intelligent Complexity Assessment & Workflow Routing

### Step 8.1: Informed Complexity Analysis

**NOW perform complexity assessment with full context from comprehensive audit:**

```
## 🧭 Intelligent Workflow Routing Analysis

Based on comprehensive investigation findings, assess actual complexity:

### Implementation Scope Assessment
**Files to Modify**: [Exact count from symbol analysis]
- Single file: 1-2 files (STANDARD +0)
- Multi-file: 3-6 files (STANDARD +1)
- Cross-module: 7-15 files (COMPLEX +2)
- System-wide: 15+ files (COMPLEX +3)

**Symbol Impact Analysis** (from Serena MCP findings):
- Isolated changes: 1-3 symbols (STANDARD +0)
- Component changes: 4-10 symbols (STANDARD +1)
- Service integration: 11-25 symbols (COMPLEX +2)
- Architecture impact: 25+ symbols (COMPLEX +3)

**Database Complexity** (from db-expert findings):
- No database changes (STANDARD +0)
- Simple queries/indexes (STANDARD +1)
- Schema modifications (COMPLEX +2)
- Migration dependencies (COMPLEX +3)

**Integration Requirements** (from tech-expert findings):
- Component-only changes (STANDARD +0)
- Service layer changes (STANDARD +1)
- API boundary changes (COMPLEX +2)
- External system integration (COMPLEX +3)

**Risk Factors** (from agent consensus):
- Low risk, proven patterns (STANDARD +0)
- Medium risk, some unknowns (STANDARD +1)
- High risk, significant unknowns (COMPLEX +2)
- Critical risk, breaking changes (COMPLEX +3)

**Testing Complexity** (from test-expert findings):
- Existing test patterns (STANDARD +0)
- New test scenarios (STANDARD +1)
- Integration test requirements (COMPLEX +2)
- E2E test dependencies (COMPLEX +3)

TOTAL INFORMED COMPLEXITY SCORE: [Sum based on actual investigation]
```

### Step 8.2: Evidence-Based Workflow Decision

**Make routing decision based on comprehensive investigation:**

```
📊 Informed Complexity Assessment for $ARGUMENTS:

🔍 INVESTIGATION-BASED COMPLEXITY: [STANDARD/COMPLEX]
📈 Evidence Score: [X]/18 points

DETAILED BREAKDOWN:
- Implementation Scope: [X] points - [reasoning from investigation]
- Symbol Impact: [X] points - [specific symbols from Serena analysis]
- Database Changes: [X] points - [schema requirements from db-expert]
- Integration Requirements: [X] points - [API/service changes from tech-expert]
- Risk Assessment: [X] points - [risk factors from agent consensus]
- Testing Complexity: [X] points - [test requirements from test-expert]

🎯 WORKFLOW ROUTING DECISION:

[STANDARD Workflow] (Score: 0-9):
✅ Suitable for efficient implementation
→ Path: 01-audit → 02-plan → 04-implement → 06-test → 08-commit (6 steps)
→ Estimated time: 2-4 hours
→ Skips: 03-stage, 05-review, 07-document
→ Focus: Speed and direct implementation

[COMPLEX Workflow] (Score: 10+):
✅ Requires comprehensive quality gates
→ Path: 01-audit → 02-plan → 03-stage → 04-implement → 05-review → 06-test → 07-document → 08-commit (8 steps)
→ Estimated time: 1-3 days
→ Includes: All quality gates and comprehensive validation
→ Focus: Quality and risk mitigation

💡 KEY EVIDENCE: [Most important factor that drives this decision]
```

### Step 8.3: Concise Final Output

**Present clean, actionable summary:**

```
✅ RR-XXX Audit Complete

Status: [VALID/INVALID] | Confidence: [HIGH/MEDIUM/LOW] | Type: [STANDARD/COMPLEX]

Key Finding: [One sentence summary of main discovery]

Next Steps:
[STANDARD] → 02-plan → 04-implement → 06-test → 08-commit (2-4 hours)
[COMPLEX] → 02-plan → 03-stage → 04-implement → 05-review → 06-test → 07-document → 08-commit (1-3 days)

Run: 02-plan RR-XXX
```

## Phase 9: Interactive Follow-up Options

After presenting the comprehensive report and routing decision, offer these options:
...
4. **Review specific changes** → Examine particular findings
5. **Additional investigation** → Deep dive specific areas
...
Always conclude with the concise summary from Step 8.3 above. The comprehensive investigation details are available during the analysis phases, but the final output focuses on actionable next steps.
...
The audit findings and workflow routing will inform the implementation strategy in the planning phase, with subsequent commands auto-adapting based on the stored workflow state.

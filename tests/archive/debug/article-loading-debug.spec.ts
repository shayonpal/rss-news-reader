  ...
  test("should debug article loading after navigation", async ({ page }) => {
    ...
    if (initialState.articlesInDOM === 0) {
      ...
      const apiErrors = await page.evaluate(() => {
        ...
        return {
          hasNoArticlesMessage:
            bodyText.includes("No articles") ||
            bodyText.includes("no articles"),
          hasErrorMessage:
            bodyText.includes("Error") || bodyText.includes("error"),
          hasLoadingMessage:
    ...
    const firstArticleId = await page.evaluate(() => {
      ...
      return articles.length > 0
        ? articles[0].getAttribute("data-article-id")
        : null;
    });
    ...
    const articlePageState = await page.evaluate(() => {
      return {
        ...
        hasArticleContent:
          document.querySelector("[data-article-content]") !== null,
        hasBackButton:
          document.querySelector("[data-back]") !== null ||
    ...
    const trackLoading = async () => {
      const state = await page.evaluate(() => {
        return {
          ...
          articlesInDOM: document.querySelectorAll("[data-article-id]").length,
          readArticles: document.querySelectorAll(
            '[data-article-id][data-is-read="true"]'
          ).length,
          unreadArticles: document.querySelectorAll(
    ...
    loadingStates.forEach((state, idx) => {
      const elapsed =
        idx === 0 ? 0 : state.timestamp - loadingStates[0].timestamp;
      console.log(
        `  ${elapsed}ms: Articles=${state.articlesInDOM} (R:${state.readArticles}, U:${state.unreadArticles}), Loading=${state.loadingElements}, Session=${state.hasSessionState}`
      );
    });

    console.log("\n🔍 Final Analysis:");
    console.log(
      `  Expected: At least 1 read article visible (session preserved)`
    );
    ...
    if (finalState.articlesInDOM === 0) {
      ...
      console.log(
        "This suggests a data loading problem, not just a session state issue"
      );
      ...
      const afterReloadState = await page.evaluate(() => {
        return {
          ...
          hasSessionState: !!sessionStorage.getItem("articleListState"),
          errorElements: document.querySelectorAll("[data-error], .error")
            .length,
        };
      });
      ...
      if (afterReloadState.articlesInDOM > 0) {
        ...
        console.log(
          "🔍 This indicates a SPA navigation issue, not a data issue"
        );
      } else {
      ...
      console.log(
        "🔍 This suggests the session state preservation is not working"
      );

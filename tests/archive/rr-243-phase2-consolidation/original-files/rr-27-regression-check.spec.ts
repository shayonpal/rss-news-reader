...
test.describe("RR-27 Regression: Article Preservation", () => {
  ...
  test("Article preservation with India/Canada topic filter", async ({
    page,
  }) => {
    ...
    const mainContentArea = page
      .locator('[class*="flex-1"], [class*="min-w-0"]')
      .first();
    const articleElements = mainContentArea.locator(".cursor-pointer").filter({
      hasText:
        /Canada|India|immigration|visa|border|Toronto|Ottawa|Delhi|Mumbai/i,
    });
    ...
    if ((await articleElements.count()) > 0) {
      ...
      console.log(
        `📍 Found India/Canada article: ${articleText.substring(0, 100)}...`
      );
    } else {
      // Fallback: try any clickable element in main content that looks like an article
      console.log(
        "📍 No India/Canada specific articles found, trying any article in main content"
      );
      articleElement = mainContentArea
      ...
      if ((await articleElement.count()) > 0) {
        articleText = (await articleElement.textContent()) || "";
        console.log(
          `📍 Found fallback article: ${articleText.substring(0, 100)}...`
        );
      } else {
    ...
    const preservedArticle = page.locator(
      "text=" + articleText.substring(0, 50)
    );
    const isArticleVisible = (await preservedArticle.count()) > 0;

    console.log(
      `📍 Article preservation result: ${isArticleVisible ? "PRESERVED ✅" : "DISAPPEARED ❌"}`
    );

    if (!isArticleVisible) {
      console.log(
        "❌ REGRESSION DETECTED: Article disappeared after being read (RR-27 broken)"
      );
      ...
      throw new Error(
        "RR-27 REGRESSION: Read article disappeared from filtered view after back navigation"
      );
    } else {
      console.log(
        "✅ RR-27 working correctly: Article preserved in filtered view"
      );
    }
  });

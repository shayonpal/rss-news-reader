...
async function testTagSync() {
  ...
  try {
    ...
    while (status === 'running' && attempts < maxAttempts) {
      ...
      const statusResponse = await fetch(`http://localhost:3000/reader/api/sync/status/${syncData.syncId}`);
      if (statusResponse.ok) {
        const statusData = await statusResponse.json();
        status = statusData.status;
        
        if (statusData.progress) {
          process.stdout.write(`\rProgress: ${statusData.progress}% - ${statusData.message || 'Processing...'}`);
        }
      }
      attempts++;
    }

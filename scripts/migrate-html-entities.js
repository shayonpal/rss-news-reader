...
async function logProgress(data) {
  ...
  console.log(`📝 ${logEntry.timestamp} - ${logEntry.message || JSON.stringify(data)}`);
...
async function createBackupColumns() {
  if (!BACKUP_ENABLED) {
    await logProgress({ action: 'backup_skip', message: 'Backup columns disabled' });
    return;
  }

  await logProgress({ action: 'backup_start', message: 'Creating backup columns' });

  try {
    // Check if backup columns already exist
    const { data: columns } = await supabase.rpc('get_table_columns', { table_name: 'articles' });
    
    const hasBackupTitle = columns?.some(col => col.column_name === 'title_original');
    const hasBackupContent = columns?.some(col => col.column_name === 'content_original');

    if (!hasBackupTitle) {
      const { error } = await supabase.rpc('exec_sql', {
        query: 'ALTER TABLE articles ADD COLUMN IF NOT EXISTS title_original TEXT;'
      });
      if (error) throw error;
      await logProgress({ action: 'backup_column_created', column: 'title_original' });
    }

    if (!hasBackupContent) {
      const { error } = await supabase.rpc('exec_sql', {
        query: 'ALTER TABLE articles ADD COLUMN IF NOT EXISTS content_original TEXT;'
      });
      if (error) throw error;
      await logProgress({ action: 'backup_column_created', column: 'content_original' });
    }

    await logProgress({ action: 'backup_complete', message: 'Backup columns ready' });
  } catch (error) {
    await logProgress({ 
      action: 'backup_error', 
      error: error.message,
...
async function findArticlesToProcess() {
  await logProgress({ action: 'scan_start', message: 'Scanning for articles with HTML entities' });

  // Use LIKE patterns for better detection
  const { data: titleMatches, error: titleError } = await supabase
    .from('articles')
    .select('id')
    .or('title.like.%&rsquo;%,title.like.%&lsquo;%,title.like.%&amp;%,title.like.%&quot;%,title.like.%&#8217;%,title.like.%&ndash;%,title.like.%&mdash;%,title.like.%&ldquo;%,title.like.%&rdquo;%');

  const { data: contentMatches, error: contentError } = await supabase
    .from('articles')
    .select('id')
    .or('content.like.%&rsquo;%,content.like.%&lsquo;%,content.like.%&amp;%,content.like.%&quot;%,content.like.%&#8217;%,content.like.%&ndash;%,content.like.%&mdash;%,content.like.%&ldquo;%,content.like.%&rdquo;%');
...
async function verifyMigration() {
  await logProgress({ action: 'verify_start', message: 'Verifying migration results' });
...
async function rollbackMigration() {
  ...
  await logProgress({ action: 'rollback_start', message: 'Starting rollback of HTML entity migration' });
  ...
  try {
    ...
    await logProgress({ action: 'rollback_complete', message: 'Rollback completed successfully' });

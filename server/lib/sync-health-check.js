...
class SyncHealthCheck {
  ...
  async checkHealth(syncService) {
    ...
    const avgApiCallTime = this.calculateAverage(
      performanceMetrics.inoreaderApiCalls
    );
    ...
    try {
      ...
      if (stats) {
        metadata = {
          ...
          lastProcessed:
            syncService.lastProcessedTime || new Date().toISOString(),
        };
        ...
        checks.push({
          ...
          message: syncActive
            ...
            : "Bi-directional sync is inactive",
        });
      } else {
        checks.push({
          name: "queue-health",

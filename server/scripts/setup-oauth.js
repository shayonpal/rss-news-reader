...
const CONFIG = {
  ...
  redirectUri:
    process.env.INOREADER_OAUTH_REDIRECT_URI ||
...
function createOAuthServer() {
  return new Promise((resolve, reject) => {
    ...
    server = app.listen(CONFIG.serverPort, () => {
      console.log(
        `🚀 OAuth callback server listening on http://localhost:${CONFIG.serverPort}`
      );
    });
...
async function automateOAuthFlow() {
  ...
  try {
    ...
    const authorizeButton = await page.$(
      'button:has-text("Authorize"), input[value="Authorize"]'
    );
    if (authorizeButton) {

...
const CONFIG = {
  ...
  redirectUri: process.env.INOREADER_REDIRECT_URI || 'http://localhost:8080/auth/callback',
...
async function main() {
  ...
  if (code) {
    try {
      ...
      try {
        const result = execSync('pm2 logs rss-sync-server --nostream --lines 5', { encoding: 'utf8' });

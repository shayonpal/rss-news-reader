...
export const inoreaderApi = {
  ...
  async getUserInfo(trigger = "unknown"): Promise<UserInfo> {
    const response = await fetch(
      `/api/inoreader/user-info?trigger=${encodeURIComponent(trigger)}`,
      {
  ...
  async getSubscriptions(
    ...
    const response = await fetch(
      `/api/inoreader/subscriptions?trigger=${encodeURIComponent(trigger)}`,
      {
        credentials: "include",
      }
    );
  ...
  async getUnreadCounts(
    trigger = "unknown"
  ): Promise<{ unreadcounts: UnreadCount[] }> {
    const response = await fetch(
      `/api/inoreader/unread-counts?trigger=${encodeURIComponent(trigger)}`,
      {
        credentials: "include",
      }
    );
  ...
  async getStreamContents(
    ...
    const response = await fetch(`/api/inoreader/stream-contents?${params}`, {
      credentials: "include",
    });

    if (!response.ok) {
      throw new Error(
        `Failed to fetch stream contents: ${response.statusText}`
      );
    }
  ...
  async markAsRead(itemIds: string[], trigger = "unknown"): Promise<void> {
    const response = await fetch(
      `/api/inoreader/edit-tag?trigger=${encodeURIComponent(trigger)}`,
      {
        method: "POST",
        credentials: "include",
        headers: {
  ...
  getFeedArticles(
    feedId: string,
    options?: { count?: number; continuation?: string; trigger?: string }
  ) {

 ...
 * Provides backward compatibility while transitioning from direct table updates to RPC functions
 */
...
// Type interfaces for RPC function parameters
interface RPCIncrementParams {
  p_service: string;
  p_date: string;
  p_increment: number;
}

interface RPCZoneUpdateParams {
  p_service: string;
  p_date: string;
  p_zone1_usage?: number | null;
  p_zone2_usage?: number | null;
  p_zone1_limit?: number | null;
  p_zone2_limit?: number | null;
  p_reset_after?: number | null;
}
...
export class ApiUsageTracker {
   ...
   */
  async trackUsage(params: TrackApiUsageParams): Promise<{
    success: boolean;
    error?: string;
    data?: any;
  }> {
    ...
    try {
      ...
      if (
        ...
        const rpcParams: RPCZoneUpdateParams = {
      ...
      const rpcParams: RPCIncrementParams = {
        p_service: params.service,
        p_date: date,
   ...
   */
  async trackUsageWithFallback(params: TrackApiUsageParams): Promise<{
    success: boolean;
    error?: string;
    data?: any;
  }> {
...
export async function incrementApiUsage(
  ...
  return tracker.trackUsageWithFallback({ service, date, increment });
}

...
export async function captureRateLimitHeaders(headers: Headers): Promise<void> {
  try {
    ...
    if (zone1Usage || zone1Limit || zone2Usage || zone2Limit || resetAfter) {
      ...
      const usage1 = zone1Usage != null ? parseInt(zone1Usage.replace(/,/g, '')) : undefined;
      const limit1 = zone1Limit != null ? parseInt(zone1Limit.replace(/,/g, '')) : undefined;
      const usage2 = zone2Usage != null ? parseInt(zone2Usage.replace(/,/g, '')) : undefined;
      const limit2 = zone2Limit != null ? parseInt(zone2Limit.replace(/,/g, '')) : undefined;
      const resetSeconds = resetAfter != null ? parseInt(resetAfter.replace(/\.\d+$/, '')) : undefined;
      ...
      if (existing) {
        ...
        if (error) {
          console.error('[RateLimitCapture] Failed to update api_usage:', error);
        ...
        // Insert new record
        const insertFields: Record<string, any> = { service: 'inoreader', date: today };
        ...
        if (error) {
          console.error('[RateLimitCapture] Failed to insert api_usage:', error);
        } else {
...
export async function getCurrentApiUsage() {
  try {
    ...
    const { data, error } = await supabase
      .from('api_usage')
      .select('zone1_usage, zone1_limit, zone2_usage, zone2_limit, reset_after, updated_at, count')
      .eq('service', 'inoreader')
      .eq('date', today)
      .single();
    ...
    const zone1Used = Math.min(Math.max(zone1UsedHeader, dailyCount), zone1Limit);
    ...
    return {
      ...
      zone2: {
        ...
        percentage: (data.zone2_limit || 100) ? ((data.zone2_usage || 0) / (data.zone2_limit || 100)) * 100 : 0,
      },

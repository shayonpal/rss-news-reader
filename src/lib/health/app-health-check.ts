...
export class AppHealthCheck {
    ...
    try {
      ...
      if (!exists) {
        ...
        if (!tokens.encrypted) {
          ...
          if (daysRemaining < 30) { // Less than 30 days remaining
            checks.push({
              name: 'token-validity',
              status: 'degraded',
  ...
  private trackPerformance(metric: keyof typeof performanceMetrics, duration: number) {
  ...
  static trackMetric(type: 'dbQueries' | 'apiCalls' | 'syncOperations', duration: number) {

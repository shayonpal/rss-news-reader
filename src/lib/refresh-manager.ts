...
class RefreshManager {
  ...
  private async showSyncToast(status: string, metrics: SyncMetrics): Promise<void> {
    if (status === 'failed') {
    ...
    if (status === 'partial' || metrics.failedFeeds > 0) {
      ...
      toast.warning(`Partial sync • ${totalFeeds - failedCount} of ${totalFeeds} feeds updated`, {
  ...
  private async applySidebarData(sidebarData: SidebarData | undefined): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.debounceTimer = setTimeout(async () => {
        ...
        try {
          ...
          if (result?.metrics) {
            await this.showSyncToast(result.status || 'completed', result.metrics);
          }

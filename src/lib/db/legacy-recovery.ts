...
export class LegacyRecoverySystem {
  ...
  async discoverLegacyDatabases(): Promise<LegacyDatabaseInfo[]> {
    ...
    try {
      ...
      this.updateProgress(
        "discovery",
        `Found ${databases.length} legacy databases`,
        100
      );
      return databases;
    } catch (error) {
   ...
   */
  private async inspectLegacyDatabase(
    dbName: string
  ...
  async extractFromLegacyDatabase(dbName: string): Promise<LegacyData> {
    ...
    try {
      ...
      for (const tableName of tables) {
        try {
          ...
          this.updateProgress(
            "extraction",
            `Extracted ${tableName}`,
            (completedTables / tables.length) * 100
          );
        } catch (error) {
          console.warn(`Failed to extract from table ${tableName}:`, error);
          this.updateProgress(
            "extraction",
            `Failed to extract ${tableName}`,
            0,
            [error instanceof Error ? error.message : "Unknown error"]
          );
        }
      ...
      this.updateProgress(
        "extraction",
        `Extracted ${legacyData.articles.length} articles, ${legacyData.feeds.length} feeds`,
        100
      );
      return legacyData;
    } catch (error) {
  ...
  private mapToLegacyArticle(record: any): LegacyArticle {
    return {
      ...
      published_at:
        record.publishedAt || record.published_at || new Date().toISOString(),
  ...
  private mapToLegacyFolder(record: any): LegacyFolder {
    return {
      ...
      user_id: record.userId || record.user_id || "legacy-user",
      inoreader_id:
        record.inoreaderFolderId || record.inoreader_id || record.id,
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      try {
        ...
        if (error) {
          ...
          throw new Error(
            `Failed to upload batch to ${tableName}: ${error.message}`
          );
        }
  ...
  async verifyMigration(originalData: LegacyData): Promise<boolean> {
    ...
    try {
      ...
      const [usersCount, feedsCount, articlesCount, foldersCount] =
        await Promise.all([
          this.supabase
            .from("users")
  ...
  async cleanupLegacyDatabases(databaseNames: string[]): Promise<void> {
    ...
    for (const dbName of databaseNames) {
      try {
        ...
        this.updateProgress(
          "cleanup",
          `Cleaned up ${dbName}`,
          (cleaned / databaseNames.length) * 100
        );
      } catch (error) {
        console.warn(`Failed to cleanup database ${dbName}:`, error);
        this.updateProgress("cleanup", `Failed to cleanup ${dbName}`, 0, [
          error instanceof Error ? error.message : "Unknown error",
        ]);
      }
    }
    ...
    this.updateProgress(
      "cleanup",
      `Cleanup completed (${cleaned}/${databaseNames.length} databases)`,
      100
    );
  }
    ...
    try {
      ...
      for (const dbInfo of databases) {
        try {
          ...
          result.errors.push(
            `Failed to extract from ${dbInfo.name}: ${error instanceof Error ? error.message : "Unknown error"}`
          );
        }
      }
      ...
      allData.articles = this.removeDuplicates(
        allData.articles,
        "inoreader_id"
      );

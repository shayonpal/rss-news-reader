...
export class DatabaseMigrations {
  ...
  static setupMigrations(): void {
    ...
    db.version(2)
      .stores({
        articles: 'id, feedId, publishedAt, isRead, [feedId+isRead], [feedId+publishedAt], createdAt, updatedAt, extractedAt',
        feeds: 'id, folderId, title, isActive, lastFetchedAt, createdAt, updatedAt, errorCount',
      ...
      .upgrade(async (trans) => {
        ...
        await trans.table('feeds').toCollection().modify((feed) => {
        ...
        await trans.table('summaries').toCollection().modify((summary) => {
          if (!summary.version) {
            summary.version = 1;
            migratedCount++;
          }
        ...
        await trans.table('apiUsage').toCollection().modify((usage) => {
        ...
        await trans.table('dbInfo').toCollection().modify((info) => {
          info.lastMigrationAt = new Date();
        });

        console.log(`Migration to v2 completed. Updated ${migratedCount} records.`);
      });
    ...
    db.version(3)
      .stores({
        ...
        articles: 'id, feedId, publishedAt, isRead, [feedId+isRead], [feedId+publishedAt], createdAt, updatedAt, extractedAt, readingTime',
        // Add new tables
  ...
  static async validateDatabaseIntegrity(): Promise<{
    ...
    try {
      ...
      if (orphanedSummaries > 0) {
        ...
        await db.summaries.where('articleId').noneOf(Array.from(articleIds)).delete();
      ...
      if (orphanedArticles > 0) {
        ...
        fixedIssues.push(`Cleaned up ${orphanedArticles} articles with invalid feeds`);
      }

      // Check for invalid dates
      const invalidDateArticles = await db.articles
        .filter(article => !(article.publishedAt instanceof Date) || isNaN(article.publishedAt.getTime()))
        .count();
      ...
      if (invalidDateArticles > 0) {
        ...
        fixedIssues.push(`Fixed ${invalidDateArticles} articles with invalid dates`);
      }

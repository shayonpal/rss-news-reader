...
export class ApiUsageRepository {
  ...
  async recordInoreaderCall(cost: number = 0): Promise<void> {
    ...
    try {
      const existing = await db.apiUsage
        .where('date')
        .equals(today)
        .first();
  ...
  async getWeeklyUsage(): Promise<{
    ...
    try {
      ...
      return {
        thisWeek: { inoreaderCalls: 0, claudeCalls: 0, date: 'aggregated', estimatedCost: 0 },
        lastWeek: { inoreaderCalls: 0, claudeCalls: 0, date: 'aggregated', estimatedCost: 0 }
      };
    }
  }

...
export class UserPreferencesRepository {
  ...
  async getPreferences(userId?: string): Promise<UserPreferences | null> {
    try {
      ...
      const { id, userId: storedUserId, version, updatedAt, ...userPrefs } = preferences;
      return userPrefs;
    } catch (error) {
  ...
  async getPreferencesHistory(userId?: string, limit: number = 10): Promise<Array<{
    ...
    try {
      ...
      return history.map(stored => {
        const { id, userId: storedUserId, version, updatedAt, ...preferences } = stored;
        return {
          preferences,
          version,
          updatedAt
  ...
  private validatePreferences(preferences: Partial<UserPreferences>): boolean {
    ...
    if (preferences.readingWidth && !validWidths.includes(preferences.readingWidth)) {
      return false;
    }
    
    if (preferences.syncFrequency !== undefined && 
        (preferences.syncFrequency < 1 || preferences.syncFrequency > 24)) {
      return false;
    }
    
    if (preferences.maxArticles !== undefined && 

 ...
 */
export function validatePreferences(data: PreferencesData): PreferencesValidationErrors {
 ...
 */
function validateAiPreferences(ai: PreferencesData['ai']): PreferencesValidationErrors['ai'] {
  ...
  if (!PREFERENCES_CONSTRAINTS.ai.contentFocusOptions.includes(ai.contentFocus)) {
 ...
 */
function validateSyncPreferences(sync: PreferencesData['sync']): PreferencesValidationErrors['sync'] {
 ...
 * @returns Validation errors object (empty if valid)
 */
export function validatePreferencesPatch(patch: PreferencesPatch): PreferencesValidationErrors {
  const errors: PreferencesValidationErrors = {};
  ...
  if (patch.ai) {
    ...
    if (patch.ai.model !== undefined && !PREFERENCES_CONSTRAINTS.ai.models.includes(patch.ai.model as any)) {
      aiErrors.model = `Invalid model. Must be one of: ${PREFERENCES_CONSTRAINTS.ai.models.join(', ')}`;
    }

    if (patch.ai.summaryLengthMin !== undefined) {
      if (patch.ai.summaryLengthMin < PREFERENCES_CONSTRAINTS.ai.summaryLengthMin.min) {
    ...
    if (patch.ai.summaryLengthMax !== undefined) {
      if (patch.ai.summaryLengthMax > PREFERENCES_CONSTRAINTS.ai.summaryLengthMax.max) {
        aiErrors.summaryLengthMax = `Maximum length cannot exceed ${PREFERENCES_CONSTRAINTS.ai.summaryLengthMax.max}`;
      }
    }

    if (patch.ai.summaryStyle !== undefined && !PREFERENCES_CONSTRAINTS.ai.summaryStyles.includes(patch.ai.summaryStyle)) {
      aiErrors.summaryStyle = `Invalid style. Must be one of: ${PREFERENCES_CONSTRAINTS.ai.summaryStyles.join(', ')}`;
    }

    if (patch.ai.contentFocus !== undefined && !PREFERENCES_CONSTRAINTS.ai.contentFocusOptions.includes(patch.ai.contentFocus)) {
 ...
 */
export function hasValidationErrors(errors: PreferencesValidationErrors): boolean {
...
export function sanitizePreferences(data: PreferencesData): PreferencesData {
  ...
  if (sanitized.ai) {
    ...
    if (sanitized.ai.summaryLengthMin > sanitized.ai.summaryLengthMax) {
      const avg = Math.floor((sanitized.ai.summaryLengthMin + sanitized.ai.summaryLengthMax) / 2);
      sanitized.ai.summaryLengthMin = avg;
      sanitized.ai.summaryLengthMax = avg;
    }

    // Validate enum values
    if (!PREFERENCES_CONSTRAINTS.ai.models.includes(sanitized.ai.model as any)) {
      sanitized.ai.model = PREFERENCES_CONSTRAINTS.ai.models[0];
    }
    if (!PREFERENCES_CONSTRAINTS.ai.summaryStyles.includes(sanitized.ai.summaryStyle)) {
      sanitized.ai.summaryStyle = PREFERENCES_CONSTRAINTS.ai.summaryStyles[0];
    }
    if (!PREFERENCES_CONSTRAINTS.ai.contentFocusOptions.includes(sanitized.ai.contentFocus)) {
      sanitized.ai.contentFocus = PREFERENCES_CONSTRAINTS.ai.contentFocusOptions[0] || 'general';
    }
  }

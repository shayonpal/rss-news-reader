...
describe('HTML Entity Decoder (RR-154)', () => {
  describe('decodeHtmlEntities', () => {
    it('should decode common HTML entities in text', () => {
      const input = 'Johnson &amp; Johnson&rsquo;s new &quot;breakthrough&quot; drug';
    ...
    it('should preserve URLs with query parameters', () => {
      const input = 'Visit https://example.com?param=value&amp;other=test for more info';
      const expected = 'Visit https://example.com?param=value&other=test for more info';
  ...
  describe('decodeContent', () => {
    it('should decode HTML entities in article content', () => {
      const input = '&lt;p&gt;Johnson &amp; Johnson announced that it&rsquo;s launching a new &quot;breakthrough&quot; treatment.&lt;/p&gt;';
      const expected = '<p>Johnson & Johnson announced that it\u2019s launching a new "breakthrough" treatment.</p>';
    ...
    it('should preserve HTML structure after decoding', () => {
      const input = '&lt;div class=&quot;article&quot;&gt;&lt;h2&gt;Tech &amp; Innovation&lt;/h2&gt;&lt;p&gt;AI&rsquo;s impact on society.&lt;/p&gt;&lt;/div&gt;';
      const expected = '<div class="article"><h2>Tech & Innovation</h2><p>AI\u2019s impact on society.</p></div>';
      
      expect(decodeContent(input)).toBe(expected);
    });

    it('should preserve URLs in content', () => {
      const input = '&lt;a href="https://example.com?utm_source=feed&amp;utm_medium=rss"&gt;Read more&lt;/a&gt;';
      const expected = '<a href="https://example.com?utm_source=feed&utm_medium=rss">Read more</a>';
  ...
  describe('batchDecodeArticles', () => {
    it('should decode HTML entities in multiple articles', async () => {
      ...
      expect(result.results[0].content).toBe('<p>Breaking news about tech giants.</p>');

      // Check second article
      expect(result.results[1].id).toBe('2');
      expect(result.results[1].title).toBe('Microsoft\u2019s "New Era"');
      expect(result.results[1].content).toBe('<div>Innovation & progress.</div>');
    });
    ...
    it('should collect errors but continue processing', async () => {
      const articles = [
        ...
        { id: '3', title: 'Another &rsquo;Valid&rsquo; Title', content: 'More content' }
      ];
  ...
  describe('Integration with Real RR-154 Data', () => {
    ...
    it('should handle actual problematic content from RR-154', () => {
      const problematicContent = '&lt;p&gt;Johnson &amp; Johnson announced today that it&rsquo;s launching a &quot;breakthrough&quot; treatment that could revolutionize healthcare. The company&rsquo;s CEO said, &quot;This represents a new era in medicine.&quot;&lt;/p&gt;&lt;p&gt;The treatment &ndash; developed over 5 years &ndash; shows promising results.&lt;/p&gt;';
      
      const expectedContent = '<p>Johnson & Johnson announced today that it\u2019s launching a "breakthrough" treatment that could revolutionize healthcare. The company\u2019s CEO said, "This represents a new era in medicine."</p><p>The treatment – developed over 5 years – shows promising results.</p>';

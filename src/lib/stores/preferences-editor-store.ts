 ...
 * Combined store type
 */
export type PreferencesEditorStore = PreferencesEditorState & PreferencesEditorActions;
  ...
  devtools(
    (set, get) => ({
      ...
      updateField: (path: string, value: string | number | boolean | null) => {
        ...
        const navigateToParent = (obj: PreferencesData, pathParts: string[]): Record<string, unknown> | null => {
        ...
        if (path === 'sync.maxArticles') {
          ...
          value = Math.max(min, Math.min(max, isNaN(numValue) ? 500 : numValue));
          ...
          value = Math.max(min, Math.min(max, isNaN(numValue) ? 100 : numValue));
          // Ensure min <= max
          ...
          value = Math.max(min, Math.min(max, isNaN(numValue) ? 300 : numValue));
          // Ensure min <= max
      ...
      validateAll: () => {
        ...
        if (draft.sync.maxArticles < 10 || draft.sync.maxArticles > 5000) {
          errors['sync.maxArticles'] = 'Max articles must be between 10 and 5000';
        }
        if (draft.sync.retentionCount < 1 || draft.sync.retentionCount > 365) {
          errors['sync.retentionCount'] = 'Retention count must be between 1 and 365';
        }
        if (draft.ai.summaryLengthMin < 50 || draft.ai.summaryLengthMin > 500) {
          errors['ai.summaryLengthMin'] = 'Min length must be between 50 and 500';
        }
        if (draft.ai.summaryLengthMax < 50 || draft.ai.summaryLengthMax > 500) {
          errors['ai.summaryLengthMax'] = 'Max length must be between 50 and 500';
        }

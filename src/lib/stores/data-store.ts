...
interface DataState {
  ...
  updatePreferences: (preferences: Partial<UserPreferences>, userId?: string) => Promise<boolean>;
    ...
    (set, get) => ({
      ...
      initializeDatabase: async () => {
        try {
          ...
          const validation = await DatabaseMigrations.validateDatabaseIntegrity();
          ...
          set({
            dbStatus: {
              ...
              lastError: validation.isValid ? undefined : validation.issues.join('; ')
            }
          });
      ...
      updatePreferences: async (newPreferences: Partial<UserPreferences>, userId?: string) => {
        try {
          const success = await userPreferencesRepository.updatePreferencesWithValidation(
            newPreferences, 
            userId
          );
      ...
      performMaintenance: async () => {
        try {
          ...
          if (result.articlesDeleted > 0 || result.apiRecordsDeleted > 0) {
            console.log(`Maintenance completed: ${result.articlesDeleted} articles, ${result.apiRecordsDeleted} API records cleaned`);
          }

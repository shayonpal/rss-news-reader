...
describe('UI Store Collapse State - RR-146 Specification', () => {
  ...
  describe('3. Performance & Optimization (7 tests)', () => {
    ...
    it('3.2 should not trigger unnecessary re-renders', () => {
      ...
      const TestComponent = () => {
        renderCount();
        const collapsed = (useUIStore as any)(state => state.feedsSectionCollapsed);
        return collapsed;
      };
    ...
    it('3.4 should handle concurrent access from multiple components', async () => {
      ...
      const states = results.map(r => (r.result.current as any).feedsSectionCollapsed);
    ...
    it('3.6 should optimize selector usage', () => {
      const collapseSelector = vi.fn((state: any) => state.feedsSectionCollapsed);

...
describe.sequential(
  "RR-112: Database Lifecycle Management and Race Condition Tests",
  () => {
    ...
    afterEach(async () => {
      ...
      if (currentTestDatabases.size > 0) {
        const cleanupPromises = Array.from(currentTestDatabases).map(
          async (db) => {
            await db.safeDelete();
          }
        );
      ...
      if (currentTestDbNames.size > 0) {
        const remainingCleanup = Array.from(currentTestDbNames).map(
          async (name) => {
            try {
    ...
    describe.sequential(
      "Category 1: Database Open/Close Behavior (6 tests)",
      () => {
        test("1.1 should open database successfully", async () => {
          testDb = new TestAppDatabase(
            generateUniqueDbName("open_test"),
            currentTestId
          );
        ...
        test("1.2 should handle opening already-open database", async () => {
          testDb = new TestAppDatabase(
            generateUniqueDbName("reopen_test"),
            currentTestId
          );
        ...
        test("1.3 should close and reopen database (close → open sequence)", async () => {
          testDb = new TestAppDatabase(
            generateUniqueDbName("close_reopen_test"),
            currentTestId
          );
        ...
        test("1.4 should handle simultaneous close calls without errors", async () => {
          testDb = new TestAppDatabase(
            generateUniqueDbName("simultaneous_close_test"),
            currentTestId
          );
        ...
        test("1.5 should perform read/write operations after open-close-reopen cycle", async () => {
          testDb = new TestAppDatabase(
            generateUniqueDbName("rw_cycle_test"),
            currentTestId
          );

          // Open and write data
          await testDb.open();
          await testDb.dbInfo.add({
            version: 1,
            createdAt: new Date(),
            corruptionCount: 0,
          });
        ...
        test("1.6 should handle multiple database instances in parallel", async () => {
          const db1 = new TestAppDatabase(
            generateUniqueDbName("parallel_1"),
            currentTestId
          );
          const db2 = new TestAppDatabase(
            generateUniqueDbName("parallel_2"),
            currentTestId
          );
          const db3 = new TestAppDatabase(
            generateUniqueDbName("parallel_3"),
            currentTestId
          );

          // Open all in parallel
          await Promise.all([db1.open(), db2.open(), db3.open()]);
    ...
    describe.sequential("Category 2: Deletion Edge Cases (6 tests)", () => {
      test("2.1 should close and delete database successfully (close → delete sequence)", async () => {
        testDb = new TestAppDatabase(
          generateUniqueDbName("close_delete_test"),
          currentTestId
        );
        ...
        await testDb.close();
        await expect(
          Dexie.delete(testDb.getDatabaseName())
        ).resolves.not.toThrow();
      });

      test("2.2 should handle deleting database that is still open", async () => {
        testDb = new TestAppDatabase(
          generateUniqueDbName("delete_open_test"),
          currentTestId
        );
      ...
      test("2.5 should handle rapid close → delete sequence", async () => {
        testDb = new TestAppDatabase(
          generateUniqueDbName("rapid_close_delete_test"),
          currentTestId
        );
        ...
        await expect(
          Promise.all([closePromise, deletePromise])
        ).resolves.not.toThrow();
      });

      test("2.6 should handle delete with simulated slow file removal", async () => {
        testDb = new TestAppDatabase(
          generateUniqueDbName("slow_delete_test"),
          currentTestId
        );
    ...
    describe.sequential(
      "Category 3: Concurrency/Race Conditions (8 tests)",
      () => {
        test("3.1 should handle concurrent close and delete from separate async routines", async () => {
          testDb = new TestAppDatabase(
            generateUniqueDbName("concurrent_close_delete_test"),
            currentTestId
          );
          ...
          await expect(
            Promise.all([closeRoutine, deleteRoutine])
          ).resolves.not.toThrow();
        });
        ...
        test("3.4 should handle database deletion while transaction is pending", async () => {
          ...
          // Start a transaction
          const transactionPromise = testDb.transaction(
            "rw",
            testDb.dbInfo,
            async () => {
        ...
        test("3.6 should handle interleaved open/close operations from multiple databases", async () => {
          const db1 = new TestAppDatabase(
            generateUniqueDbName("interleave_1"),
            currentTestId
          );
          const db2 = new TestAppDatabase(
            generateUniqueDbName("interleave_2"),
            currentTestId
          );
        ...
        test("3.7 should handle database recreation race conditions", async () => {
          ...
          const operations = Array.from({ length: 3 }, (_, i) =>
            (async () => {
              ...
              const newDb = new TestAppDatabase(
                `${dbName}_new_${i}`,
                currentTestId
              );
              await newDb.open();
        ...
        test("3.8 should prevent DatabaseClosedError during concurrent operations", async () => {
          testDb = new TestAppDatabase(
            generateUniqueDbName("closed_error_test"),
            currentTestId
          );
    ...
    describe.sequential("Category 4: Resource Management (5 tests)", () => {
      ...
      test("4.2 should handle databases with increasing complexity", async () => {
        ...
        for (let i = 1; i <= 3; i++) {
          operations.push(
            (async () => {
              const db = new TestAppDatabase(
                generateUniqueDbName(`complex_${i}`),
                currentTestId
              );
              await db.open();
      ...
      test("4.3 should verify no lingering IndexedDB connections after cleanup", async () => {
        testDb = new TestAppDatabase(
          generateUniqueDbName("lingering_connections_test"),
          currentTestId
        );
      ...
      test("4.5 should handle resource cleanup after operation timeout/abort", async () => {
        testDb = new TestAppDatabase(
          generateUniqueDbName("timeout_cleanup_test"),
          currentTestId
        );
    ...
    describe.sequential("Category 5: Multi-Test Isolation (5 tests)", () => {
      test("5.1 should use unique database names per test preventing cross-test pollution", async () => {
        ...
        await db1.dbInfo.add({
          version: 1,
          createdAt: new Date(),
          corruptionCount: 0,
        });
        await db2.dbInfo.add({
          version: 2,
          createdAt: new Date(),
          corruptionCount: 0,
        });
      ...
      test("5.3 should run parallel tests with different DB names without interference", async () => {
        const parallelTests = Array.from({ length: 3 }, (_, i) =>
          (async () => {
      ...
      test("5.5 should verify data stores don't auto-initialize in test environment", async () => {
        ...
        testDb = new TestAppDatabase(
          generateUniqueDbName("manual_init_test"),
          currentTestId
        );
        await testDb.open();
    ...
    describe.sequential(
      ...
      () => {
        ...
        test("should handle database corruption recovery scenario", async () => {
          testDb = new TestAppDatabase(
            generateUniqueDbName("corruption_recovery_test"),
            currentTestId
          );
          ...
          const recoveredDb = new TestAppDatabase(

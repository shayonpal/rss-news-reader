...
import type { UISystemHealth, HealthAlert, HealthCheckHistory, HealthStatus } from '@/types/health';
...
describe('HealthStore', () => {
  ...
  describe('performHealthCheck', () => {
    ...
    it('should handle check errors', async () => {
      ...
      (healthCheckService.checkHealth as any).mockRejectedValue(new Error(errorMessage));
  ...
  describe('acknowledgeAlert', () => {
    it('should acknowledge alert and update unread count', () => {
      ...
      const updatedState = useHealthStore.getState();
      const acknowledgedAlert = updatedState.alerts.find(a => a.id === 'alert-1');

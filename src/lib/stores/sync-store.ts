...
export interface SyncState {
  ...
  setRateLimit: (rateLimit: { used: number; limit: number; remaining: number } | null) => void;
  ...
  persist(
    (set, get) => ({
      ...
      performFullSync: async () => {
        const { setSyncing, setSyncError, setLastSyncTime, setSyncProgress, setSyncMessage, setRateLimit, processQueue } = get();
        ...
        try {
          ...
          if (!response.ok) {
            ...
            if (response.status === 429 && error.error === 'rate_limit_exceeded') {
              const rateInfo = error.limit ? ` (${error.used}/${error.limit} calls used today)` : '';
              throw new Error(`API rate limit exceeded${rateInfo}. Try again tomorrow.`);
            }
          ...
          if (rateLimit) {
            ...
            console.log(`API usage: ${rateLimit.used}/${rateLimit.limit} calls today`);
            if (rateLimit.used >= rateLimit.limit * 0.8) {
              console.warn(`Warning: Approaching API rate limit (${rateLimit.remaining} calls remaining)`);
            }
          }
          ...
          while (!syncComplete && pollCount < maxPolls) {
            ...
            const statusResponse = await fetch(`/reader/api/sync/status/${syncId}`);
            if (statusResponse.ok) {
              const status = await statusResponse.json();
              console.log(`Sync progress: ${status.progress}% - ${status.message || 'Processing...'}`);
...
async function processAction(action: QueuedAction): Promise<void> {
  ...
  console.log(`Action ${action.type} for article ${action.articleId} already applied to Supabase`);
}

...
describe("RR-106: Monitoring Integration Tests", () => {
  ...
  describe("Monitor Dashboard Script Integration", () => {
    it("should execute monitor-dashboard.sh without errors", async () => {
      ...
      try {
        ...
        if (error.signal === "SIGTERM") {
          ...
          console.warn(
            "Monitor dashboard script error (expected during testing):",
            error.message
          );
        }
      }
    ...
    it("should verify monitor-dashboard.sh references correct endpoints only", async () => {
      ...
      if (!hasFreshnessEndpoint) {
        ...
        console.log(
          "Monitor dashboard still contains freshness checks (expected during removal process)"
        );
      }
    });
  });
  ...
  describe("Sync Health Monitor Integration", () => {
    it("should execute sync-health-monitor.sh without syntax errors", async () => {
      ...
      try {
        ...
        const { stdout, stderr } = await execAsync(
          `bash "${scriptPath}" --help || echo "help not available"`,
          {
            cwd: PROJECT_ROOT,
          }
        );
    ...
    it("should verify sync-health-monitor.sh references correct endpoints", async () => {
      ...
      try {
        ...
        if (!hasFreshnessReferences) {
          ...
          console.log(
            "Sync health monitor still contains freshness references (expected during removal process)"
          );
        }
      } catch (error) {
  ...
  describe("Uptime Kuma Monitor Configuration", () => {
    it("should verify setup-sync-monitors.js excludes freshness after removal", async () => {
      ...
      if (!hasFreshnessMonitor) {
        ...
        console.log(
          "Uptime Kuma setup still includes freshness monitor (expected during removal process)"
        );
      }

      // Should still include other essential monitors
      const hasAppHealth = monitorConfigs.some(
        (config) =>
          config.toLowerCase().includes("app") ||
          config.toLowerCase().includes("health")
      );
  ...
  describe("Build Validation Integration", () => {
    it("should verify validate-build.sh works without freshness checks", async () => {
      ...
      try {
        ...
        const { stdout, stderr } = await execAsync(
          `timeout 60s bash "${scriptPath}" --mode quick`,
          {
            cwd: PROJECT_ROOT,
          }
        );
        ...
        if (error.signal === "SIGTERM") {
          ...
          console.log(
            "Build validation timed out (expected for comprehensive validation)"
          );
        } else {
  ...
  describe("Service Health Endpoint Integration", () => {
    it("should verify remaining health endpoints respond correctly", async () => {
      ...
      for (const endpoint of healthEndpoints) {
        try {
          ...
          console.warn(
            `Health endpoint ${endpoint.path} not accessible in test environment`
          );
        }
      }
    });
    ...
    it("should verify freshness endpoint returns 404 after removal", async () => {
      try {
        const response = await fetch(
          "http://localhost:3002/reader/api/health/freshness"
        );
        ...
        if (response.status === 404) {
          ...
          console.log(
            `Freshness endpoint still accessible with status ${response.status} (expected during removal process)`
          );
        }
      } catch (error) {
  ...
  describe("Monitoring Log Integration", () => {
    it("should verify monitoring logs are still generated", async () => {
      ...
      for (const logPath of logPaths) {
        ...
        try {
          ...
          console.warn(
            `Log file ${logPath} not found (acceptable in test environment)`
          );
        }
      }
    });
    ...
    it("should verify monitoring logs do not contain freshness errors after removal", async () => {
      ...
      try {
        ...
        for (const line of lines) {
          try {
            ...
            if (entry.service && entry.service.includes("freshness")) {
              console.log(
                "Found freshness reference in monitoring log (expected during removal process)"
              );
            }
          } catch {
        ...
        console.warn(
          "Could not read monitoring log (acceptable in test environment)"
        );
      }
    });
  });

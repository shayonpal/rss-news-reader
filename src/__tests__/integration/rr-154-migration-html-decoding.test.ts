...
async function executeHtmlEntityMigration(supabase: SupabaseClient): Promise<MigrationResult> {
  ...
  try {
    ...
    const { data: articlesWithEntities } = await supabase
      ...
      .or('title.ilike.%&amp;%,title.ilike.%&rsquo;%,title.ilike.%&lsquo;%,title.ilike.%&quot;%,title.ilike.%&#8217;%,title.ilike.%&ndash;%,content.ilike.%&amp;%,content.ilike.%&rsquo;%,content.ilike.%&lsquo;%,content.ilike.%&quot;%,content.ilike.%&#8217;%,content.ilike.%&ndash;%');
    ...
    for (let i = 0; i < articlesWithEntities.length; i += chunkSize) {
      ...
      try {
        ...
        errors.push(`Chunk ${i}-${i + chunk.length - 1}: ${(chunkError as Error).message}`);
      }
    }
...
function decodeHtmlEntitiesForMigration(text: string | null): string {
  ...
  return text.replace(/&(?:#8217|amp|rsquo|lsquo|quot|ndash|mdash|hellip|lt|gt);/g, (match) => {
...
describe('RR-154: HTML Entity Migration Database Tests', () => {
    ...
    mockArticles = [
      {
        ...
        content: '&lt;p&gt;Johnson &amp; Johnson announced &quot;breakthrough&quot; results.&lt;/p&gt;'
      },
      {
        id: 'article-2', 
        title: '&lsquo;AI Revolution&rsquo; &ndash; Tech Giants&rsquo; Response',
        content: '&lt;div&gt;The AI revolution &amp; tech companies&rsquo; strategies.&lt;/div&gt;'
      },
      {
        id: 'article-3',
            ...
            select: vi.fn(() => ({
              or: vi.fn(() => Promise.resolve({
                data: mockArticles.filter(article => 
                  (article.title && /&(?:amp|rsquo|lsquo|quot|#8217|ndash);/.test(article.title)) ||
                  (article.content && /&(?:amp|rsquo|lsquo|quot|#8217|ndash);/.test(article.content))
                )
              }))
            })),
  ...
  it('should execute migration and decode HTML entities correctly', async () => {
    ...
    expect(article1?.content).toBe('<p>Johnson & Johnson announced "breakthrough" results.</p>');

    const article2 = decodedArticles.find((a: any) => a.id === 'article-2');
    expect(article2?.title).toBe('\u2018AI Revolution\u2019 – Tech Giants\u2019 Response');
    expect(article2?.content).toBe('<div>The AI revolution & tech companies\u2019 strategies.</div>');
    ...
    expect(article5?.content).toBe('<p>Content with entities but null title.</p>');
  });
  ...
  it('should handle chunked processing for large datasets', async () => {
    ...
    upsertCalls.forEach((call, chunkIndex) => {
      ...
      chunk.forEach((article: any, articleIndex: number) => {
        ...
        expect(article.title).toBe(`Article ${expectedIndex} & Company\u2019s News`);
        expect(article.content).toBe(`<p>Content ${expectedIndex} with "entities" & symbols.</p>`);
      });
    });
  });
  ...
  it('should handle migration errors gracefully', async () => {
    ...
    mockSupabase.from = vi.fn((table: string) => {
      if (table === 'articles') {
        return {
          select: vi.fn(() => ({
            or: vi.fn(() => Promise.resolve({ data: mockArticles.slice(0, 2) }))
          })),
  ...
  it('should record migration completion in sync_metadata', async () => {
    ...
    const metadataUpserts = mockSupabase.from('sync_metadata').upsert.mock.calls;
  ...
  it('should preserve non-HTML entity content during migration', async () => {
    ...
    const mixedContent = [
      {
        ...
        content: '&lt;p&gt;Partnership between J&amp;J and AT&T announced.&lt;/p&gt;'
      },
      {
        id: 'mixed-2', 
    ...
    expect(article1?.content).toBe('<p>Partnership between J&J and AT&T announced.</p>');

  ...
  describe('Sync Pipeline Conflict Detection', () => {
    it('should detect conflicts during full sync operation', async () => {
      ...
      const performSyncWithConflictDetection = async () => {
        ...
        for (const local of mockLocalArticles) {
          const remote = mockRemoteArticles.find(r => r.id === local.inoreader_id);
          if (!remote) continue;

          const remoteIsRead = remote.categories.includes('user/-/state/com.google/read');
          const remoteIsStarred = remote.categories.includes('user/-/state/com.google/starred');

          const hasLocalChanges = local.last_local_update > local.last_sync_update;
          const statesDiffer = local.is_read !== remoteIsRead || 
    ...
    it('should handle concurrent conflict logging', async () => {
      const logConflictBatch = async (conflicts: any[]) => {
        const logLines = conflicts.map(c => JSON.stringify(c) + '\n').join('');
      ...
      const batch2 = Array(5).fill(null).map((_, i) => ({
        timestamp: new Date().toISOString(),
      ...
      await Promise.all([
        logConflictBatch(batch1),
        logConflictBatch(batch2)
      ]);
    ...
    it('should correctly map Inoreader categories to read/starred states', () => {
      const mapInoreaderCategories = (categories: string[]) => {
        return {
          ...
          is_broadcast: categories.includes('user/-/state/com.google/broadcast')
        };
      };
    ...
    it('should handle missing timestamps from Inoreader API', () => {
      const processArticleWithMissingTimestamp = (article: any) => {
        ...
        return {
          ...
          crawled: article.crawlTimeMsec ? new Date(article.crawlTimeMsec) : null,
    ...
    it('should track conflicts across entire sync session', async () => {
      class SyncSessionTracker {
        ...
        processArticle(local: any, remote: any) {
          ...
          const hasLocalChanges = local.last_local_update && 
    ...
    it('should generate comprehensive sync report with conflicts', async () => {
      const generateSyncReport = (sessionData: any) => {
        const report = {
          ...
          conflictBreakdown: {
            readStatusConflicts: sessionData.conflicts.filter((c: any) => c.type === 'read_status').length,
            starredStatusConflicts: sessionData.conflicts.filter((c: any) => c.type === 'starred_status').length,
            bothConflicts: sessionData.conflicts.filter((c: any) => c.type === 'both').length
          },
          resolutionSummary: {
          ...
          recommendations: sessionData.conflicts.length > 10 

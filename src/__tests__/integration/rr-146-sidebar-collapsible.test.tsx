...
const mockFeedsWithCounts: Map<string, FeedWithUnreadCount> = new Map([
  ['feed-1', { ...mockFeeds.get('feed-1')!, unreadCount: 15 } as FeedWithUnreadCount],
  ['feed-2', { ...mockFeeds.get('feed-2')!, unreadCount: 0 } as FeedWithUnreadCount],
  ['feed-3', { ...mockFeeds.get('feed-3')!, unreadCount: 7 } as FeedWithUnreadCount],
]);
...
describe('RR-146: SimpleFeedSidebar Collapsible Integration Tests', () => {
  ...
  describe('App Branding', () => {
    ...
    it('should position branding above action buttons', () => {
      ...
      const header = screen.getByText('RSS Reader').closest('.border-b') as HTMLElement;
  ...
  describe('Feed Selection and URL Updates', () => {
    ...
    it('should maintain selection when toggling collapse', async () => {
      ...
      expect(screen.getByText('Tech News').closest('div')).toHaveClass('bg-muted');
    });
  });
  ...
  describe('Session State Persistence', () => {
    ...
    it('should not persist collapse state to localStorage', () => {
      ...
      const collapseStateCalls = calls.filter(call => 
        call[1] && call[1].includes('filterSections')
      );
  ...
  describe('Accessibility', () => {
    it('should have proper ARIA labels for collapsible section', () => {
      ...
      expect(trigger).toHaveAttribute('aria-label', expect.stringContaining('Feeds'));
    });

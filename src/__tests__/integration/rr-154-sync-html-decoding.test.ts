...
describe('RR-154: Sync Pipeline HTML Entity Decoding Integration', () => {
          ...
          return Promise.resolve({
            ...
            json: () => Promise.resolve({
              items: [{
                ...
                title: 'Johnson &amp; Johnson&rsquo;s Q3 Results &ndash; &quot;Record Breaking&quot;',
                author: 'Financial Reporter',
                content: { 
                  content: '&lt;p&gt;Johnson &amp; Johnson announced today that it&rsquo;s launching a &quot;breakthrough&quot; treatment. The company&rsquo;s CEO said, &quot;This represents a new era &ndash; one of innovation &amp; progress.&quot;&lt;/p&gt;&lt;p&gt;Visit &lt;a href="https://jnj.com?source=press&amp;utm_medium=email"&gt;their website&lt;/a&gt; for more details.&lt;/p&gt;'
                },
                ...
                title: '&lsquo;AI Revolution&rsquo; &amp; Tech Giants&rsquo; Response',
                author: 'Tech Analyst',
                content: { 
                  content: '&lt;div class=&quot;article-body&quot;&gt;&lt;h2&gt;Breaking News&lt;/h2&gt;&lt;p&gt;The AI revolution is here &ndash; and tech companies can&rsquo;t ignore it. Apple, Google &amp; Microsoft are racing to integrate AI into their platforms.&lt;/p&gt;&lt;p&gt;As one expert noted: &quot;We&rsquo;re at an inflection point.&quot;&lt;/p&gt;&lt;/div&gt;'
                },
                ...
                content: { 
                  content: '<p>This content has no HTML entities to decode.</p><p>Just regular text with normal punctuation.</p>'
                },
  ...
  it('should decode HTML entities in article titles during sync', async () => {
    ...
    const articleWithEncodedTitle = articlesUpserted.find(a => 
      a.inoreader_id === 'tag:google.com,2005:reader/item/encoded-title'
    );
    
    expect(articleWithEncodedTitle).toBeDefined();
    expect(articleWithEncodedTitle.title).toBe('Johnson & Johnson\u2019s Q3 Results – "Record Breaking"');
    
    // Find and verify the article with mixed entities
    const articleWithMixedEntities = articlesUpserted.find(a => 
      a.inoreader_id === 'tag:google.com,2005:reader/item/mixed-entities'
    );
    
    expect(articleWithMixedEntities).toBeDefined();
    expect(articleWithMixedEntities.title).toBe('\u2018AI Revolution\u2019 & Tech Giants\u2019 Response');
    
    // Find and verify the article with no entities (should remain unchanged)
    const articleWithoutEntities = articlesUpserted.find(a => 
      a.inoreader_id === 'tag:google.com,2005:reader/item/no-entities'
    );
    ...
    const articleWithNullContent = articlesUpserted.find(a => 
      a.inoreader_id === 'tag:google.com,2005:reader/item/null-content'
    );
  ...
  it('should decode HTML entities in article content during sync', async () => {
    ...
    const articleWithEncodedContent = articlesUpserted.find(a => 
      a.inoreader_id === 'tag:google.com,2005:reader/item/encoded-title'
    );
    ...
    const articleWithMixedContent = articlesUpserted.find(a => 
      a.inoreader_id === 'tag:google.com,2005:reader/item/mixed-entities'
    );
    ...
    const articleWithPlainContent = articlesUpserted.find(a => 
      a.inoreader_id === 'tag:google.com,2005:reader/item/no-entities'
    );
  ...
  it('should preserve URLs and query parameters in content', async () => {
    ...
    const articleWithUrl = articlesUpserted.find(a => 
      a.inoreader_id === 'tag:google.com,2005:reader/item/encoded-title'
    );
    
    expect(articleWithUrl).toBeDefined();
    expect(articleWithUrl.content).toContain('https://jnj.com?source=press&utm_medium=email');
    expect(articleWithUrl.content).toContain('<a href="https://jnj.com?source=press&utm_medium=email">');
  });
  ...
  it('should maintain article metadata during HTML entity decoding', async () => {
    ...
    const readArticle = articlesUpserted.find(a => 
      a.inoreader_id === 'tag:google.com,2005:reader/item/mixed-entities'
    );
    expect(readArticle?.is_read).toBe(true);

    const starredArticle = articlesUpserted.find(a => 
      a.inoreader_id === 'tag:google.com,2005:reader/item/no-entities'
    );
  ...
  it('should handle sync performance requirements with HTML decoding', async () => {
    ...
    articlesUpserted.forEach((article, i) => {
      expect(article.title).toBe(`Performance Test ${i} & Company\u2019s Results – "Breaking"`);
      expect(article.content).toContain('<p>');
      expect(article.content).toContain('"quotes"');
      expect(article.content).toContain('& symbols');
      expect(article.content).toContain('CEO announced – "This is revolutionary."');
    });
          ...
          return Promise.resolve({
            ...
            json: () => Promise.resolve({
              items: [{
                ...
                title: 'Malformed &amp;&rsquo;&quot; &incomplete; &invalid entities',
                content: { 
                  content: '&lt;p&gt;Content with &malformed; and &amp incomplete entities&lt;/p&gt;'
                },

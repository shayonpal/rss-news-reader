...
describe("RR-116: 404 Page Content Validation", () => {
  beforeAll(async () => {
    ...
    await new Promise<void>((resolve) => {
      server.listen(testPort, () => {
        console.log(
          `> RR-116 Test server ready on http://localhost:${testPort}`
        );
        resolve();
      });
    });
  ...
  describe("Test 5 Replacement: Global 404 Page Validation", () => {
    ...
    it("should include navigation back to home/reader", async () => {
      const response = await fetch(
        `http://localhost:${testPort}/reader/non-existent-page`
      );
      const html = await response.text();
  ...
  describe("Test 6 Replacement: Article-Specific 404 Page Validation", () => {
    ...
    it("should not expose sensitive information in article pages", async () => {
      const response = await fetch(
        `http://localhost:${testPort}/reader/article/sensitive-test-id`
      );
      const html = await response.text();
  ...
  describe("Test 7 Replacement: Consistent Styling Between 404 Pages", () => {
    it("should have consistent approach to 404 handling", async () => {
      ...
      const articleResponse = await fetch(
        `http://localhost:${testPort}/reader/article/non-existent-id`
      );
    ...
    it("should have consistent button/link styling", async () => {
      ...
      const globalNavigation = globalHtml.match(
        /<(?:button|a)[^>]*(?:class|href)[^>]*>/i
      );
    ...
    it("should maintain consistent spacing and layout patterns", async () => {
      ...
      const globalHasSpacing = spacingClasses.some((cls) =>
        globalHtml.includes(cls)
      );
      const articleHasSpacing = spacingClasses.some((cls) =>
        articleHtml.includes(cls)
      );
  ...
  describe("BasePath '/reader' Handling", () => {
    it("should handle 404s correctly with basePath", async () => {
      ...
      const withoutBasePath = await fetch(
      ...
      const withBasePath = await fetch(
        `http://localhost:${testPort}/reader/non-existent`
      );
    ...
    it("should generate correct navigation URLs with basePath", async () => {
      ...
      if (html.includes("href=")) {
        ...
        if (links && links.length > 0) {
          const hasCorrectBasePath = links.some(
            (link) => link.includes("/reader") || link.includes("/")
          );
  ...
  describe("Error Handling and Edge Cases", () => {
    ...
    it("should not cache 404 responses aggressively", async () => {
      const response = await fetch(
        `http://localhost:${testPort}/reader/cache-test-404`
      );
      ...
      if (cacheControl) {
        ...
        expect(cacheControl).toMatch(
          /no-cache|no-store|max-age=0|max-age=[1-9]\d{0,2}$/
        );
      }
    });
  });

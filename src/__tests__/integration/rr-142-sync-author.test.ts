...
describe("RR-142: Integration test for author field extraction", () => {
  ...
  it("should include author field when syncing articles from Inoreader", async () => {
    ...
    const articleWithAuthor = articlesUpserted.find(
      (a) => a.inoreader_id === "tag:google.com,2005:reader/item/test123"
    );
    ...
    const articleWithoutAuthor = articlesUpserted.find(
      (a) => a.inoreader_id === "tag:google.com,2005:reader/item/test456"
    );

...
describe("RR-154: Database Migration for HTML Entity Decoding", () => {
    ...
    migrationScript = {
          ...
          try {
            ...
            for (const update of updates) {
              ...
              if (updateError) {
                results.errorCount++;
                results.errors.push(
                  `Article ${update.id}: ${updateError.message}`
                );
              } else {
      ...
      async verifyMigration() {
        ...
        return {
          ...
          migrationComplete:
            remainingEntities.titlesCount === 0 &&
            remainingEntities.contentsCount === 0,
        };
      },
      ...
      async runMigration() {
        ...
        try {
          ...
          console.log(
            `Found ${articlesWithEntities.titlesCount} titles and ${articlesWithEntities.contentsCount} contents with entities`
          );

          // Step 3: Process titles
          if (articlesWithEntities.titleIds.length > 0) {
            console.log("Processing titles...");
            const titleResults = await this.processInBatches(
              articlesWithEntities.titleIds,
              "title"
            );
          ...
          if (articlesWithEntities.contentIds.length > 0) {
            ...
            const contentResults = await this.processInBatches(
              articlesWithEntities.contentIds,
              "content"
            );
          ...
          results.totalProcessed =
            results.titlesProcessed + results.contentsProcessed;
  ...
  describe("1. Article Discovery", () => {
    ...
    it("should correctly identify articles with HTML entities in content", async () => {
      ...
      const articlesWithContentEntities = Array.from(
        { length: 277 },
        (_, i) => ({
          id: `content-${i}`,
  ...
  describe("2. Batch Processing", () => {
    it("should process articles in batches of 200", async () => {
      ...
      const results = await migrationScript.processInBatches(
        articleIds,
        "title"
      );
    ...
    it("should add delay between batches to prevent overload", async () => {
      ...
      expect(duration).toBeGreaterThanOrEqual(
        migrationScript.DELAY_BETWEEN_BATCHES
      );
    });
  ...
  describe("3. Entity Decoding", () => {
    ...
    it("should preserve URLs during migration", async () => {
      ...
      expect(articles[0].url).toBe(
        "https://example.com/article?param=value&test=1"
      );
  ...
  describe("4. Backup and Recovery", () => {
    ...
    it("should abort migration if backup fails", async () => {
      ...
      expect(results.errors).toContain(
        "Migration failed: Error: Backup creation failed"
      );
  ...
  describe("6. Full Migration Flow", () => {
    it("should complete full migration successfully", async () => {
      ...
      expect(results.totalProcessed).toBe(
        results.titlesProcessed + results.contentsProcessed
      );
    ...
    it("should handle partial migration failure", async () => {
      ...
      mockSupabase.in.mockReturnValueOnce({
        data: [
          { id: "1", title: "Title with &amp;", url: "https://example.com" },
        ],
        error: null,
      });

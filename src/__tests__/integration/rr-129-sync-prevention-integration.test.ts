...
describe("RR-129: Sync Prevention Integration", () => {
  ...
  const simulateSync = async (feedId: string, inoreaderArticles: any[]) => {
    ...
    const deletedIds = new Set(
      deletedTrackingData?.map((d) => d.inoreader_id) || []
    );
    ...
    for (const article of inoreaderArticles) {
      ...
      const isReadInInoreader = article.categories?.includes(
  ...
  describe("Delete → Sync → No Re-import Flow", () => {
    it("should not re-import previously deleted read articles", async () => {
      ...
      const initialSyncResult = await simulateSync(
        feed.id,
        initialInoreaderArticles
      );
      ...
      const subsequentSyncResult = await simulateSync(
        feed.id,
        subsequentInoreaderArticles
      );
      ...
      const { data: trackingEntries } = await supabase
        ...
        .select("inoreader_id")
        .in("inoreader_id", [
          "test-rr129-sync-article-1",
          "test-rr129-sync-article-2",
        ]);
    ...
    it("should handle starred articles that become unstarred", async () => {
      ...
      const inoreaderArticles = [
        createInoreaderArticle(
          "test-rr129-sync-becomes-unstarred",
          true,
          false
        ), // Read but not starred
      ];
  ...
  describe("Complex Sync Scenarios", () => {
    it("should handle mixed article states correctly during sync", async () => {
      ...
      const articlesMap = new Map(
        finalArticles!.map((a) => [a.inoreader_id, a])
      );
      ...
      finalArticles?.forEach((a) => {
        const originalArticle = setupArticleData?.find(
          (orig) => orig.inoreader_id === a.inoreader_id
        );
        if (originalArticle) {
    ...
    it("should handle state transitions from deleted tracking", async () => {
      ...
      const { data: remainingTracking } = await supabase
        ...
        .select("inoreader_id")
        .in("inoreader_id", [
          "test-rr129-sync-transition-1",
          "test-rr129-sync-transition-2",
        ]);

      expect(remainingTracking).toHaveLength(1);
      expect(remainingTracking![0].inoreader_id).toBe(
        "test-rr129-sync-transition-1"
      );
  ...
  describe("Performance and Scale Testing", () => {
    it("should handle large numbers of tracked deletions efficiently", async () => {
      ...
      await supabase.from("deleted_articles").insert(trackingEntries);

      // Create large sync batch with mixed scenarios
      const syncBatchSize = 300;
      const inoreaderArticles = Array.from(
        { length: syncBatchSize },
        (_, i) => {
      ...
      expect(syncResult.syncedCount + syncResult.skippedCount).toBe(
        syncBatchSize
      );
      expect(syncResult.skippedCount).toBeGreaterThan(0); // Some should be skipped due to tracking

      console.log(
        `Synced ${syncResult.syncedCount} articles, skipped ${syncResult.skippedCount} in ${syncTime}ms`
      );
    });
    ...
    it("should maintain consistent performance with growing tracking table", async () => {
      ...
      for (const batchSize of batchSizes) {
        ...
        await supabase.from("articles").delete().eq("feed_id", feed.id);
      }
      ...
      console.log(
        `Lookup times for ${batchSizes.join(", ")} tracking entries: ${lookupTimes.join(", ")}ms`
      );
    });
  });
  ...
  describe("Error Handling and Edge Cases", () => {
    it("should handle database errors during tracking lookup gracefully", async () => {
      ...
      const problematicArticles = [
        {
          ...createInoreaderArticle("test-rr129-sync-valid", false),
          id: "test-rr129-sync-valid",
        },

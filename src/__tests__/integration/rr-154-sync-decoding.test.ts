  ...
  beforeEach(() => {
    ...
    mockDecoder = {
      ...
      decodeArticleData: vi.fn((article) => {
        ...
        return {
          ...article,
          title: article.title ? mockDecoder.decodeHtmlEntities(article.title) : article.title,
          content: article.content ? mockDecoder.decodeHtmlEntities(article.content) : article.content
        };
      })
    };
  ...
  describe('1. Sync Route Processing', () => {
    describe('1.1 Article Processing with Decoding', () => {
      it('should decode HTML entities in article titles during sync', async () => {
          ...
          items: [
            {
              ...
              summary: {
                content: 'The president&rsquo;s &ldquo;bold approach&rdquo; to R&amp;D'
              },
            ...
            },
            {
              ...
              summary: {
                content: 'Industry leaders discuss &ldquo;innovation&rdquo; &ndash; key insights'
              },
              canonical: [{ href: 'https://example.com/article-2?param=value&test=1' }],
        ...
        expect(processedArticles[0].title).toBe("Biden's New Policy & Strategy");
        expect(processedArticles[0].content).toBe('The president\'s "bold approach" to R&D');
        ...
        expect(processedArticles[1].content).toBe('Industry leaders discuss "innovation" – key insights');
        expect(processedArticles[1].url).toBe('https://example.com/article-2?param=value&test=1');
      ...
      it('should handle articles with no entities gracefully', async () => {
        ...
        expect(processedArticle.content).toBe('Plain content without any HTML entities');
      });
    ...
    describe('1.2 Batch Processing Performance', () => {
      it('should efficiently process large batches of articles', async () => {
        ...
        const items = Array.from({ length: 500 }, (_, i) => ({
          ...
          canonical: [{ href: `https://example.com/article-${i}?id=${i}&test=true` }],
        ...
        expect(processedArticles[0].title).toBe("Article 0 & Title with ' apostrophe");
        expect(processedArticles[0].content).toBe('Content 0 with "quotes" – testing');
        expect(processedArticles[0].url).toBe('https://example.com/article-0?id=0&test=true');

        expect(processedArticles[499].title).toBe("Article 499 & Title with ' apostrophe");
        expect(processedArticles[499].content).toBe('Content 499 with "quotes" – testing');
        expect(processedArticles[499].url).toBe('https://example.com/article-499?id=499&test=true');
      });
    });
  ...
  describe('3. Sync Route Integration Points', () => {
    describe('3.1 Integration with Existing Sync Logic', () => {
      it('should decode entities at the correct point in sync pipeline', () => {
        ...
        expect(expectedIndex).toBeGreaterThan(syncPipeline.indexOf('transformArticleData'));
        expect(expectedIndex).toBeLessThan(syncPipeline.indexOf('saveToDatabase'));
      });
    });

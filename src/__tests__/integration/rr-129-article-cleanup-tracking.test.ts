...
describe('RR-129: Article Cleanup with Tracking Integration', () => {
  ...
  describe('Read Article Cleanup with Tracking', () => {
    it('should track and delete read, unstarred articles', async () => {
      ...
      expect(articlesToDelete![0].inoreader_id).toBe('test-rr129-cleanup-read-unstarred');
      ...
      const readStarred = remainingArticles!.find(a => a.inoreader_id === 'test-rr129-cleanup-read-starred');
      const unread = remainingArticles!.find(a => a.inoreader_id === 'test-rr129-cleanup-unread');
      ...
      remainingArticles?.forEach(article => {
        const originalArticle = articles!.find(a => a.inoreader_id === article.inoreader_id);
        if (originalArticle) {
  ...
  describe('Tracking Table Operations', () => {
    ...
    it('should support batch operations for large cleanups', async () => {
      ...
      const { count: trackingCount } = await supabase
        ...
        .in('inoreader_id', toDelete!.map(a => a.inoreader_id));
  ...
  describe('Tracking Cleanup and Maintenance', () => {
    it('should clean up old tracking entries (>90 days)', async () => {
      ...
      const recentTimestamp = new Date(Date.now() - (50 * 24 * 60 * 60 * 1000)).toISOString(); // 50 days ago
      ...
      const { data: remainingEntries } = await supabase
        .from('deleted_articles')
        .select('inoreader_id')
        .in('inoreader_id', oldEntries.map(e => e.inoreader_id));

      expect(remainingEntries).toHaveLength(1);
      expect(remainingEntries![0].inoreader_id).toBe('test-rr129-cleanup-recent');
    });
  ...
  describe('Integration with Article States', () => {
    it('should handle mixed article states correctly', async () => {
      ...
      const mixedArticles = [
        { inoreader_id: 'mixed-unread-unstarred', is_read: false, is_starred: false },
        { inoreader_id: 'mixed-unread-starred', is_read: false, is_starred: true },
        { inoreader_id: 'mixed-read-unstarred', is_read: true, is_starred: false },
      ...
      expect(toDelete).toHaveLength(1);
      expect(toDelete![0].inoreader_id).toBe('test-rr129-cleanup-mixed-read-unstarred');

...
describe('RR-5: Sync Status Display - Integration Tests', () => {
  ...
  describe('End-to-End Sync Flow with Header Capture', () => {
    it('should capture and store zone usage from sync operation', async () => {
      ...
      // Trigger sync
      const syncResponse = await fetch(`http://localhost:${TEST_PORT}/reader/api/sync`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const usageResponse = await fetch(`http://localhost:${TEST_PORT}/reader/api/sync/api-usage`);
  ...
  describe('API Usage Endpoint', () => {
    it('should return current zone usage from database', async () => {
      ...
      const response = await fetch(`http://localhost:${TEST_PORT}/reader/api/sync/api-usage`);
  ...
  describe('Progressive Zone Usage Updates', () => {
    it('should track increasing zone usage across multiple API calls', async () => {
      ...
      for (const usage of usageSequence) {
        vi.mocked(fetch).mockImplementation((url) => {
          if (url.toString().includes('inoreader.com')) {
            return Promise.resolve(mockInoreaderResponse(usage.zone1, usage.zone2));
          }
          return Promise.resolve(new Response('Not found', { status: 404 }));
        });
    ...
    it('should handle zone limit changes', async () => {
      ...
      const mockResponseWithNewLimits = new Response(JSON.stringify({ success: true }), {
        status: 200,
        headers: {
          'Content-Type': 'application/json',
  ...
  describe('Error Scenarios', () => {
    it('should handle missing headers gracefully', async () => {
      ...
      const mockResponseNoHeaders = new Response(JSON.stringify({ success: true }), {
        status: 200,
        headers: {
          'Content-Type': 'application/json',
        },

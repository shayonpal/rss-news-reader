...
describe('RR-171: RefreshManager Contract', () => {
  ...
  describe('Background Sync Behavior', () => {
    ...
    it('should handle background sync errors silently', async () => {
      mockFeedStore.refreshFeeds = vi.fn().mockRejectedValue(new Error('Network error'));
  ...
  describe('State Management', () => {
    it('should track last refresh time', async () => {
      ...
      expect(refreshManager.lastRefreshTime!.getTime()).toBeGreaterThanOrEqual(beforeTime.getTime());
      expect(refreshManager.lastRefreshTime!.getTime()).toBeLessThanOrEqual(afterTime.getTime());
    });

    it('should accumulate refresh errors', async () => {
      mockFeedStore.refreshFeeds = vi.fn().mockRejectedValue(new Error('Feed error'));
      mockTagStore.refreshTags = vi.fn().mockRejectedValue(new Error('Tag error'));
    ...
    it('should clear old errors on successful refresh', async () => {
      ...
      refreshManager.refreshErrors = [
        { component: 'feeds', error: new Error('Old error'), timestamp: new Date() }
      ];

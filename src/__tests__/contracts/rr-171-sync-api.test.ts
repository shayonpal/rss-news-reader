 ...
 * 
 * This test defines the contract for the /api/sync endpoint that must
 * return metrics about the sync operation.
 * 
...
describe('RR-171: Sync API Contract', () => {
  ...
  describe('Successful Sync', () => {
    it('should return complete metrics for successful sync', async () => {
      ...
      expect(response.metrics.newArticles).toBe(10);
      expect(response.metrics.deletedArticles).toBe(5);
      expect(response.metrics.newTags).toBe(2);
      expect(response.metrics.failedFeeds).toBe(0);
    });
  ...
  describe('Failed Sync', () => {
    it('should return failed status on complete failure', async () => {
      mockSyncManager.syncFromInoreader.mockRejectedValue(new Error('Network error'));
      ...
      const responses: SyncResponse[] = [
        {
          syncId: 'sync-1',
          status: 'completed',
          metrics: { newArticles: 10, deletedArticles: 5, newTags: 2, failedFeeds: 0 }
        },
        {
          syncId: 'sync-2',
          status: 'partial',
          metrics: { newArticles: 5, deletedArticles: 2, newTags: 1, failedFeeds: 2 }
        },
        {
          syncId: 'sync-3',
          status: 'failed',
          metrics: { newArticles: 0, deletedArticles: 0, newTags: 0, failedFeeds: 10 }
        }
      ];
  ...
  describe('Error Handling', () => {
    ...
    it('should handle OAuth token errors', async () => {
      mockSyncManager.syncFromInoreader.mockRejectedValue(new Error('OAuth token expired'));
    ...
    it('should handle rate limit errors', async () => {
      mockSyncManager.syncFromInoreader.mockRejectedValue(new Error('Rate limit exceeded'));

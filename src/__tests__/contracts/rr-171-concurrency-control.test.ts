...
describe('RR-171: Concurrency Control Contract', () => {
    ...
    syncManager = {
      ...
      startSync: async function() {
        if (!this.canStartSync()) {
          throw new Error('Cannot start sync: already in progress or debounced');
        }
  ...
  describe('Sync Concurrency Control', () => {
    it('should prevent concurrent sync operations', async () => {
      ...
      await expect(syncManager.startSync()).rejects.toThrow('Cannot start sync');
    ...
    it('should handle timeout scenarios correctly', async () => {
      ...
      mockSyncAPI.mockImplementation(() => 
        new Promise(resolve => setTimeout(() => resolve({
          syncId: 'sync-slow',
          status: 'completed',
          metrics: { newArticles: 1, deletedArticles: 0, newTags: 0, failedFeeds: 0 }
        }), 10000))
      );

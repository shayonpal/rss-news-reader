...
describe("RR-171: Store Refresh Contract", () => {
  ...
  describe("Feed Store Refresh", () => {
    ...
    beforeEach(() => {
      feedStore = {
        ...
        updateUnreadCount: vi.fn(function (
          this: FeedStore,
          feedId: string,
          count: number
        ) {
    ...
    it("should update lastRefreshTime after successful refresh", async () => {
      ...
      expect(feedStore.lastRefreshTime!.getTime()).toBeGreaterThanOrEqual(
        beforeTime.getTime()
      );
      expect(feedStore.lastRefreshTime!.getTime()).toBeLessThanOrEqual(
        afterTime.getTime()
      );
    });
  ...
  describe("Cross-Store Coordination", () => {
    it("should refresh all stores independently", async () => {
      ...
      await Promise.all([feedRefresh(), articleRefresh(), tagRefresh()]);

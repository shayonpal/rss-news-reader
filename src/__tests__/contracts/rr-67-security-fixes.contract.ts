...
export const ViewBehaviorContracts = {
  ...
  sync_queue_stats: {
    ...
    after: {
      ...
      testQuery: 'SELECT * FROM sync_queue_stats',
      expectedForNonOwner: 'Returns only rows matching RLS policies for current user',
  ...
  author_quality_report: {
    before: {
      ...
      expectedForNonOwner: 'Returns all author statistics (security vulnerability)',
    ...
    after: {
      ...
      testQuery: 'SELECT * FROM author_quality_report',
      expectedForNonOwner: 'Returns only authors from user\'s accessible articles',
      expectedForOwner: 'Returns authors based on owner\'s article access',
      validation: {
        dataIsolation: 'User A should not see authors from User B\'s private feeds',
  ...
  author_statistics: {
    before: {
      ...
      expectedForNonOwner: 'Returns all author metrics (security vulnerability)',
  ...
  sync_author_health: {
    before: {
      ...
      expectedForNonOwner: 'Returns all sync health data (security vulnerability)',
...
export const FunctionBehaviorContracts = {
  ...
  get_unread_counts_by_feed: {
    before: {
      hasSearchPath: false,
      vulnerability: 'Vulnerable to schema hijacking - attacker could create malicious objects in user schema',
      testScenario: 'Create malicious "articles" table in user schema',
      expectedBehavior: 'Function might read from malicious table instead of public.articles'
    },
    after: {
      hasSearchPath: true,
      searchPath: 'public',
      protection: 'Always reads from public schema regardless of search_path manipulation',
      testValidation: {
        sql: `
  ...
  get_articles_optimized: {
    before: {
      hasSearchPath: false,
      vulnerability: 'Critical vulnerability - main article fetching could be hijacked',
      impact: 'Critical - Could return malicious content instead of real articles'
    },
    after: {
      hasSearchPath: true,
      searchPath: 'public',
      ...
      testValidation: {
        description: 'Verify function resists schema hijacking for article fetching',
...
export const SecurityValidationContracts = {
  ...
  functionsValidation: {
    ...
    validation: 'proconfig should contain "search_path=public" for each function'
  }
};
  ...
  migrationSteps: [
    {
      ...
      action: 'DROP VIEW IF EXISTS sync_queue_stats, author_quality_report, author_statistics, sync_author_health CASCADE',
      validation: {
  ...
  dataIntegrity: {
    ...
    validation: [
      ...
      },
      {
        ...
        description: 'Functions return same results, just with improved security',
    ...
    tests: [
      {
        ...
        expectedBehavior: 'Returns correct articles with search_path protection'
      },
      {
        name: 'Unread counts',

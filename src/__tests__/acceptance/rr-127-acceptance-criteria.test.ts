...
describe('RR-127 Acceptance Criteria: Test Infrastructure Overhaul', () => {
  ...
  beforeAll(async () => {
    ...
    allTestFiles = await glob('src/**/*.{test,spec}.{ts,tsx}', { cwd: projectRoot });
    ...
    testExecutionReport = {
      ...
      categorizedTests: {
        ...
        performance: await glob('src/**/__tests__/performance/**/*.test.ts', { cwd: projectRoot }),
        e2e: await glob('src/**/__tests__/e2e/**/*.{test,spec}.{ts,tsx}', { cwd: projectRoot })
      }
    };
  });
  ...
  describe('AC1: Test File Discovery - All 45+ test files discoverable', () => {
    ...
    it('should categorize test files correctly', () => {
      const totalCategorized = Object.values(testExecutionReport.categorizedTests)
        .flat()
        .length;
  ...
  describe('AC2: TypeScript/JSX Extension Issues Resolved', () => {
    it('should have component tests using .tsx extension', async () => {
      const componentTestFiles = await glob('src/components/**/*.test.{ts,tsx}', { cwd: projectRoot });
    ...
    it('should have utility tests using .ts extension appropriately', async () => {
      const utilityTestFiles = await glob('src/lib/**/*.test.ts', { cwd: projectRoot });
    ...
    it('should validate no extension mismatches exist', async () => {
      ...
      for (const testFile of allTestFiles.slice(0, 20)) { // Sample check
        const fullPath = resolve(projectRoot, testFile);
        if (existsSync(fullPath)) {
          const content = await readFile(fullPath, 'utf-8');
  ...
  describe('AC4: Mock Configuration Mismatches Fixed', () => {
    it('should have consistent vi.mock usage', async () => {
      ...
      for (const testFile of allTestFiles.slice(0, 15)) { // Sample check
        const fullPath = resolve(projectRoot, testFile);
        if (existsSync(fullPath)) {
          const content = await readFile(fullPath, 'utf-8');
    ...
    it('should have store mocks properly configured', async () => {
      const storeTestFiles = allTestFiles.filter(file => 
        file.includes('store') || file.includes('component')
      );

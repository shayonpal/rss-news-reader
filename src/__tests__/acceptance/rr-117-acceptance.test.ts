...
describe('RR-117: Auth Status Endpoint Acceptance Criteria', () => {
  ...
  beforeAll(async () => {
    ...
    await new Promise<void>((resolve) => {
      server.listen(port, () => {
        console.log(`> Acceptance test server ready on http://localhost:${port}`);
        resolve();
      });
    });
  }, 30000);
  ...
  describe('AC-6: Integration with Existing Health Check System', () => {
    ...
    it('MUST follow existing API response patterns', async () => {
      ...
      const authResponse = await fetch(`http://localhost:${port}/reader/api/auth/inoreader/status`);
      const healthResponse = await fetch(`http://localhost:${port}/reader/api/health/app`);
  ...
  describe('AC-8: Performance and Resource Usage', () => {
    ...
    it('MUST handle reasonable file sizes efficiently', async () => {
      ...
      const largeTokenData = {
        ...
        metadata: {
          history: Array(100).fill({ action: 'refresh', timestamp: new Date().toISOString() })
        }
      };

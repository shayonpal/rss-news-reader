    ...
    it('dropdown items should have clear action descriptions', () => {
      const { container } = render(
        <MorphingDropdown
          ...
          items={[
            { label: 'Generate AI Summary', onClick: () => {}, ariaLabel: 'Generate an AI-powered summary of this article' },
            { label: 'Share', onClick: () => {}, ariaLabel: 'Share this article via link' },
            { label: 'Archive', onClick: () => {}, ariaLabel: 'Archive this article for later' },
          ]}
          open={true}
        />
      );
      
      const aiSummary = screen.getByRole('menuitem', { name: /AI-powered summary/ });
      const share = screen.getByRole('menuitem', { name: /Share.*via link/ });
      const archive = screen.getByRole('menuitem', { name: /Archive.*for later/ });
    ...
    it('should have adequate spacing between targets', () => {
      const { container } = render(
        <div style={{ display: 'flex', gap: '8px' }}>
          <GlassIconButton icon="star" aria-label="Star" onClick={() => {}} data-testid="btn1" data-left={0} />
          <GlassIconButton icon="share" aria-label="Share" onClick={() => {}} data-testid="btn2" data-left={56} />
          <GlassIconButton icon="archive" aria-label="Archive" onClick={() => {}} data-testid="btn3" data-left={112} />
        </div>
      );
...
const MorphingDropdown = ({ trigger, items, open = false }: any) => {
  const reducedMotion = document.documentElement.getAttribute('data-prefers-reduced-motion') === 'reduce';

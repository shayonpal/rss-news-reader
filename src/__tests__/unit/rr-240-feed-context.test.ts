...
describe("RR-240: Feed Context Preservation", () => {
  ...
  describe("SessionStorage Structure with Feed Context", () => {
    ...
    it("should handle articles from different feeds with separate feedIds", async () => {
      ...
      act(() => {
        ...
        const articlesMap = new Map<string, Article>();
        articlesMap.set("article-1", {
          id: "article-1",
          feedId: feed1Id,
          isRead: false,
        } as Article);
        articlesMap.set("article-2", {
          id: "article-2",
          feedId: feed1Id,
          isRead: false,
        } as Article);
      ...
      act(() => {
        ...
        articlesMap2.set("article-3", {
          id: "article-3",
          feedId: feed2Id,
          isRead: false,
        } as Article);
        articlesMap2.set("article-4", {
          id: "article-4",
          feedId: feed2Id,
          isRead: false,
        } as Article);
      ...
      const feed1Articles = parsed.filter(
        (item: any) => item.feedId === feed1Id
      );
      const feed2Articles = parsed.filter(
        (item: any) => item.feedId === feed2Id
      );
    ...
    it("should preserve feedId from context even when article has different feedId", async () => {
      ...
      const preservedArticle = parsed.find(
        (item: any) => item.id === "article-1"
      );
  ...
  describe("getPreservedArticleIds Function Filtering", () => {
    ...
    const getPreservedArticleIds = (feedId?: string): string[] => {
      try {
        const preservedIds = window.sessionStorage.getItem(
    ...
    it("should filter preserved article IDs by feedId when provided", () => {
      ...
      expect(feed1Articles).toEqual(
        expect.arrayContaining(["article-1", "article-3"])
      );
  ...
  describe("Backward Compatibility", () => {
    ...
    it("should migrate legacy format to new format when marking articles as read", async () => {
      ...
      const newItem = parsed.find(
        (item: any) => typeof item === "object" && item.id === "new-article-1"
      );
      ...
      const migratedLegacyItems = parsed.filter(
        (item: any) =>
          typeof item === "object" &&
          (item.id === "old-article-1" || item.id === "old-article-2")
      );
  ...
  describe("Integration with Article Store", () => {
    it("should preserve feed context across navigation", async () => {
      ...
      expect(feed1PreservedIds).toEqual(
        expect.arrayContaining(["article-1", "article-2"])
      );
      ...
      expect(allPreservedIds).toEqual(
        expect.arrayContaining(["article-1", "article-2"])
      );
    });
    ...
    it("should maintain separate preserved articles for different mark types in same feed", async () => {
      ...
      act(() => {
        ...
        articlesMap.set("article-3", {
          id: "article-3",
          feedId,
          isRead: false,
        } as Article);
        articlesMap.set("article-4", {
          id: "article-4",
          feedId,
          isRead: false,
        } as Article);

...
describe('RR-154: HTML Entity Decoder Module', () => {
  ...
  describe('1. Core Decoding Functions', () => {
    describe('1.1 decodeHtmlEntities(text: string): string', () => {
      it('should decode apostrophe entities correctly', async () => {
        ...
        expect(decodeHtmlEntities('Biden&rsquo;s decision')).toBe("Biden\u2019s decision");
        expect(decodeHtmlEntities('It&rsquo;s working')).toBe("It\u2019s working");
        
        // Numeric character reference for apostrophe
        expect(decodeHtmlEntities('Biden&#8217;s decision')).toBe("Biden\u2019s decision");
        expect(decodeHtmlEntities('It&#8217;s working')).toBe("It\u2019s working");
        
        // Multiple apostrophes
        expect(decodeHtmlEntities('John&rsquo;s friend&rsquo;s car')).toBe("John\u2019s friend\u2019s car");
      });
      ...
      it('should decode quotation mark entities correctly', async () => {
        ...
        expect(decodeHtmlEntities('&lsquo;Hello World&rsquo;')).toBe("\u2018Hello World\u2019");
        
        // Standard double quote entity
        expect(decodeHtmlEntities('&quot;Hello World&quot;')).toBe('"Hello World"');
        
        // Mixed quotes
        expect(decodeHtmlEntities('He said &ldquo;It&rsquo;s done&rdquo;')).toBe('He said \u201cIt\u2019s done\u201d');
      });
      ...
      it('should decode dash entities correctly', async () => {
        ...
        // Em dash
        expect(decodeHtmlEntities('Breaking&mdash;this just in')).toBe('Breaking\u2014this just in');
        expect(decodeHtmlEntities('The result&mdash;unexpected')).toBe('The result\u2014unexpected');
      });
      ...
      it('should handle mixed entities in complex text', async () => {
        ...
        const input = 'AT&amp;T&rsquo;s &ldquo;Innovation&rdquo; &ndash; Q&amp;A';
        const expected = 'AT&T\u2019s \u201cInnovation\u201d \u2013 Q&A';
        expect(decodeHtmlEntities(input)).toBe(expected);
        
        const input2 = 'The company&#8217;s R&amp;D &mdash; it&rsquo;s amazing!';
    ...
    describe('1.2 decodeArticleData(article: object): object', () => {
      it('should decode title and content but preserve URLs', async () => {
        ...
        const article = {
          ...
          content: 'The president&rsquo;s &ldquo;new approach&rdquo; includes R&amp;D.',
        ...
        expect(decoded.content).toBe('The president\u2019s \u201cnew approach\u201d includes R&D.');
        
        // URLs should remain exactly as-is
        expect(decoded.url).toBe('https://example.com/article?param=value&amp;other=test');
        expect(decoded.canonical_url).toBe('https://example.com/article?id=123&amp;category=news');
        expect(decoded.inoreader_id).toBe('tag:google.com,2005:reader/item/000000001234');
      });
      ...
      it('should handle description field if present', async () => {
        ...
        const article = {
          id: 'article-4',
          title: 'Title',
          description: 'A &ldquo;brief&rdquo; description &ndash; it&rsquo;s useful',
        ...
        expect(decoded.description).toBe('A \u201cbrief\u201d description \u2013 it\u2019s useful');
      });
    });
    ...
    describe('1.3 isSafeUrl(text: string): boolean', () => {
      it('should identify URLs correctly', async () => {
        ...
        expect(isSafeUrl('https://example.com/path?param=value&other=test')).toBe(true);
        expect(isSafeUrl('tag:google.com,2005:reader/item/000000001234')).toBe(true);
  ...
  describe('3. Error Handling', () => {
    it('should handle malformed entities gracefully', async () => {
      ...
      expect(decodeHtmlEntities('Test &rsquo incomplete')).toBe('Test &rsquo incomplete');
  ...
  describe('4. Performance Requirements', () => {
    it('should decode 1000 articles in under 1 second', async () => {
      ...
      expect(decoded[0].title).toBe('Article 0 & \u2019 \u201ctest\u201d \u2013 complex');
      expect(decoded[999].title).toBe('Article 999 & \u2019 \u201ctest\u201d \u2013 complex');
    });
  });
});
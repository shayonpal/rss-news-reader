...
describe("RR-148: Retention Policy Configuration", () => {
  ...
  describe("Retention Policy Configuration Types", () => {
    it("should define comprehensive retention configuration interface", () => {
      ...
      const validateRetentionConfig = (
        ...
        const cronRegex =
          /^(\*|[0-5]?[0-9]) (\*|[01]?[0-9]|2[0-3]) (\*|[01]?[0-9]|[12][0-9]|3[01]) (\*|[01]?[0-9]|1[0-2]) (\*|[0-6])$/;
      ...
      expect(invalidValidation.errors).toContain(
        "read_articles_days must be non-negative"
      );
      expect(invalidValidation.errors).toContain(
        "full_content_cache_days must be non-negative"
      );
      expect(invalidValidation.errors).toContain("batch_size must be positive");
      expect(invalidValidation.errors).toContain(
        "cleanup_schedule must be valid cron expression"
      );
    });
    ...
    it("should provide sensible default configurations", () => {
      ...
      expect(defaults.unread_articles_days).toBeGreaterThan(
        defaults.read_articles_days
      );
      expect(defaults.starred_articles_days).toBe(-1); // Never delete starred
      expect(defaults.full_content_cache_days).toBeLessThan(
        defaults.read_articles_days
      );
  ...
  describe("Article Classification for Cleanup", () => {
    it("should identify articles eligible for cleanup based on age and status", () => {
      ...
      const classifyForCleanup = (
        article: ArticleForCleanup,
        config: any,
      ...
      const recentResult = classifyForCleanup(
        recentArticle,
        config,
        currentDate
      );
      ...
      const oldReadResult = classifyForCleanup(
        oldReadArticle,
        config,
        currentDate
      );
      ...
      const starredResult = classifyForCleanup(
        starredArticle,
        config,
        currentDate
      );
    ...
    it("should handle full content cache cleanup separately", () => {
      ...
      const shouldCleanupFullContent = (
        article: any,
        config: any,
        currentDate: Date = new Date()
      ) => {
        ...
        const extractedDate = article.extracted_at
          ? new Date(article.extracted_at)
          : new Date(article.updated_at);
        const ageInDays = Math.floor(
          (currentDate.getTime() - extractedDate.getTime()) /
            (1000 * 60 * 60 * 24)
        );
      ...
      const result = shouldCleanupFullContent(
        oldCachedArticle,
        config,
        currentDate
      );
      ...
      const freshResult = shouldCleanupFullContent(
        freshCachedArticle,
        config,
        currentDate
      );
      ...
      class RetentionCleanupService {
        ...
        async cleanupReadArticles(dryRun: boolean = false): Promise<{
          ...
          try {
            ...
            cutoffDate.setDate(
              cutoffDate.getDate() - this.config.read_articles_days
            );
            ...
            while (hasMore) {
              ...
              if (!dryRun) {
                ...
                if (deleteError) {
                  errors.push(
                    `Delete batch ${batches}: ${deleteError.message}`
                  );
                } else {
      ...
      class CleanupScheduler {
        ...
        getDueJobs(currentTime: Date = new Date()): CleanupJob[] {
          return this.jobs.filter((job) => {
            ...
            const nextRun = job.next_run
              ...
              : this.getNextRunTime(job.schedule);
        ...
        updateJobLastRun(jobId: string, runTime: Date): void {
          ...
          if (job) {
            ...
            job.next_run = this.getNextRunTime(
              job.schedule,
              runTime
            ).toISOString();
          }
  ...
  describe("Database Storage Management", () => {
    it("should monitor database size and trigger cleanup when needed", async () => {
      ...
      const criticalDecision = shouldTriggerEmergencyCleanup(
        criticalStats,
        1000
      );
    ...
    it("should prioritize cleanup order based on impact and safety", () => {
      ...
      const prioritizeCleanupStrategies = (
        ...
        return strategies
          .map((strategy) => ({
            ...
            efficiency:
              strategy.estimated_space_mb /
              (strategy.execution_time_estimate / 1000), // MB per second
          }))
      ...
      const calculateScore = (strategy: CleanupStrategy, weights: any) => {
        const safetyScore =
          strategy.safety === "safe"
            ? 3
            : strategy.safety === "moderate"
              ? 2
              : 1;
        const impactScore =
          strategy.impact === "high" ? 3 : strategy.impact === "medium" ? 2 : 1;
        const efficiencyScore =
          strategy.estimated_space_mb /
          (strategy.execution_time_estimate / 60000); // MB per minute
      ...
      const emergencyPriority = prioritizeCleanupStrategies(
        strategies,
        "emergency"
      );
  ...
  describe("Configuration Management", () => {
    it("should support dynamic configuration updates", async () => {
      ...
      class RetentionConfigManager {
        ...
        async updateConfig(
          ...
          try {
            // Validate updates
            if (
              updates.read_articles_days !== undefined &&
              updates.read_articles_days < 0
            ) {
      ...
      expect(invalidUpdate.errors).toContain(
        "read_articles_days cannot be negative"
      );
    ...
    it("should provide configuration presets for different use cases", () => {
      ...
      expect(presets.conservative.config.read_articles_days).toBeGreaterThan(
        presets.moderate.config.read_articles_days
      );
      expect(presets.moderate.config.read_articles_days).toBeGreaterThan(
        presets.aggressive.config.read_articles_days
      );

...
import { GlassButton, GlassIconButton, GlassToolbarButton } from '@/components/ui/glass-button';
...
describe('RR-180: Glass Button Components', () => {
  ...
  describe('GlassButton Component', () => {
    ...
    it('should prevent gray flash on touch', () => {
      const { container } = render(
        <GlassButton>No Flash</GlassButton>
      );
...
const ArticleDetail = ({ articleId }: { articleId: string }) => (
  <div>
    <MorphingDropdown
      toolbarElements={[
        <button key="star" aria-label="Star">★</button>,
        <button key="summary" aria-label="Summary">📄</button>,
      ]}
      items={[
...
describe('RR-180: Article Detail Integration', () => {
  it('should replace existing dropdown with morphing version', () => {
    const { container } = render(
      <ArticleDetail articleId="test-123" />
    );

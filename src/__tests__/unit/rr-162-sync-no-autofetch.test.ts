...
describe('RR-162: Sync Route Without Auto-Fetch', () => {
  ...
  describe('Contract 1: Sync Completes Without Auto-Fetch', () => {
        ...
        if (table === 'sync_metadata') {
          return {
            ...
            select: () => Promise.resolve({
              data: { last_sync_at: new Date(Date.now() - 3600000).toISOString() },
              error: null,
            }),
          };
        }
    ...
    it('should not execute performAutoFetch function', async () => {
      ...
      expect(ValidationUtilities.hasAutoFetchReferences(fileContent)).toBe(false);
  ...
  describe('Contract 4: Error Handling Without Auto-Fetch', () => {
    it('should handle sync errors gracefully', async () => {
      ...
      const lastWrite = mockFs.writeFile.mock.calls[mockFs.writeFile.mock.calls.length - 1];
      if (lastWrite) {

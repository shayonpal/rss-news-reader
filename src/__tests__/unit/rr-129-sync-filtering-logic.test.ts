...
describe('RR-129: Sync Filtering Logic', () => {
  ...
  const SyncFilterService = {
    async filterArticlesForSync(inoreaderArticles: any[], deletedArticleIds: Set<string>) {
      ...
      for (const article of inoreaderArticles) {
        ...
        const isReadInInoreader = article.categories?.includes('read') || article.categories?.includes('user/-/state/com.google/read');
    ...
    createInoreaderArticle(id: string, isRead: boolean = false, isStarred: boolean = false) {
      const categories = [];
      if (isRead) {
  ...
  describe('Article Filtering Logic', () => {
    it('should skip previously deleted articles that are still read in Inoreader', async () => {
      ...
      const result = await SyncFilterService.filterArticlesForSync(inoreaderArticles, deletedArticleIds);
    ...
    it('should allow previously deleted articles that are now unread in Inoreader', async () => {
      ...
      const deletedArticleIds = new Set(['article-1', 'article-2', 'article-3']);

      const result = await SyncFilterService.filterArticlesForSync(inoreaderArticles, deletedArticleIds);

      expect(result.articlesToSync).toHaveLength(2);
      expect(result.articlesToSync.map(a => a.id)).toEqual(['article-1', 'article-3']);
      expect(result.articlesToRemoveFromTracking).toEqual(['article-1', 'article-3']);
    ...
    it('should handle mixed scenarios correctly', async () => {
      ...
      expect(result.articlesToSync.map(a => a.id)).toEqual(['article-1', 'article-2', 'article-4', 'article-5']);
  ...
  describe('Category Detection Logic', () => {
    it('should correctly identify read articles by category', async () => {
      ...
      const result = await SyncFilterService.filterArticlesForSync([readArticle], deletedIds);
    ...
    it('should correctly identify unread articles (no read category)', async () => {
      ...
      const result = await SyncFilterService.filterArticlesForSync([unreadArticle], deletedIds);
    ...
    it('should handle articles with no categories', async () => {
      ...
      const result = await SyncFilterService.filterArticlesForSync([articleWithoutCategories], deletedIds);
    ...
    it('should handle articles with undefined categories', async () => {
      ...
      const result = await SyncFilterService.filterArticlesForSync([articleUndefinedCategories], deletedIds);

      // Should be treated as unread and allowed
      expect(result.articlesToSync).toHaveLength(1);
      expect(result.articlesToRemoveFromTracking).toEqual(['undefined-categories']);
    });
    ...
    it('should handle alternative read category formats', async () => {
      ...
      const result = await SyncFilterService.filterArticlesForSync([readArticleAlt], deletedIds);
  ...
  describe('Deleted Article Tracking Integration', () => {
    it('should fetch deleted article IDs from database', async () => {
      ...
      expect(mockSupabase.in).toHaveBeenCalledWith('inoreader_id', inoreaderIds);
    ...
    it('should remove articles from tracking when they become unread', async () => {
      ...
      mockSupabase.delete.mockResolvedValue({ data: null, error: null, count: 2 });
  ...
  describe('Performance and Edge Cases', () => {
    it('should handle large article sets efficiently', async () => {
      ...
      const result = await SyncFilterService.filterArticlesForSync(largeArticleSet, deletedIds);
    ...
    it('should handle empty article arrays', async () => {
      const result = await SyncFilterService.filterArticlesForSync([], new Set());
    ...
    it('should handle empty deleted tracking set', async () => {
      ...
      const result = await SyncFilterService.filterArticlesForSync(articles, new Set());
    ...
    it('should preserve article order in filtered results', async () => {
      ...
      const result = await SyncFilterService.filterArticlesForSync(articles, deletedIds);

      expect(result.articlesToSync.map(a => a.id)).toEqual(['article-1', 'article-3', 'article-4']);
    });
  });
  ...
  describe('State Reconciliation Logic', () => {
    it('should handle state changes from read to unread in Inoreader', async () => {
      ...
      const article = SyncFilterService.createInoreaderArticle('state-change', false); // Now unread
      const deletedIds = new Set(['state-change']); // Was previously deleted

      const result = await SyncFilterService.filterArticlesForSync([article], deletedIds);
    ...
    it('should respect continued read state in Inoreader', async () => {
      ...
      const article = SyncFilterService.createInoreaderArticle('still-read', true); // Still read
    ...
    it('should handle complex state reconciliation scenarios', async () => {
      ...
      expect(result.articlesToSync.map(a => a.id)).toEqual(['made-unread', 'never-deleted', 'also-unread']);
      expect(result.articlesToRemoveFromTracking).toEqual(['made-unread', 'also-unread']);
      expect(result.skippedCount).toBe(1);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle database errors when fetching deleted IDs', async () => {
      ...
      const result = await SyncFilterService.getDeletedArticleIds(['article-1']);
    ...
    it('should handle removal errors gracefully', async () => {
      ...
      const result = await SyncFilterService.removeFromDeletedTracking(['article-1']);
    ...
    it('should handle malformed article data', async () => {
      ...
      const result = await SyncFilterService.filterArticlesForSync(malformedArticles, new Set());

...
describe('RR-129: Cleanup Service Functions', () => {
  ...
  const CleanupService = {
    ...
    async cleanupDeletedFeeds(inoreaderFeeds: any[]) {
      ...
      if (localFeeds && feedsToDelete.length > localFeeds.length * 0.5) {
        throw new Error('Safety check failed: attempting to delete more than 50% of feeds');
      }
  ...
  describe('trackDeletedArticles', () => {
    ...
    it('should handle empty article arrays', async () => {
      ...
      expect(mockSupabase.upsert).toHaveBeenCalledWith([], { onConflict: 'inoreader_id' });
    });
  });
  ...
  describe('cleanupReadArticles', () => {
    ...
    it('should handle database errors gracefully', async () => {
      ...
      await expect(CleanupService.cleanupReadArticles()).rejects.toThrow('Database connection failed');
    });
  });
  ...
  describe('cleanupDeletedFeeds', () => {
    it('should delete feeds not present in Inoreader subscription list', async () => {
      ...
      expect(mockSupabase.delete).toHaveBeenCalled();
      expect(mockSupabase.in).toHaveBeenCalledWith('inoreader_id', ['feed-3', 'feed-4']);
    });
    ...
    it('should enforce safety check for mass deletion', async () => {
      ...
      await expect(CleanupService.cleanupDeletedFeeds(inoreaderFeeds))
        .rejects.toThrow('Safety check failed: attempting to delete more than 50% of feeds');
    ...
    it('should allow deletion when under safety threshold', async () => {
      ...
      mockSupabase.select.mockResolvedValue({ data: localFeeds, error: null });
      mockSupabase.delete.mockResolvedValue({ data: null, error: null, count: 1 });
  ...
  describe('getDeletedArticleIds', () => {
    it('should return set of previously deleted article IDs', async () => {
      ...
      mockSupabase.select.mockResolvedValue({ data: deletedArticles, error: null });
  ...
  describe('cleanupOldDeletedEntries', () => {
    it('should clean up entries older than 90 days', async () => {
      ...
      const expectedCutoff = new Date(mockDate.getTime() - (90 * 24 * 60 * 60 * 1000)).toISOString();

      vi.spyOn(Date, 'now').mockReturnValue(mockDate.getTime());

      mockSupabase.delete.mockResolvedValue({ data: null, error: null, count: 5 });

      await CleanupService.cleanupOldDeletedEntries();

      expect(mockSupabase.from).toHaveBeenCalledWith('deleted_articles');
      expect(mockSupabase.delete).toHaveBeenCalled();
      expect(mockSupabase.lt).toHaveBeenCalledWith('deleted_at', expectedCutoff);
  ...
  describe('Performance Considerations', () => {
    it('should handle large batches of articles efficiently', async () => {
      ...
      mockSupabase.select.mockResolvedValue({ data: largeArticleSet, error: null });
      mockSupabase.upsert.mockResolvedValue({ data: [], error: null });
      mockSupabase.delete.mockResolvedValue({ data: null, error: null, count: 1000 });

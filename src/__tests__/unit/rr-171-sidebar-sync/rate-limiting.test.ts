    ...
    rateLimitManager = {
      ...
      handleRateLimitError: (retryAfter?: number) => {
        const cooldownSeconds =
          retryAfter || mockSystemConfig.rate_limit_cooldown_minutes * 60;
  ...
  describe("Button State Management", () => {
    it("should disable button during cooldown", () => {
      ...
      const buttonState: SyncButtonState = {
        ...
        label: rateLimitManager.formatCountdown(
          rateLimitManager.getRemainingCooldown()
        ),
        onClick: vi.fn(),
      };
    ...
    it("should update button label every second during countdown", () => {
      ...
      for (let i = 0; i <= 5; i++) {
        labels.push(
          rateLimitManager.formatCountdown(
            rateLimitManager.getRemainingCooldown()
          )
        );

...
describe('Preferences API (RR-274)', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(encrypt).mockImplementation((data) => `encrypted_${JSON.stringify(data)}`);
    vi.mocked(decrypt).mockImplementation((data) => JSON.parse(data.replace('encrypted_', '')));
  });
  ...
  describe('GET /api/users/[id]/preferences', () => {
    it('should return default preferences when none exist', async () => {
      ...
      const request = new NextRequest(`http://localhost/api/users/${mockUserId}/preferences`);
  ...
  describe('PUT /api/users/[id]/preferences', () => {
    describe('Input Validation', () => {
      ...
      it('should reject retention below 100', async () => {
        ...
        expect(response.status).toBe(400);
        expect(data.error).toContain('retentionCount must be between 100 and 5000');
      });
    ...
    describe('Persistence', () => {
      it('should encrypt preferences with AES-256-GCM', async () => {
        ...
        const preferences = { sync: { maxArticles: 250, retentionCount: 1000 } };
        const request = new NextRequest(`http://localhost/api/users/${mockUserId}/preferences`, {
          method: 'PUT',

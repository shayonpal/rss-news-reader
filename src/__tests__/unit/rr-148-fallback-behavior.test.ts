...
describe('RR-148: Fallback Behavior for Content Parsing', () => {
  ...
  describe('Content Parsing Failure Scenarios', () => {
    it('should handle network timeout failures gracefully', async () => {
      ...
      class ContentParsingService {
        ...
        async parseContentWithFallback(articleId: string, options: {
          timeout?: number;
          retries?: number;
          ...
          const { timeout = 5000, retries = 2, fallbackToOriginal = true } = options;
          ...
          try {
            ...
            for (let attempt = 1; attempt <= retries; attempt++) {
              try {
                const extractedContent = await this.extractContentWithTimeout(article.url, timeout);
                ...
                this.logger.log(`Content extraction successful for ${articleId} on attempt ${attempt}`);
                ...
                this.logger.warn(`Content extraction attempt ${attempt} failed for ${articleId}: ${lastError.message}`);
            ...
            if (fallbackToOriginal && article.content) {
              this.logger.error(`Content extraction failed for ${articleId}, using fallback: ${lastError?.message}`);
            ...
            this.logger.error(`Failed to parse content for ${articleId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        ...
        private async extractContentWithTimeout(url: string, timeout: number): Promise<string> {
    ...
    it('should handle malformed HTML gracefully', async () => {
      ...
      const handleMalformedHtml = async (htmlContent: string, originalContent: string) => {
        try {
          // Simulate Mozilla Readability parsing attempt
          if (htmlContent.includes('<unclosed-tag>') || 
      ...
      const malformedHtml = '<div><p>Content with <unclosed-tag> problems</div>';
    ...
    it('should handle CORS and access denied errors', async () => {
      ...
      const handleAccessErrors = async (url: string, fallbackContent: string) => {
        try {
          ...
          let fallbackStrategy: 'original_content' | 'minimal_info' | 'error_message';
          
          if (errorMessage.includes('CORS') || errorMessage.includes('forbidden')) {
          ...
          switch (fallbackStrategy) {
            case 'original_content':
              ...
              break;
            case 'minimal_info':
              content = 'Article content is not available. The original page may have been removed or moved.';
              break;
            default:
      ...
      // Test 403 Forbidden
      const forbiddenResult = await handleAccessErrors('https://example.com/forbidden', 'Original article content');
      ...
      // Test 404 Not Found
      const notFoundResult = await handleAccessErrors('https://example.com/missing', 'Original content');
      ...
      // Test CORS error
      const corsResult = await handleAccessErrors('https://blocked-site.com/article', 'Original content');
  ...
  describe('User Experience During Failures', () => {
    it('should provide loading states and error messages to users', () => {
      ...
      class ContentParsingUI {
        ...
        async parseContent(articleId: string, originalContent: string): Promise<void> {
          this.setState({
            status: 'loading',
        ...
        getUserMessage(): string {
          switch (this.state.status) {
            ...
            case 'success':
              return this.state.method === 'cached' ? 'Content loaded from cache' : 'Full content loaded';
            case 'fallback':
    ...
    it('should implement progressive loading with partial content', async () => {
      ...
      class ProgressiveContentLoader {
        async loadContent(articleId: string): Promise<{
          ...
          try {
            const enhancedContent = await this.enhanceContent(articleId, originalContent);
            return {
              immediate,
              enhanced: {
        ...
        private async enhanceContent(articleId: string, originalContent: string): Promise<string> {
        ...
        async loadWithTimeout(articleId: string, timeoutMs: number = 3000): Promise<{
          content: string;
  ...
  describe('Error Recovery and Retry Logic', () => {
    it('should implement intelligent retry strategies', async () => {
      ...
      const nonRetryableResult = await retryService.executeWithRetry(nonRetryableOperation);
      expect(nonRetryableResult.success).toBe(false);
      expect(nonRetryableResult.attempts).toBe(1); // Should not retry
      expect(nonRetryableResult.lastError?.message).toBe('Authentication failed');

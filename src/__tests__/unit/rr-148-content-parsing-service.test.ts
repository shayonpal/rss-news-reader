...
describe('RR-148: Content Parsing Service', () => {
  ...
  describe('Content Parsing Service Core', () => {
    it('should define ContentParsingService class with required methods', () => {
      ...
      expect(() => service.parseContent('test')).rejects.toThrow('Not implemented');
      expect(() => service.isContentCached('test')).rejects.toThrow('Not implemented');
      expect(() => service.saveToCache('test', 'content')).rejects.toThrow('Not implemented');
      expect(() => service.extractFromUrl('http://example.com')).rejects.toThrow('Not implemented');
    });
    ...
    it('should extract content when not cached', async () => {
      ...
      const mockParseContentWithExtraction = async (articleId: string, url: string) => {
        // Check cache first
        const { data: article } = await mockSupabase
          .from('articles')
          .select('*')
      ...
      const result = await mockParseContentWithExtraction('article-456', 'https://example.com/article');
      
      expect(result.success).toBe(true);
      expect(result.cached).toBe(false);
      expect(result.content).toBe('Extracted full content from https://example.com/article');
  ...
  describe('Performance Requirements', () => {
    it('should complete parsing within 2-3 seconds', async () => {
      ...
      const parseWithTimeout = async (articleId: string, timeoutMs: number = 3000) => {
        const startTime = Date.now();
    ...
    it('should implement rate limiting for concurrent requests', async () => {
      ...
      class RateLimitedParser {
        ...
        async parseContent(articleId: string): Promise<any> {
          ...
          if (this.activeRequests.size >= this.maxConcurrent) {
            throw new Error(`Rate limit exceeded: ${this.maxConcurrent} concurrent requests maximum`);
          }
      ...
      await expect(parser.parseContent('article-6'))
    ...
    it('should handle memory constraints for large content', async () => {
      ...
      const processLargeContent = async (content: string, maxSize: number = 1024 * 1024) => {
      ...
      expect(largeResult.content).toContain('[Content truncated for performance]');
      expect(largeResult.originalSize).toBeGreaterThan(largeResult.processedSize);
    });
  });
  ...
  describe('Mozilla Readability Integration', () => {
    it('should integrate with Mozilla Readability for content extraction', async () => {
      ...
      const mockReadabilityExtract = async (html: string, url: string) => {
        ...
        const mockReadability = {
          parse: () => ({
            ...
            content: '<div><h1>Article Title</h1><p>Full article content extracted by Readability.</p></div>',
      ...
      const extracted = await mockReadabilityExtract(htmlContent, 'https://example.com/article');
    ...
    it('should handle Readability extraction failures gracefully', async () => {
      ...
      const extractWithFallback = async (html: string, originalContent: string) => {
        try {
      ...
      const goodResult = await extractWithFallback(goodHtml, 'fallback content');
      expect(goodResult.success).toBe(true);
      expect(goodResult.method).toBe('readability');

      // Test fallback scenario
      const badHtml = '<invalid-markup>';
      const fallbackResult = await extractWithFallback(badHtml, 'Original article content');
  ...
  describe('Caching and Storage', () => {
    it('should implement efficient caching strategy', async () => {
      ...
      class ContentCache {
        private memoryCache = new Map<string, { content: string; timestamp: number; hits: number }>();
    ...
    it('should persist parsed content in database', async () => {
      ...
      const persistParsedContent = async (articleId: string, parsedData: any) => {
        const updateData = {
  ...
  describe('Error Handling and Logging', () => {
    it('should log parsing attempts and outcomes', () => {
      ...
      class ParseLogger {
        ...
        logSuccess(articleId: string, duration: number, method: string, contentLength: number): void {
          this.logs.push({
            articleId,
            status: 'success',
            timestamp: new Date().toISOString(),
        ...
        getStats(): { attempts: number; successes: number; failures: number; cacheHits: number } {
          const stats = { attempts: 0, successes: 0, failures: 0, cacheHits: 0 };
          for (const log of this.logs) {
            switch (log.status) {
              case 'attempt': stats.attempts++; break;
              case 'success': stats.successes++; break;
              case 'failure': stats.failures++; break;
              case 'cached': stats.cacheHits++; break;
            }
          }
          return stats;
        }
    ...
    it('should handle network timeouts and retries', async () => {
        ...
        while (attempts <= maxRetries) {
          try {
            ...
            const response = await new Promise<string>((resolve, reject) => {
              const shouldFail = url.includes('timeout') && attempts <= 2;
      ...
      const failResult = await parseWithRetry('https://example.com/timeout-always');

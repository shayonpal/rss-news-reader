...
describe('RR-123: Test Runner Memory Fix', () => {
  ...
  describe('Package.json Configuration Tests', () => {
    ...
    it('should have emergency cleanup script', async () => {
      ...
      const killScriptExists = await fs.access(path.join(projectRoot, 'scripts/kill-test-processes.sh')).then(() => true).catch(() => false);
  ...
  describe('Memory and Resource Monitoring Tests', () => {
    ...
    it('should detect orphaned vitest processes', async () => {
      ...
      try {
        const { stdout } = await execAsync('ps aux | grep -E "(vitest|sleep)" | grep -v grep');
        ...
        try {
          await execAsync('kill $(cat /tmp/test-vitest.pid 2>/dev/null) 2>/dev/null || true');
    ...
    it('should measure memory usage during test execution', async () => {
      ...
      try {
        await execAsync('timeout 10s npx vitest run --no-coverage src/__tests__/unit/health-endpoints/app.test.ts', {
          timeout: 15000
        });
      } catch (error) {
        // Timeout is expected
  ...
  describe('Process Cleanup and Recovery Tests', () => {
    it('should kill all vitest processes with cleanup script', async () => {
      ...
      const { stdout } = await execAsync('ps aux | grep "sleep 30" | grep -v grep || echo "no processes"');
  ...
  describe('CI/CD Integration Tests', () => {
    it('should have GitHub Actions workflow with resource limits', async () => {
      try {
        const workflowFiles = await fs.readdir(path.join(projectRoot, '.github/workflows'));
        const testWorkflow = workflowFiles.find(file => file.includes('test') || file.includes('ci'));
        ...
        if (testWorkflow) {
          ...
          expect(workflow).toMatch(/npm run test:safe|scripts\/safe-test-runner\.sh/);
...
async function getSystemMemoryUsage(): Promise<{ total: number; used: number; free: number }> {
  try {
    const { stdout } = await execAsync('vm_stat | grep -E "(Pages free|Pages active|Pages inactive|Pages wired down)"');

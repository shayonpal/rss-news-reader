...
describe('RR-123: Memory Exhaustion Edge Cases', () => {
  ...
  afterEach(async () => {
    ...
    try {
      await execAsync('pkill -f "test.*vitest|vitest.*test|sleep.*test" || true');
      await execAsync('rm -f /tmp/test-*.pid /tmp/test-*.lock /tmp/vitest-*.log');
    } catch (error) {
      // Ignore cleanup errors
    }
  });
  ...
  describe('Concurrent Test Execution Prevention', () => {
    it('should prevent multiple npm test executions', async () => {
      ...
      const firstTest = spawn('bash', ['-c', 'sleep 5 && echo "test1 complete"'], {
        stdio: 'pipe'
      });
      ...
      const secondTest = spawn('bash', ['-c', 'sleep 5 && echo "test2 complete"'], {
        stdio: 'pipe'
      });
      ...
      // Check process count
      const { stdout } = await execAsync('ps aux | grep "sleep.*echo" | grep -v grep | wc -l');
    ...
    it('should handle corrupted lock files', async () => {
      ...
      try {
        ...
        expect(result.stdout + result.stderr).toMatch(/handled gracefully|cleaned|verified|Lock verification/i);
    ...
    it('should handle stale lock files from crashed processes', async () => {
      ...
      try {
        ...
        const { stdout } = await execAsync(`ps -p ${stalePid} || echo "no such process"`);
        ...
        expect(result.stdout + result.stderr).toMatch(/cleaned|handled|removed|Stale lock/i);
  ...
  describe('Resource Exhaustion Scenarios', () => {
    it('should handle low memory conditions gracefully', async () => {
      ...
      if (availableMemory < 1024 * 1024 * 1024) { // Less than 1GB free
        ...
        try {
          ...
          expect(result.stderr).not.toMatch(/out of memory|memory exhausted|heap.*limit/i);
          
        } catch (error: any) {
          // If it fails due to memory, the error should be handled gracefully
          if (error.message.includes('memory') || error.message.includes('heap')) {
            expect(error.message).toMatch(/gracefully.*terminated|safely.*stopped/i);
          }
        }
      } else {
    ...
    it('should handle CPU overload scenarios', async () => {
      ...
      const cpuTasks = Array.from({ length: 2 }, () => 
        spawn('bash', ['-c', 'for i in {1..100000}; do echo $i > /dev/null; done'], {
          stdio: 'ignore'
        })
      );
      ...
      try {
        ...
        expect(result.stdout + result.stderr).not.toMatch(/process.*stuck|unable.*to.*respond/i);
      ...
      if (availableSpace.includes('G') && parseInt(availableSpace) < 2) {
        try {
          ...
          if (error.message.includes('space') || error.message.includes('disk')) {
            expect(error.message).toMatch(/safely.*handled|gracefully.*terminated/i);
          }
        }
      } else {
        console.log('Sufficient disk space available, skipping disk exhaustion test');
      }
    });
  });
  ...
  describe('Process Management Edge Cases', () => {
    it('should handle zombie processes', async () => {
      ...
      const { stdout } = await execAsync('ps aux | grep -E "Z|<defunct>" | grep -v grep || echo "no zombies"');
      
      // Emergency cleanup should handle zombies
      try {
        await execAsync('./scripts/emergency-test-cleanup.sh --cleanup-zombies || pkill -9 -f sleep', {
          cwd: projectRoot,
          timeout: 5000
        });
      } catch (error: any) {
        if (error.code !== 'ENOENT') {
          console.log('Emergency cleanup script not found, using manual cleanup');
        }
      }
      
      // Verify cleanup
      const { stdout: afterCleanup } = await execAsync('ps aux | grep -E "Z|<defunct>" | grep -v grep || echo "no zombies"');
    ...
    it('should handle process group termination', async () => {
      ...
      const { stdout } = await execAsync(`ps -g ${pgid} | grep -v PID || echo "group terminated"`);
    ...
    it('should handle unresponsive processes requiring SIGKILL', async () => {
      ...
      try {
        const { stdout } = await execAsync(`ps -p ${pid} | grep -v PID || echo "not running"`);
      ...
      if (stillRunning) {
        ...
        const { stdout } = await execAsync(`ps -p ${pid} | grep -v PID || echo "force killed"`);
  ...
  describe('Configuration Error Scenarios', () => {
    it('should handle invalid vitest configuration', async () => {
      ...
      try {
        ...
        expect(result.stdout + result.stderr).toMatch(/config.*error.*handled|cannot.*find.*config/i);
    ...
    it('should handle missing test files gracefully', async () => {
      try {
        ...
        expect(result.stdout + result.stderr).toMatch(/no.*tests.*found|missing.*test.*handled/i);
    ...
    it('should handle network connectivity issues during tests', async () => {
      ...
      try {
        ...
        expect(result.stdout + result.stderr).not.toMatch(/memory.*exhausted|out.*of.*memory/i);
    ...
    it('should handle file descriptor exhaustion', async () => {
      ...
      if (fdLimit < 10000) {
        ...
        try {
          ...
          expect(result.stdout + result.stderr).not.toMatch(/too many open files.*crash/i);
          
        } catch (error: any) {
          if (error.message.includes('files') || error.message.includes('descriptor')) {
    ...
    it('should handle maximum process limit', async () => {
      ...
      if (maxProcs < 2000) {
        ...
        try {
          ...
          expect(result.stdout + result.stderr).not.toMatch(/fork.*failed.*catastrophic/i);
          
        } catch (error: any) {
          if (error.message.includes('fork') || error.message.includes('process')) {
...
async function getSystemMemoryInfo(): Promise<{ total: number; used: number; free: number }> {
  try {

...
async function captureSessionStateSnapshot(
  page: Page,
  label: string
  ...
  if (snapshot.sessionData && !snapshot.sessionData.error) {
    console.log(
      `   Auto-read: ${snapshot.sessionData.autoReadArticles?.length || 0}`
    );
    console.log(
      `   Manual-read: ${snapshot.sessionData.manualReadArticles?.length || 0}`
    );
...
async function clickArticleWithJS(
  page: Page,
  articleId: string
): Promise<boolean> {
...
async function waitForNavigation(
  ...
  if (expectedPath) {
    ...
    if (!currentUrl.includes(expectedPath)) {
      console.log(
        `⚠️ Expected navigation to ${expectedPath}, but URL is ${currentUrl}`
      );
    } else {
...
test.describe("Session State Bug Reproduction", () => {
  test("should reproduce session state corruption after second article read", async ({
    page,
  }) => {
    ...
    const snapshot1 = await captureSessionStateSnapshot(
      page,
    ...
    const snapshot2 = await captureSessionStateSnapshot(
      page,
      "After Unread Filter"
    );
    ...
    if (snapshot2.unreadArticlesInDOM < 2) {
      console.log(
        `❌ Not enough unread articles (${snapshot2.unreadArticlesInDOM}), need at least 2`
      );
      return;
    }
    ...
    const firstArticleId = await page.evaluate(() => {
      ...
      return unreadArticles.length > 0
        ? unreadArticles[0].getAttribute("data-article-id")
        : null;
    });
    ...
    const snapshot3 = await captureSessionStateSnapshot(
      page,
      "On First Article Detail Page"
    );
    ...
    const snapshot4 = await captureSessionStateSnapshot(
      page,
    ...
    console.log(
      `   Actual: ${snapshot4.visibleReadArticles} read articles visible`
    );
    ...
    await waitForNavigation(page, "/article/");
    const snapshot5 = await captureSessionStateSnapshot(
      page,
      "On Second Article Detail Page"
    );
    ...
    const snapshot6 = await captureSessionStateSnapshot(
      page,
      "Back to List After Second Article (CRITICAL)"
    );

    // Step 7: Analyze the bug
    console.log("\n🐛 BUG ANALYSIS:");
    console.log(
      `   Expected visible read articles: 2 (both session preserved)`
    );
    console.log(
      `   Actual visible read articles: ${snapshot6.visibleReadArticles}`
    );

    // Detailed session state comparison
    console.log("\n📊 SESSION STATE EVOLUTION:");
    console.log(
      `   After 1st article: ${snapshot4.sessionPreservedCount} preserved, ${snapshot4.visibleReadArticles} visible`
    );
    console.log(
      `   After 2nd article: ${snapshot6.sessionPreservedCount} preserved, ${snapshot6.visibleReadArticles} visible`
    );

    // Get detailed article information for debugging
    const detailedArticleInfo = await page.evaluate(
      (firstId, secondId) => {
        ...
        allArticles.forEach((article) => {
          ...
          const isVisible =
            window.getComputedStyle(article).display !== "none" &&
            window.getComputedStyle(article).visibility !== "hidden";
          const hasSessionClass =
            article.className.includes("session-preserved");
    ...
    console.log(
      `   Total read articles in DOM: ${detailedArticleInfo.totalReadArticles}`
    );
    console.log(
      `   Visible read articles: ${detailedArticleInfo.visibleReadArticles}`
    );
    ...
    detailedArticleInfo.visibleReadArticleDetails.forEach((article, idx) => {
      ...
      const sessionStatus = article.hasSessionClass
        ? "✅ SESSION"
        : "❌ NOT SESSION";
    ...
    const targetArticlesVisible =
      detailedArticleInfo.visibleReadArticleDetails.filter(
        (a) => a.isTargetArticle
      ).length;
    const nonTargetArticlesVisible =
      detailedArticleInfo.visibleReadArticles - targetArticlesVisible;

    console.log("\n🎯 FINAL BUG ASSESSMENT:");
    console.log(
      `   Expected visible: ${expectedVisible} (only our 2 target articles)`
    );
    ...
    if (actualVisible > expectedVisible) {
      console.log(`\n🐛 BUG CONFIRMED!`);
      console.log(
        `   ${nonTargetArticlesVisible} extra read articles are showing that shouldn't be visible`
      );
      console.log(
        `   This indicates session state corruption - the "Unread Only" filter is showing`
      );
      console.log(
        `   ALL read articles from the feed instead of only session-preserved ones`
      );
    } else if (
      actualVisible === expectedVisible &&
      targetArticlesVisible === expectedVisible
    ) {
      console.log(`\n✅ BEHAVIOR CORRECT`);
      ...
      console.log(
        `   The visible count matches expected, but target articles may be missing`
      );
    }

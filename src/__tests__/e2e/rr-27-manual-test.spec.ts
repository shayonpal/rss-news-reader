...
test.describe('RR-27: Manual User Workflow Test', () => {
  test('should reproduce the original bug and verify the fix', async ({ page }) => {
    ...
    const unreadButton = page.locator('button').filter({ hasText: 'Unread Only' }).or(page.locator('button').filter({ hasText: 'Unread' }));
    ...
    await page.screenshot({ path: 'manual-2-unread-filter.png', fullPage: true });
    ...
    const firstArticleTitle = await firstArticle.locator('h2, h3, .article-title').first().textContent();
    
    console.log(`🎯 Selected article: ${firstArticleId} - "${firstArticleTitle?.substring(0, 50)}..."`);
    ...
    if (!visibleAfterClick) {
      console.log('❌ BUG REPRODUCED: Article disappeared immediately after click!');
      await page.screenshot({ path: 'manual-3-article-disappeared.png', fullPage: true });
      
      // If we can't see the article, count total articles to see if list changed
      const countAfterClick = await articles.count();
      console.log(`Article count after click: ${countAfterClick} (was ${unreadCount})`);
      ...
      await page.screenshot({ path: 'manual-3-article-still-visible.png', fullPage: true });
    }
    ...
    if (urlAfterClick !== urlBeforeClick) {
      ...
      const originalArticleAfterBack = page.locator(`[data-article-id="${firstArticleId}"]`);
      const isPreservedAndVisible = await originalArticleAfterBack.isVisible();
      
      console.log(`Original article preserved and visible: ${isPreservedAndVisible}`);
      
      if (isPreservedAndVisible) {
        console.log('✅ RR-27 SUCCESS: Article is preserved in Unread Only mode after back navigation');
        
        // Check if it has the session-preserved styling
        const hasSessionPreservedClass = await originalArticleAfterBack.evaluate(el => 
          el.classList.contains('session-preserved-read')
        );
        console.log(`Has session-preserved styling: ${hasSessionPreservedClass}`);
        
      } else {
        console.log('❌ RR-27 FAILURE: Article was not preserved after back navigation');
      }
      
      await page.screenshot({ path: 'manual-4-after-back-navigation.png', fullPage: true });
      
      // Final assertion
      expect(isPreservedAndVisible).toBe(true);
      
    } else {
      console.log('❌ Navigation did not occur - need to investigate click handler');
      await page.screenshot({ path: 'manual-error-no-navigation.png', fullPage: true });

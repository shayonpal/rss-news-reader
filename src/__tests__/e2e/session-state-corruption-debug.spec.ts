...
async function captureDebugSnapshot(page: Page, label: string): Promise<DebugSnapshot> {
  ...
  console.log(`Session Preserved Count: ${debugInfo.stateManagerDebug.sessionPreservedCount}`);
  ...
  if (debugInfo.sessionState) {
    ...
    console.log(`- Read States: ${Object.keys(debugInfo.sessionState.readStates).length}`);
    console.log(`- Timestamp: ${new Date(debugInfo.sessionState.timestamp).toISOString()}`);
  }
  
  // Log visible articles with read status
  console.log(`\nVisible Articles (first 5):`);
  debugInfo.visibleArticles.slice(0, 5).forEach((article, idx) => {
    console.log(`${idx + 1}. ${article.id} - Read: ${article.isRead}, Preserved: ${article.isSessionPreserved}, Visible: ${article.visible}`);
  });
...
async function selectSpecificFeed(page: Page): Promise<string | null> {
  ...
  if (feedCount === 0) {
    console.log('🔄 No specific feeds found, continuing with all articles view');
    return 'All Articles';
  }
...
test.describe('Session State Corruption Debug', () => {
  test('should reproduce and debug session state corruption after second article read', async ({ page }) => {
    ...
    console.log('\n📍 Step 4: Read the first article');
    const firstArticleId = await articles.first().getAttribute('data-article-id');
    ...
    const snapshot2 = await captureDebugSnapshot(page, 'After First Article Read (on detail page)');
    ...
    const snapshot3 = await captureDebugSnapshot(page, 'Back to List After First Article');
    
    // Verify only 1 article is preserved
    console.log('\n🔍 Verification after first article:');
    const preservedAfterFirst = snapshot3.visibleArticles.filter(a => a.isRead && a.visible).length;
    console.log(`📊 Read articles still visible: ${preservedAfterFirst} (should be 1)`);
    
    // Step 6: Read the second article
    console.log('\n📍 Step 6: Read the second article');
    
    // Find the first unread article (since the first one should now be read and preserved)
    const unreadArticles = page.locator('[data-article-id][data-is-read="false"]');
    ...
    const secondArticleId = await unreadArticles.first().getAttribute('data-article-id');
    ...
    const snapshot4 = await captureDebugSnapshot(page, 'After Second Article Read (on detail page)');
    ...
    const snapshot5 = await captureDebugSnapshot(page, 'Back to List After Second Article');
    
    // Step 8: Analyze the bug
    console.log('\n🐛 BUG ANALYSIS:');
    const preservedAfterSecond = snapshot5.visibleArticles.filter(a => a.isRead && a.visible).length;
    console.log(`📊 Read articles still visible: ${preservedAfterSecond} (should be 2)`);
    
    // Compare session states
    console.log('\n📋 SESSION STATE COMPARISON:');
    console.log(`Before first article: ${snapshot1.stateManagerDebug.sessionPreservedCount} preserved`);
    console.log(`After first article: ${snapshot3.stateManagerDebug.sessionPreservedCount} preserved`);
    console.log(`After second article: ${snapshot5.stateManagerDebug.sessionPreservedCount} preserved`);
    ...
    if (snapshot5.sessionState) {
      ...
      console.log(`Manual-read articles: ${state.manualReadArticles?.length || 0}`);
      console.log(`Total read states: ${Object.keys(state.readStates).length}`);
      console.log(`Read articles (true): ${Object.values(state.readStates).filter(r => r).length}`);
      
      // Check if our target articles are properly tracked
      const firstInAuto = state.autoReadArticles?.includes(firstArticleId || '') || false;
      const firstInManual = state.manualReadArticles?.includes(firstArticleId || '') || false;
      const secondInAuto = state.autoReadArticles?.includes(secondArticleId || '') || false;
      const secondInManual = state.manualReadArticles?.includes(secondArticleId || '') || false;
    ...
    if (preservedAfterSecond > 2) {
      console.log(`❌ BUG CONFIRMED: ${preservedAfterSecond} articles preserved instead of 2`);
      console.log('🔍 This indicates session state corruption after second article read');
      
      // Don't fail the test - this is a debug test to capture the issue
      // expect(preservedAfterSecond).toBeLessThanOrEqual(2);
    } else {
      console.log(`✅ Behavior correct: ${preservedAfterSecond} articles preserved`);
    }
    
    console.log('\n🏁 Session state corruption debug test completed');
    console.log('📄 Check the generated screenshot and console logs for detailed analysis');
  });
  
  test('should verify session storage state transitions in detail', async ({ page }) => {
    console.log('🔬 Starting detailed session storage state transition test...');
    ...
    const logAfterFirst = await page.evaluate(() => (window as any).sessionStorageLog || []);
    ...
    const secondArticles = page.locator('[data-article-id][data-is-read="false"]');
    if (await secondArticles.count() > 0) {
      const secondArticleId = await secondArticles.first().getAttribute('data-article-id');
      ...
      const completeLog = await page.evaluate(() => (window as any).sessionStorageLog || []);
      console.log('Complete session storage transaction log:');
      completeLog.forEach((entry: any, idx: number) => {
        console.log(`${idx + 1}. ${entry.action} at ${new Date(entry.timestamp).toISOString()}`);
        if (entry.fullValue) {
          try {
            const parsed = JSON.parse(entry.fullValue);
            console.log(`   - Auto-read: ${parsed.autoReadArticles?.length || 0}`);
            console.log(`   - Manual-read: ${parsed.manualReadArticles?.length || 0}`);

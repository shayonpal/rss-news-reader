...
test.describe("RR-129: Full Cleanup Cycle E2E", () => {
  ...
  test.describe("Article Reading and Cleanup Flow", () => {
    test("should complete full article lifecycle: read → cleanup → sync prevention", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="article-detail"]')
      ).toBeVisible();
      ...
      await expect(remainingArticles.first()).toContainText(
        "E2E Test Article 2"
      );
    ...
    test("should restore article when marked unread in Inoreader after deletion", async ({
      page,
    }) => {
      ...
      await page.route("**/api/sync", async (route) => {
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            ...
            restoration_stats: {
              articles_restored:
                articleState === "unread" && syncCount > 2 ? 1 : 0,
              tracking_removed:
                articleState === "unread" && syncCount > 2 ? 1 : 0,
            },
          }),
      ...
      await expect(
        restoredArticle.locator('[data-testid="read-indicator"]')
      ).not.toBeVisible();
    });
  });

  test.describe("Starred Article Protection", () => {
    test("should never delete starred articles during cleanup", async ({
      page,
    }) => {
      ...
      await expect(
        starredArticle.locator('[data-testid="star-indicator"]')
      ).toBeVisible();
      await expect(
        starredArticle.locator('[data-testid="read-indicator"]')
      ).toBeVisible();
    });
  });

  test.describe("Feed Deletion Scenarios", () => {
    test("should remove feeds and articles when feeds deleted in Inoreader", async ({
      page,
    }) => {
  ...
  test.describe("Performance and User Experience", () => {
    test("should complete cleanup operations within acceptable time limits", async ({
      page,
    }) => {
      ...
      if (await syncButton.isVisible()) {
        ...
        await expect(
          page.locator('[data-testid="sync-progress"]')
        ).toBeVisible();
      ...
      console.log(
        `Cleanup completed in ${cleanupTime}ms, total page interaction ${totalPageTime}ms`
      );
    });

    test("should provide user feedback during cleanup operations", async ({
      page,
    }) => {
      ...
      if (await statusMessage.isVisible()) {
        await expect(statusMessage).toContainText(
          /Fetching|Syncing|Cleaning|Complete/
        );
      }
  ...
  test.describe("Database Size and Storage Impact", () => {
    test("should demonstrate database size reduction after cleanup", async ({
      page,
    }) => {
      ...
      if (await analyticsLink.isVisible()) {
        ...
        await expect(
          page.locator('[data-testid="storage-stats"]')
        ).toBeVisible();

        // Should indicate potential savings
        await expect(
          page.locator('[data-testid="storage-reduction"]')
        ).toContainText(/60%|reduction/);
      }
      ...
      if (await cleanupResults.isVisible()) {
        await expect(cleanupResults).toContainText(
          /1200.*deleted|90.*MB.*freed/
        );
      }
  ...
  test.describe("Error Scenarios and Recovery", () => {
    test("should handle sync failures gracefully without data corruption", async ({
      page,
    }) => {
      let syncAttempts = 0;
      ...
      await expect(page.locator('[data-testid="sync-error"]')).toContainText(

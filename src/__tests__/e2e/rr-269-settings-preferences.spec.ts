...
test.describe("RR-269: Settings Page - User Preferences E2E", () => {
  ...
  test.describe("Display Preferences", () => {
    test("should load and display current preferences", async ({ page }) => {
      ...
      await expect(readingWidthSelect).toHaveValue(
        prefs.readingWidth || "medium"
      );
    });
    ...
    test("should show default values for new users", async ({ page }) => {
      ...
      await expect(page.locator('input[name="summaryWordCount"]')).toHaveValue(
        "70-80"
      );
      await expect(page.locator('select[name="summaryStyle"]')).toHaveValue(
        "objective"
      );
      await expect(page.locator('input[name="syncMaxArticles"]')).toHaveValue(
        "100"
      );
    });
  });
  ...
  test.describe("Sync Preferences", () => {
    ...
    test("should show sync impact warning for high values", async ({
      page,
    }) => {
      const syncInput = page.locator('input[name="syncMaxArticles"]');
      ...
      const warning = page.locator(
        '.warning-message:has-text("may impact performance")'
      );
  ...
  test.describe("Display Preferences", () => {
    ...
    test("should update reading width and preview", async ({ page }) => {
      ...
      const narrowWidth = await previewContainer.evaluate(
        (el) => el.offsetWidth
      );
  ...
  test.describe("Performance", () => {
    test("should load settings page within 3 seconds on mobile", async ({
      page,
    }) => {
      await setMobileViewport(page);
    ...
    test("should debounce rapid preference changes", async ({ page }) => {
      ...
      page.on("request", (request) => {
        if (
          request.url().includes("/api/users/preferences") &&
          request.method() === "PUT"
        ) {
          apiCallCount++;
        }
  ...
  test.describe("Error Handling", () => {
    test("should show error when API fails", async ({ page }) => {
      ...
      const errorToast = page.locator(
        '.toast-error:has-text("Failed to save")'
      );

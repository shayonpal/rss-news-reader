...
test.describe('RR-27: State Clearing Tests', () => {
  test('should clear preserved state when switching feeds', async ({ page }) => {
    ...
    const dropdownTrigger = page.locator('button[aria-label*="Current filter"]');
    if (await dropdownTrigger.isVisible()) {
      await dropdownTrigger.click();
      await page.waitForTimeout(500);
      
      const unreadOnlyOption = page.locator('[role="menuitem"]', { hasText: 'Unread only' });
    ...
    const preservedArticle = page.locator(`[data-article-id="${firstArticleId}"]`);
    ...
    const feedSidebar = page.locator('[data-testid="feed-sidebar"]').or(page.locator('.feed-sidebar')).or(page.locator('aside')).first();
    
    // Try to find a specific feed to click
    const specificFeed = page.locator('text=AppleInsider').or(page.locator('text=MacRumors')).or(page.locator('text=Forbes')).first();
    ...
    if (await specificFeed.isVisible()) {
      ...
      if (await allArticlesButton.isVisible()) {
        ...
        console.log(`Article still visible after feed switch: ${articleStillVisible}`);
        
        // The article should be gone from "Unread Only" view since state was cleared
        if (articleStillVisible) {
          console.log('❌ FAILURE: Article is still preserved after feed switch');
          expect(articleStillVisible).toBe(false);
        } else {
          console.log('✅ SUCCESS: Article correctly disappeared after feed switch');
        }
      }
    } else {
  ...
  test('should clear preserved state when switching filters', async ({ page }) => {
    ...
    if (await dropdownTrigger.isVisible()) {
      ...
      const allArticlesOption = page.locator('[role="menuitem"]', { hasText: 'All articles' });
    ...
    const articleStillVisible = await preservedArticle.isVisible();
    console.log(`Article still visible after filter switch: ${articleStillVisible}`);
    
    if (articleStillVisible) {
      console.log('❌ FAILURE: Article is still preserved after filter switch');
      expect(articleStillVisible).toBe(false);
    } else {
      console.log('✅ SUCCESS: Article correctly disappeared after filter switch');
    }
    
    console.log('🏁 Filter switch test completed');
  });
  
  test('should verify session storage is cleared on state clearing', async ({ page }) => {
    ...
    console.log('Session state after reading article:', sessionState ? 'exists' : 'null');
    ...
    const specificFeed = page.locator('text=AppleInsider').or(page.locator('text=MacRumors')).first();
    if (await specificFeed.isVisible()) {
      await specificFeed.click();
      await page.waitForTimeout(2000);
      ...
      console.log('Session state after feed change:', sessionState ? 'exists' : 'null');

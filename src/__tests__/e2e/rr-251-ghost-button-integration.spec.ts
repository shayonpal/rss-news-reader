...
test.describe("RR-251: Ghost Button Browser Integration", () => {
  ...
  test.describe("CSS Variable Resolution in Browser", () => {
    test("should resolve --ghost-text-light to violet-700 RGB", async ({ page }) => {
    ...
    test("should resolve --ghost-text-dark to white", async ({ page }) => {
      ...
      expect(ghostTextDarkValue).toMatch(/(?:rgb\()?255,?\s*255,?\s*255\)?|white/);
    });
  });

  test.describe("Article Navigation Ghost Buttons", () => {
    test("should display violet text on article navigation buttons", async ({ page }) => {
  ...
  test.describe("Theme Switching Integration", () => {
    test("should transition colors smoothly between light and dark mode", async ({ page }) => {
  ...
  test.describe("Visual Regression", () => {
    test("should maintain visual consistency of ghost buttons", async ({ page }) => {
      ...
      const ghostButtons = page.locator('button').filter({ hasText: /Previous|Next|Cancel|Close/ });
    ...
    test("should maintain hover effects with violet theme", async ({ page }) => {
      await page.goto("/reader/articles/test");
      await page.waitForLoadState("networkidle");

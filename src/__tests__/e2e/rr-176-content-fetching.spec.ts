...
test.describe("RR-176: Content Fetching Regression Tests", () => {
  ...
  test.describe("Auto-Parse Behavior", () => {
    test("should NOT auto-fetch for normal feeds with sufficient content", async ({
      page,
    }) => {
      // Navigate to a normal feed (not partial)
      await page.click(
        '[data-testid="feed-item"]:not([data-partial="true"]):first-child'
      );
    ...
    test("should auto-fetch ONLY for partial feeds", async ({ page }) => {
      ...
      await page.click(
        '[data-testid="feed-item"][data-partial="true"]:first-child'
      );
      await page.waitForSelector('[data-testid="article-item"]');
      ...
      const fetchPromise = page.waitForResponse(
        (response) =>
          response.url().includes("/fetch-content") && response.status() === 200
      );
    ...
    test("should auto-fetch for short content (<500 chars) in any feed", async ({
      page,
    }) => {
      ...
      const shortArticle = page
        .locator('[data-testid="article-item"][data-short="true"]')
        .first();
    ...
    test("should auto-fetch for content with truncation indicators", async ({
      page,
    }) => {
      ...
      const truncatedArticle = await page
  ...
  test.describe("Manual Fetch/Revert Operations", () => {
    test("fetch button should retrieve and display full content", async ({
      page,
    }) => {
      ...
      await expect(fetchButton).toHaveAttribute(
        "aria-label",
        "Original Content"
      );
    ...
    test("revert button should restore previous content without API call", async ({
      page,
    }) => {
      ...
      // Get fetched content
      const fetchedContent = await page
        .locator(".article-content")
        .textContent();
      ...
      const revertedContent = await page
        .locator(".article-content")
        .textContent();
  ...
  test.describe("Button Synchronization", () => {
    test("header and footer buttons should always show same state", async ({
      page,
    }) => {
      // Navigate to article
      await page.click('[data-testid="feed-item"]:first-child');
      ...
      const headerButton = page.locator(
        '.article-header [data-testid="fetch-button"]'
      );
      const footerButton = page.locator(
        '.article-footer [data-testid="fetch-button"]'
      );
      ...
      await expect(headerButton).toHaveAttribute(
        "aria-label",
        "Original Content"
      );
      await expect(footerButton).toHaveAttribute(
        "aria-label",
        "Original Content"
      );
    ...
    test("rapid clicks should not desynchronize buttons", async ({ page }) => {
      ...
      await Promise.all([headerButton.click(), footerButton.click()]);
  ...
  test.describe("Content State Transitions", () => {
    test("should maintain fetched content when navigating between articles", async ({
      page,
    }) => {
      ...
      const newFetchButton = page
        .locator('[data-testid="fetch-button"]')
        .first();
      await expect(newFetchButton).toHaveAttribute(
        "aria-label",
        "Full Content"
      );
  ...
  test.describe("Performance and Edge Cases", () => {
    ...
    test("should handle very large content efficiently", async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="article-item"]');
      await page.click(
        '[data-testid="article-item"][data-large="true"]:first-child'
      );

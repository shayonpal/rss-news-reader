...
test.describe('RR-115: Service Availability Detection and Timing', () => {
  ...
  const testHealthEndpoint = async (page: any, endpoint: string, options: any = {}) => {
    let response;
    let data;
  ...
  test.describe('Service Availability Detection', () => {
    test('should detect when all services are available and healthy', async ({ page }) => {
      ...
      console.log(`  - App: HTTP ${appResult.response.status()}, status=${appResult.data.status}`);
      console.log(`  - DB: HTTP ${dbResult.response.status()}, database=${dbResult.data.database}`);
    });

    test('should handle service startup sequence correctly', async ({ page }) => {
      ...
      requests.forEach((req, index) => {
        ...
        console.log(`✓ Startup request ${index + 1}: HTTP ${req.status}, status=${req.data.status}`);
      });
    });

    test('should detect unavailable services and return appropriate status codes', async ({ page }) => {
      // Test ping endpoint which should always be available
      const pingResult = await testHealthEndpoint(page, '/reader/api/health/app?ping=true');
      ...
      const fullResult = await testHealthEndpoint(page, '/reader/api/health/app');
      ...
      if (fullResult.data.dependencies) {
        ...
        if (deps.database === 'error' && fullResult.data.status === 'unhealthy') {
          expect(fullResult.response.status()).toBe(503);
        }
      }
      
      console.log(`✓ Service availability detection: HTTP ${fullResult.response.status()}`);
    });

    test('should maintain service availability consistency over time', async ({ page }) => {
      ...
      results.forEach(result => {
        console.log(`✓ Check ${result.check}: HTTP ${result.status}, health=${result.health}, time=${result.responseTime}ms`);
      });
  ...
  test.describe('Timing and Race Conditions', () => {
    test('should handle concurrent requests without race conditions', async ({ page }) => {
      ...
      const appPromises = Array(concurrentRequests).fill(null).map(() => 
        testHealthEndpoint(page, '/reader/api/health/app')
      );
      
      const dbPromises = Array(concurrentRequests).fill(null).map(() =>
        testHealthEndpoint(page, '/reader/api/health/db')
      );
      ...
      console.log(`✓ Handled ${concurrentRequests * 2} concurrent requests in ${totalTime}ms`);
      ...
      console.log(`✓ App status code consistency: ${uniqueAppCodes.join(', ')}`);
      console.log(`✓ DB status code consistency: ${uniqueDbCodes.join(', ')}`);
    });

    test('should handle rapid sequential requests efficiently', async ({ page }) => {
    ...
    test('should prevent race conditions in health status determination', async ({ page }) => {
      ...
      setTimeout(() => {
        promises.push(testHealthEndpoint(page, '/reader/api/health/app?ping=true'));
      }, 100);
  ...
  test.describe('Network Timeout Handling', () => {
    test('should handle network timeouts gracefully', async ({ page }) => {
      ...
      try {
        ...
        if (error.message.includes('timeout')) {
          console.log(`⚠️ Health endpoint timed out - this may indicate performance issues`);
          throw new Error('Health endpoints should not timeout under normal conditions');
        }
        throw error;
      }
    });

    test('should provide fast ping responses even under load', async ({ page }) => {
      const pingPromises = Array(10).fill(null).map(() => {
        const startTime = Date.now();
        return testHealthEndpoint(page, '/reader/api/health/app?ping=true')
          .then(result => ({
            ...result,
      ...
      const avgPingTime = results.reduce((sum, r) => sum + r.responseTime, 0) / results.length;
    ...
    test('should handle slow database responses appropriately', async ({ page }) => {
  ...
  test.describe('Performance Under Load', () => {
    test('should maintain performance under sustained load', async ({ page }) => {
      ...
      const loadTest = async () => {
        while (Date.now() - startTime < loadTestDuration) {
          const requestStart = Date.now();
          try {
            const result = await testHealthEndpoint(page, '/reader/api/health/app?ping=true');
    ...
    test('should recover gracefully from temporary overload', async ({ page }) => {
      ...
      const successful = burstResults.filter(r => [200, 503].includes(r.response.status())).length;
      ...
      const recoveryResult = await testHealthEndpoint(page, '/reader/api/health/app?ping=true');

...
test.describe('RR-27: Debug Session State', () => {
  test('should debug the complete state flow', async ({ page }) => {
    ...
    const debugInfo = await page.evaluate((targetId) => {
      ...
      const targetArticle = document.querySelector(`[data-article-id="${targetId}"]`);
      const targetExists = targetArticle !== null;
      const targetVisible = targetArticle ? window.getComputedStyle(targetArticle).display !== 'none' : false;
      const targetReadStatus = targetArticle ? targetArticle.getAttribute('data-is-read') : null;
    ...
    if (debugInfo.sessionParsed && debugInfo.sessionValid) {
      const isManuallyRead = debugInfo.sessionParsed.manualReadArticles?.includes(firstArticleId);
      const isAutoRead = debugInfo.sessionParsed.autoReadArticles?.includes(firstArticleId);
      const isExpired = debugInfo.sessionParsed.timestamp && (Date.now() - debugInfo.sessionParsed.timestamp > 30 * 60 * 1000);
      ...
      console.log(`- Should be preserved: ${(isManuallyRead || isAutoRead) && !isExpired}`);
    }

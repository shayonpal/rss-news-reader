...
test.describe("Core User Journeys - RR-184", () => {
  ...
  test("Article Reading Journey: Browse feeds → Select article → Read content → Navigate back", async ({
    page,
  }) => {
    ...
    const feedsList = page.locator(
      '[data-testid="feeds-list"], .feeds-list, [role="navigation"] ul'
    );
    ...
    const articlesList = page.locator(
      '[data-testid="articles-list"], .articles-list, main [role="list"]'
    );
    await expect(articlesList).toBeVisible({ timeout: 10000 });

    // Step 5: Select first article
    const firstArticle = articlesList
      .locator('article, [role="article"], .article-item')
      .first();
    ...
    const selectedFeed = sidebar.locator(
      '.selected, [aria-current="page"], [data-selected="true"]'
    );
    if (feedTitle) {
  ...
  test("Sync Validation: Manual sync triggers and updates UI", async ({
    page,
  }) => {
    // Step 1: Find sync button
    const syncButton = page.locator(
      '[data-testid="sync-button"], button:has-text("Sync"), [aria-label*="sync" i]'
    );

    // Step 2: Get initial article count
    const articleCount = page.locator(
      '[data-testid="article-count"], .unread-count, .badge'
    );
  ...
  test("Cross-Device State: Read/unread status persists", async ({ page }) => {
    ...
    const unreadArticle = articlesList
      .locator('[data-read="false"], .unread, article:not(.read)')
      .first();

    if (await unreadArticle.isVisible()) {
      // Step 3: Get article identifier
      const articleId =
        (await unreadArticle.getAttribute("data-article-id")) ||
        (await unreadArticle.getAttribute("id"));
      const articleTitle = await unreadArticle
        .locator("h2, h3, .article-title")
        .textContent();
      ...
      const readArticle = articleId
        ? articlesList.locator(
            `[data-article-id="${articleId}"], #${articleId}`
          )
        : articlesList.locator(`article:has-text("${articleTitle}")`);

      // Verify it's marked as read
      await expect(readArticle)
  ...
  test("Performance Testing: Page load times are acceptable", async ({
    page,
  }) => {
  ...
  test("PWA Installation: Verify PWA manifest and service worker", async ({
    page,
    browserName,
  }) => {
    ...
    test.skip(
      browserName === "firefox",
      "Firefox does not fully support PWA installation"
    );
...
test.describe("Mobile-Specific Tests - iOS Focus", () => {
  ...
  test("Touch Interactions: Swipe and tap gestures work correctly", async ({
    page,
  }) => {
    await page.goto(APP_URL);
  ...
  test("Viewport Responsiveness: Layout adapts to device orientation", async ({
    page,
  }) => {
    await page.goto(APP_URL);

...
import { createE2EHelpers, TestUser, TestFeed, TestArticle } from '../helpers/e2e-helpers';
...
const TEST_ARTICLES = {
  techArticles: [
    { id: 'tech-1', title: 'AI Breakthrough', feedId: 'feed-tech-001', isRead: false },
    { id: 'tech-2', title: 'Quantum Computing', feedId: 'feed-tech-001', isRead: false },
    { id: 'tech-3', title: 'Robotics Update', feedId: 'feed-tech-001', isRead: false }
  ],
  businessArticles: [
    { id: 'biz-1', title: 'Market Analysis', feedId: 'feed-business-001', isRead: false },
    { id: 'biz-2', title: 'Startup Funding', feedId: 'feed-business-001', isRead: false },
    { id: 'biz-3', title: 'IPO News', feedId: 'feed-business-001', isRead: false }
  ],
  sportsArticles: [
    { id: 'sport-1', title: 'Championship Final', feedId: 'feed-sports-001', isRead: false },
    { id: 'sport-2', title: 'Transfer News', feedId: 'feed-sports-001', isRead: false },
    { id: 'sport-3', title: 'Match Preview', feedId: 'feed-sports-001', isRead: false }
  ]
};
...
test.describe('RR-240: Feed-Filtered Navigation with Preserved Articles', () => {
  ...
  test.describe('Feed-Specific Article Preservation', () => {
    test('preserves read articles in feed-filtered view on back navigation', async ({ page }) => {
      ...
      const techArticlesBefore = await page.$$('[data-testid^="article-card-tech-"]');
      ...
      await expect(page.locator('[data-testid="article-read-indicator"]')).toBeVisible();
      ...
      const readArticle = await page.locator('[data-testid="article-card-tech-1"][data-read="true"]');
      await expect(readArticle).toBeVisible();
      
      // Verify other articles remain unread
      const unreadArticles = await page.$$('[data-testid^="article-card-tech-"][data-read="false"]');
      expect(unreadArticles).toHaveLength(2);
    });

    test('prevents cross-feed contamination of preserved articles', async ({ page }) => {
      ...
      await page.click('[data-testid="feed-filter-button"]');
      await page.click(`[data-testid="feed-option-${TEST_FEEDS.businessFeed.id}"]`);
      await helpers.waitForArticlesToLoad();
      
      // Verify no tech articles appear in business feed
      const techArticlesInBusinessFeed = await page.$$('[data-testid^="article-card-tech-"]');
      expect(techArticlesInBusinessFeed).toHaveLength(0);
      
      // Verify only business articles are shown
      const businessArticles = await page.$$('[data-testid^="article-card-biz-"]');
      expect(businessArticles).toHaveLength(3);
      
      // All should be unread (no preserved tech article)
      const readArticles = await page.$$('[data-testid^="article-card-"][data-read="true"]');
      expect(readArticles).toHaveLength(0);
    });

    test('includes all preserved articles in "All Articles" view', async ({ page }) => {
      ...
      // Sports feed
      await page.click('[data-testid="feed-filter-button"]');
      await page.click(`[data-testid="feed-option-${TEST_FEEDS.sportsFeed.id}"]`);
      ...
      await expect(page.locator('[data-testid="article-card-tech-1"][data-read="true"]')).toBeVisible();
      await expect(page.locator('[data-testid="article-card-biz-1"][data-read="true"]')).toBeVisible();
      await expect(page.locator('[data-testid="article-card-sport-1"][data-read="true"]')).toBeVisible();
  ...
  test.describe('Navigation Flow Consistency', () => {
    test('maintains preserved articles through multiple navigation cycles', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="article-card-tech-1"][data-read="true"]')).toBeVisible();
      await expect(page.locator('[data-testid="article-card-tech-2"][data-read="true"]')).toBeVisible();
      ...
      await expect(page.locator('[data-testid="article-card-tech-1"][data-read="true"]')).toBeVisible();
      await expect(page.locator('[data-testid="article-card-tech-2"][data-read="true"]')).toBeVisible();
      await expect(page.locator('[data-testid="article-card-tech-3"][data-read="true"]')).toBeVisible();
    });

    test('handles rapid feed switching without losing preserved articles', async ({ page }) => {
      ...
      for (let i = 0; i < 3; i++) {
        for (const { feed, article } of feedSequence) {
          ...
          await expect(page.locator(`[data-testid="article-card-${article}"][data-read="true"]`)).toBeVisible();
        }
      }
    });

    test('preserves articles correctly with browser back/forward navigation', async ({ page }) => {
  ...
  test.describe('Filter Combinations', () => {
    test('applies read status filter with feed-specific preserved articles', async ({ page }) => {
      ...
      const visibleArticles = await page.$$('[data-testid^="article-card-tech-"]');
      ...
      const readArticles = await page.$$('[data-testid^="article-card-tech-"][data-read="true"]');
      expect(readArticles).toHaveLength(2);
    });

    test('combines topic filter with feed filter and preserved articles', async ({ page }) => {
  ...
  test.describe('Performance Requirements', () => {
    test('maintains query performance under 50ms with preserved articles', async ({ page }) => {
      ...
      const preservedArticles = await page.$$('[data-testid^="article-card-tech-"][data-read="true"]');
      expect(preservedArticles).toHaveLength(3);
    });

    test('handles maximum preserved articles limit efficiently', async ({ page }) => {
      ...
      const oldestArticle = await page.$$('[data-testid="article-card-bulk-0"][data-read="true"]');
      expect(oldestArticle).toHaveLength(0);
      
      // Verify newest preserved articles are kept
      await expect(page.locator('[data-testid="article-card-bulk-49"][data-read="true"]')).toBeVisible();
      await expect(page.locator('[data-testid="article-card-tech-1"][data-read="true"]')).toBeVisible();
    });
  });

  test.describe('Edge Cases and Error Handling', () => {
    test('handles corrupt sessionStorage data gracefully', async ({ page, context }) => {
    ...
    test('handles missing feedId in article data', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="article-card-orphan-1"][data-read="true"]')).toBeVisible();
    });
  });
});

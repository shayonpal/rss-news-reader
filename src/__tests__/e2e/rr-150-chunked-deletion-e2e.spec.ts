...
test.describe("RR-150: Chunked Article Deletion E2E", () => {
  ...
  test.describe("Manual Sync with Large Article Cleanup", () => {
    test("should complete sync without 414 errors when cleaning up 1000+ articles", async ({
      page,
    }) => {
      ...
      for (let i = 0; i < 120; i++) {
        ...
        const errorMessage = await page
          .locator('[data-testid="sync-error"]')
          .textContent();
        ...
        if (
          errorMessage?.includes("414") ||
          errorMessage?.includes("Request-URI Too Large")
        ) {
          has414Error = true;
          break;
        }
    ...
    test("should show progress indicators during chunked cleanup operations", async ({
      page,
    }) => {
      // Start manual sync
      ...
      const progressText = await page
        .locator('[data-testid="cleanup-progress"]')
        .textContent();
      ...
      if (progressText && !progressText.includes("No articles to clean")) {
        ...
        expect(progressText).toMatch(
          /Processing chunk \d+ of \d+|Cleaned \d+ articles/
        );
      }
  ...
  test.describe("Automatic Sync Behavior", () => {
    test("should handle scheduled sync without URI length errors", async ({
      page,
    }) => {
  ...
  test.describe("Error Handling and Recovery", () => {
    test("should display helpful error messages if chunked cleanup fails", async ({
      page,
    }) => {
    ...
    test("should allow retry of failed cleanup operations", async ({
      page,
    }) => {
  ...
  test.describe("Performance and User Experience", () => {
    test("should maintain responsive UI during large cleanup operations", async ({
      page,
    }) => {
    ...
    test("should show estimated time remaining for large cleanup operations", async ({
      page,
    }) => {
  ...
  test.describe("Configuration and Admin Controls", () => {
    test("should allow admin to configure chunk size for cleanup operations", async ({
      page,
    }) => {
    ...
    test("should show cleanup statistics and health metrics", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="articles-cleaned-today"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="average-chunk-size"]')
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="cleanup-success-rate"]')
      ).toBeVisible();
  ...
  test.describe("Monitoring and Alerting", () => {
    test("should provide monitoring data for chunked operations", async ({
      page,
    }) => {
      ...
      expect(metricsText).toMatch(
        /chunks processed|average chunk time|success rate/i
      );
    });

    test("should alert on cleanup failures but not on 414 errors", async ({
      page,
    }) => {
      // Check alerts page
      ...
      if ((await alerts.count()) > 0) {
        ...
        const uri414Alerts = alertTexts.filter(
          (text) =>

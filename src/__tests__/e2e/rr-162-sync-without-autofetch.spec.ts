...
test.describe('RR-162: E2E Sync Without Auto-Fetch', () => {
  ...
  test('should complete sync to 100% without hanging at 92%', async ({ page }) => {
    // Look for sync button
    const syncButton = page.locator('[data-testid="sync-button"], button:has-text("Sync")').first();
    ...
    await expect(page.locator('[data-testid="sync-progress"], [role="progressbar"]').first())
    ...
    while (!syncCompleted && (Date.now() - startTime) < SYNC_TIMEOUT) {
      ...
      const completionIndicators = await Promise.race([
        page.locator('text=/sync.{0,20}complet/i').isVisible().catch(() => false),
        page.locator('[data-testid="sync-complete"]').isVisible().catch(() => false),
        page.locator('[data-testid="sync-progress"][data-value="100"]').isVisible().catch(() => false),
      ]);
      ...
      const errorVisible = await page.locator('text=/sync.{0,20}(fail|error)/i').isVisible().catch(() => false);
      if (errorVisible) {
        throw new Error('Sync failed with error');
      }

      // Check current progress if displayed
      const progressElement = page.locator('[data-testid="sync-progress"], [data-progress]').first();
  ...
  test('should not show auto-fetch related UI elements', async ({ page }) => {
    ...
    if (await settingsButton.isVisible().catch(() => false)) {
      ...
      await expect(page.locator('label:has-text("Automatically fetch full content")')).not.toBeVisible();
      await expect(page.locator('[data-testid="auto-fetch-toggle"]')).not.toBeVisible();

      // Close settings if there's a close button
      const closeButton = page.locator('[data-testid="close-settings"], button:has-text("Close")').first();
  ...
  test('should allow manual content fetching', async ({ page }) => {
    ...
    const firstArticle = page.locator('[data-testid="article-list"] article, article').first();
    ...
    if (await fetchButton.isVisible().catch(() => false)) {
      ...
      await expect(page.locator('[data-testid="loading-content"], .loading')).toBeVisible({ timeout: 5000 })
        .catch(() => {/* Loading indicator might be quick */});
  ...
  test('should track sync progress smoothly', async ({ page }) => {
    ...
    await page.waitForFunction(
      () => {
        const progressEl = document.querySelector('[data-testid="sync-progress"]');
        const completeEl = document.querySelector('[data-testid="sync-complete"]');
        return completeEl || (progressEl && progressEl.textContent?.includes('100'));
      },
    ...
    if (progressSequence.length > 0) {
      ...
      for (let i = 1; i < progressSequence.length; i++) {
        const timeDiff = progressSequence[i].timestamp - progressSequence[i - 1].timestamp;
  ...
  test('should handle network errors gracefully during sync', async ({ page }) => {
    ...
    await expect(page.locator('text=/error|fail/i')).toBeVisible({ timeout: 5000 });
  ...
  test('should display correct sync statistics', async ({ page }) => {
    ...
    await page.waitForFunction(
      ...
      { timeout: SYNC_TIMEOUT }
    ).catch(() => {/* Sync might already be complete */});

    // Check for sync statistics
    const statsSection = page.locator('[data-testid="sync-stats"], [data-testid="statistics"]').first();
  ...
  test('should persist manual fetch option for articles', async ({ page }) => {
    ...
    const partialArticle = page.locator('article:not(:has([data-testid="full-content"]))').first();
    
    if (await partialArticle.isVisible().catch(() => false)) {
      await partialArticle.click();

      // Should show manual fetch option
      const fetchButton = page.locator('[data-testid="fetch-content-button"]').first();
      ...
      const contentLoaded = await page.locator('[data-testid="article-content"]').isVisible()
        .catch(() => false);
      const fetchButtonGone = !(await fetchButton.isVisible().catch(() => false));
  ...
  while (Date.now() - startTime < timeout) {
    ...
    const isComplete = await page.evaluate(() => {
      const indicators = [
        ...
        document.querySelector('[data-testid="sync-progress"][data-value="100"]'),
        document.querySelector('.sync-status:has-text("Completed")'),
        Array.from(document.querySelectorAll('*')).find(
          el => el.textContent?.match(/sync.{0,20}complete/i)
        ),
      ];

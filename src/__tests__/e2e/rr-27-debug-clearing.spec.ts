...
test.describe('RR-27: Debug State Clearing', () => {
  test('should debug the filter change state clearing process', async ({ page }) => {
    ...
    console.log('Session state after reading:', sessionState ? 'exists with preserved articles' : 'null');
    ...
    const currentFilter = await page.evaluate(() => {
      ...
      const dropdownButton = document.querySelector('button[aria-label*="Current filter"]');
      if (dropdownButton) {
    ...
    console.log('Session state after switching to All:', sessionState ? 'still exists' : 'cleared');
    ...
    if (isPreservedAfter) {
      console.log('❌ Article is still preserved - state clearing may not be working');
    } else {
  ...
  test('should debug feed changes for state clearing', async ({ page }) => {
    ...
    console.log('Session state before feed change:', sessionState ? 'exists' : 'null');
    ...
    if (await specificFeed.isVisible()) {
      ...
      console.log('Session state after clicking specific feed:', sessionState ? 'exists' : 'cleared');
    }
    ...
    if (await allArticlesButton.isVisible()) {
      ...
      console.log('Session state after clicking All Articles:', sessionState ? 'exists' : 'cleared');
    }
    ...
    if (isPreservedAfter) {
      ...
      console.log('✅ SUCCESS: Article correctly disappeared after feed change');
    }

...
test.describe('RR-27: Real Navigation Test', () => {
  test('should navigate to article and back with proper state preservation', async ({ page }) => {
    ...
    const originalArticle = page.locator(`[data-article-id="${firstArticleId}"]`);
    ...
    const isRead = isVisible ? await originalArticle.getAttribute('data-is-read') : null;
    ...
    if (isVisible) {
      ...
      await page.screenshot({ path: 'real-navigation-failure.png', fullPage: true });
      expect(isVisible).toBe(true);
    }
    ...
    if (sessionState) {
      console.log(`Session contains ${sessionState.manualReadArticles?.length || 0} manually read articles`);
      console.log(`Session contains ${sessionState.autoReadArticles?.length || 0} auto read articles`);
      
      // Check if our article is tracked
      const isTrackedAsManual = sessionState.manualReadArticles?.includes(firstArticleId);
      const isTrackedAsAuto = sessionState.autoReadArticles?.includes(firstArticleId);

...
test.describe("RR-216 + RR-27: Complete Integration Test", () => {
  ...
  test("Complete filter preservation + article preservation flow", async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(10, await articleCandidates.count()); i++) {
      ...
      if (text.includes('unread') || text.includes('Topics') || text.includes('Feeds')) {
        continue;
      }
      ...
      if (isClickable && text.length > 20) {
        ...
        if (newUrl.includes('/article/')) {
          ...
          console.log("❌ Click didn't navigate to article, trying next candidate");
        }
      }
    }
    ...
    console.log("✅ Complete integration test finished - check console logs for preservation behavior");
  });

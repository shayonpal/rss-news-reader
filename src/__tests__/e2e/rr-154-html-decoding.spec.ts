...
test.describe('RR-154: HTML Entity Decoding E2E Tests', () => {
  ...
  test.describe('Article Display', () => {
    test('should display article titles without HTML entities', async ({ page }) => {
    ...
    test('should display article content without HTML entities', async ({ page }) => {
    ...
    test('should display decoded characters in article preview', async ({ page }) => {
      // Look for article previews/descriptions
      const previews = await page.locator('[data-testid="article-preview"]').allTextContents();
  ...
  test.describe('Article Links', () => {
    test('should preserve article URLs correctly', async ({ page }) => {
      ...
      const links = await page.locator('[data-testid="article-link"]').evaluateAll(
    ...
    test('should open article in new tab with correct URL', async ({ page, context }) => {
      // Find an article with external link
      const articleWithLink = page.locator('[data-testid="article-external-link"]').first();
  ...
  test.describe('Sync Functionality', () => {
    test('should sync articles with properly decoded entities', async ({ page }) => {
      // Trigger manual sync
      const syncButton = page.locator('[data-testid="sync-button"]');
      
      if (await syncButton.count() > 0) {
        // Record initial article count
        const initialArticles = await page.locator('[data-testid="article-item"]').count();
        
        // Click sync
        await syncButton.click();
        
        // Wait for sync to complete
        await page.waitForSelector('[data-testid="sync-status"]:has-text("Complete")', {
          timeout: 30000
        });
        
        // Check new articles don't have entities
        const newArticles = await page.locator('[data-testid="article-item"]').count();
        
        if (newArticles > initialArticles) {
          // Get the newest articles
          const newTitles = await page.locator('[data-testid="article-title"]')
            .first()
  ...
  test.describe('Search and Filter', () => {
    test('should search with decoded text', async ({ page }) => {
      ...
      if (await searchInput.count() > 0) {
        ...
        const results = await page.locator('[data-testid="article-title"]').allTextContents();
  ...
  test.describe('Performance', () => {
    test('should load articles quickly with decoding', async ({ page }) => {
      ...
      const articleCount = await page.locator('[data-testid="article-item"]').count();
      expect(articleCount).toBeGreaterThan(0);
    });

    test('should handle infinite scroll with decoded content', async ({ page }) => {
      // Wait for initial articles
      await page.waitForSelector('[data-testid="article-list"]');
      
      const initialCount = await page.locator('[data-testid="article-item"]').count();
      ...
      if (initialCount >= 20) {
        ...
        const newCount = await page.locator('[data-testid="article-item"]').count();
        
        if (newCount > initialCount) {
          // Check new articles for entities
          const lastArticles = await page.locator('[data-testid="article-title"]')
            .evaluateAll((elements, initial) => 
  ...
  test.describe('AI Summary', () => {
    test('should display AI summaries with decoded text', async ({ page }) => {
      ...
      const aiButton = page.locator('[data-testid="ai-summary-button"]').first();
      ...
      if (await aiButton.count() > 0) {
        ...
        // Get summary text
        const summary = await page.locator('[data-testid="ai-summary-content"]').textContent();
  ...
  test.describe('Mobile Responsiveness', () => {
    test('should display decoded content on mobile viewport', async ({ page }) => {
      // Set mobile viewport
      ...
      const mobileTitles = await page.locator('[data-testid="article-title"]').allTextContents();
  ...
  test.describe('Accessibility', () => {
    test('should have proper text for screen readers', async ({ page }) => {
      ...
      const altTexts = await page.locator('img[alt]').evaluateAll(
        elements => elements.map(el => el.getAttribute('alt'))
      );

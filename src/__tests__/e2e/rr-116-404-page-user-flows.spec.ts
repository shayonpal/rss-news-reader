...
test.describe("RR-116: 404 Page User Flows", () => {
  test.describe("Global 404 Page User Flows", () => {
    test("should display global 404 page for non-existent routes", async ({
      page,
    }) => {
    ...
    test("should be accessible via keyboard navigation", async ({ page }) => {
      ...
      expect(
        ["BODY", "BUTTON", "A", "INPUT"].includes(focusedElement || "BODY")
      ).toBeTruthy();
    });
  });

  test.describe("Article 404 Page User Flows", () => {
    test("should display article 404 page for non-existent article", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator(
          '[data-testid="file-question-icon"], .lucide-file-question, svg'
        )
      ).toBeVisible();
    });

    test("should navigate back to reader from article 404", async ({
      page,
    }) => {
    ...
    test("should handle malformed article IDs gracefully", async ({ page }) => {
      ...
      for (const malformedId of malformedIds) {
        await page.goto(
          `${READER_BASE}/article/${encodeURIComponent(malformedId)}`
        );
        await page.waitForLoadState("networkidle");

        // Should either show 404 or handle gracefully without error
        const hasError = await page
          .locator('h2:has-text("Article not found")')
          .isVisible();
        const hasContent = await page
          .locator('article, main, [data-testid="article-content"]')
          .isVisible();
  ...
  test.describe("Visual Consistency and Theming", () => {
    test("should maintain consistent styling in light mode", async ({
      page,
    }) => {
      // Force light mode
      ...
      const articleStyles = await page
        .locator("h2")
        .evaluate((el) => window.getComputedStyle(el));
    ...
    test("should maintain consistent styling in dark mode", async ({
      page,
    }) => {
      // Force dark mode
    ...
    test("should maintain layout consistency across viewport sizes", async ({
      page,
    }) => {
      const viewports = [
  ...
  test.describe("Accessibility User Flows", () => {
    ...
    test("should support high contrast mode", async ({ page }) => {
      ...
      await expect(
        page.locator('button:has-text("Back to Reader")')
      ).toBeVisible();
    ...
    test("should handle URL manipulation attacks", async ({ page }) => {
      ...
      for (const url of maliciousUrls) {
        ...
        const has404Content = await page
          .locator('h2:has-text("Article not found"), h2:has-text("Not Found")')
          .isVisible();
  ...
  test.describe("Integration with Main App", () => {
    ...
    test("should preserve URL structure for analytics", async ({ page }) => {
      await page.goto(
        `${READER_BASE}/article/analytics-test-id?utm_source=test&ref=direct`
      );
      await page.waitForLoadState("networkidle");

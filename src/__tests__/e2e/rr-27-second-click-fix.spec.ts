...
test.describe('RR-27: Second Article Click Fix', () => {
  test('should preserve only session-read articles after clicking second article', async ({ page }) => {
    ...
    const filterButton = page.locator('button').filter({ hasText: /Current filter|All articles|Unread only/i }).first();
    if (await filterButton.isVisible()) {
      await filterButton.click();
      await page.waitForTimeout(500);
      
      const unreadOption = page.locator('[role="menuitem"]').filter({ hasText: 'Unread only' });
    ...
    const firstArticleVisible = await page.locator(`[data-article-id="${firstArticleId}"]`).isVisible();
    ...
    const firstStillVisible = await page.locator(`[data-article-id="${firstArticleId}"]`).isVisible();
    const secondVisible = await page.locator(`[data-article-id="${secondArticleId}"]`).isVisible();
    ...
    console.log('Final session state:', {
      ...
      hasFirstArticle: finalSessionState?.manualReadArticles?.includes(firstArticleId),
      hasSecondArticle: finalSessionState?.manualReadArticles?.includes(secondArticleId)
    });

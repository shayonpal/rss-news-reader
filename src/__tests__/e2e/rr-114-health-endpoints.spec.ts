...
test.describe('RR-114: Health Endpoint Enhancements - E2E', () => {
  ...
  test.describe('/api/health/app version property tests', () => {
    test('should return version property in app health endpoint', async ({ page }) => {
      const { response, data } = await testHealthEndpoint(page, '/reader/api/health/app');
    ...
    test('should return version property with ping=true parameter', async ({ page }) => {
      const { response, data } = await testHealthEndpoint(page, '/reader/api/health/app?ping=true');
      ...
      console.log(`✓ App health ping endpoint returned version: ${data.version}`);
    });

    test('should return version property consistently across multiple requests', async ({ page }) => {
      const requests = [];
  ...
  test.describe('/api/health/db connection property alias tests', () => {
    test('should return connection property as alias for database property', async ({ page }) => {
      const { response, data } = await testHealthEndpoint(page, '/reader/api/health/db');
      ...
      console.log(`✓ DB health endpoint returned database: ${data.database}, connection: ${data.connection}`);
    });

    test('should maintain connection alias across different database states', async ({ page }) => {
      ...
      results.forEach(({ data }, index) => {
        ...
        console.log(`✓ Request ${index + 1}: database=${data.database}, connection=${data.connection}`);
      });
    });

    test('should return connection alias even when database health checks fail', async ({ page }) => {
      ...
      console.log(`✓ DB health status ${response.status()}: database=${data.database}, connection=${data.connection}`);
    });
  });

  test.describe('Combined health endpoint validation', () => {
    test('should return all required properties from both endpoints', async ({ page }) => {
      ...
      console.log(`  - DB status: database=${dbResult.data.database}, connection=${dbResult.data.connection}`);
    });
    ...
    test('should handle network timeouts gracefully', async ({ page }) => {
      ...
      try {
        ...
        console.log('✓ Health endpoints respond within timeout with version property');
      } catch (error) {
        // If timeout occurs, this is valuable information for the implementation
        console.log(`⚠️  Timeout occurred - this may indicate performance issues: ${error}`);
  ...
  test.describe('RR-114 Implementation validation', () => {
    test('should validate version matches package.json format', async ({ page }) => {
      const { response, data } = await testHealthEndpoint(page, '/reader/api/health/app');
      ...
      console.log(`✓ Version format validated: ${data.version} (${major}.${minor}.${patch})`);
    });

    test('should maintain backwards compatibility with existing properties', async ({ page }) => {
      const [appResult, dbResult] = await Promise.all([
        testHealthEndpoint(page, '/reader/api/health/app'),
        testHealthEndpoint(page, '/reader/api/health/db')
      ]);
      ...
      console.log('✓ All existing properties maintained alongside new RR-114 requirements');
    });
  });
});
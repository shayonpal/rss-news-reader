...
test.describe('RR-30: Sync Conflict Detection E2E', () => {
  ...
  test.describe('Manual Sync with Conflict Detection', () => {
    test('should detect and log conflicts during manual sync', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="sync-button"]', { state: 'visible' });
    ...
    test('should continue sync operation despite detecting conflicts', async ({ page }) => {
      let syncAttempts = 0;
          ...
          await route.fulfill({
            ...
            body: JSON.stringify({
              ...
              summary: {
                ...
                articlesUpdated: 45,
                conflictsDetected: 5,
                conflictResolution: 'All conflicts resolved using remote values'
              }
            }),
          });
        } else {
      ...
      const firstSyncStatus = await page.textContent('[data-testid="sync-status"]');
      ...
      const secondSyncStatus = await page.textContent('[data-testid="sync-status"]');
  ...
  test.describe('User Interaction During Conflicts', () => {
    test('should handle marking article as read that conflicts with remote', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="article-list"]', { state: 'visible' });
      ...
      const articleState = await page.locator('[data-testid="article-item"]').first()
    ...
    test('should handle starring article that conflicts with remote', async ({ page }) => {
      ...
      const syncCompleted = await page.isVisible('[data-testid="sync-status"]:has-text("completed")');
  ...
  test.describe('Conflict Visibility and Monitoring', () => {
    test('should not show conflict errors to users', async ({ page }) => {
      ...
      const conflictWarnings = await page.locator('[data-testid="conflict-warning"]').count();
    ...
    test('should complete sync workflow end-to-end with conflicts', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="sync-status"]:has-text("progress")', {
        timeout: 5000
      });
      ...
      const userFacingConflictIndicators = await page.locator('.conflict-indicator').count();

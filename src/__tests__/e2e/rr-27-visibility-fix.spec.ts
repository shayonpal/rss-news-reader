...
test.describe('RR-27: Article Visibility Fix', () => {
  test('should keep clicked article visible in Unread Only mode', async ({ page }) => {
    ...
    const firstArticleTitle = await firstArticle.locator('h2, h3, [class*="title"]').first().textContent();
    
    console.log(`🎯 Target article: ${firstArticleId} - "${firstArticleTitle?.substring(0, 50)}..."`);
    ...
    if (navigationResult.success) {
      ...
      console.log(`Original article visible after back navigation: ${isVisibleAfterBack}`);
      
      // This is the key test - the article should still be visible
      if (isVisibleAfterBack) {
        console.log('✅ SUCCESS: Article remained visible after back navigation');
        
        // Check the read status - it should now be read
        const isReadAfter = await originalArticleAfterBack.getAttribute('data-is-read');
        ...
        const hasReadStyling = await originalArticleAfterBack.evaluate(el => {
          ...
          return {
            ...
            hasSessionPreservedClass: el.classList.contains('session-preserved-read')
          };
        });
        ...
        await page.screenshot({ path: 'visibility-test-failure.png', fullPage: true });
      ...
      throw new Error(`Navigation simulation failed: ${navigationResult.error}`);
    }
    
    console.log('🏁 Visibility fix test completed');
  });
  
  test('should verify session storage contains the article state', async ({ page }) => {

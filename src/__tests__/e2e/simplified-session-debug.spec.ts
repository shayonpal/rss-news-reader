...
async function captureSessionState(page: Page, label: string) {
  ...
  if (state.sessionState) {
    console.log(`Auto-read: ${state.sessionState.autoReadArticles?.length || 0}`);
    console.log(`Manual-read: ${state.sessionState.manualReadArticles?.length || 0}`);
...
test.describe('Simplified Session Debug', () => {
  test('should debug article reading and session state creation', async ({ page }) => {
    console.log('🚀 Starting simplified session debug test...');
    
    // Enable console logging for state management
    page.on('console', msg => {
      if (msg.type() === 'log' && (msg.text().includes('🔧') || msg.text().includes('Hook') || msg.text().includes('State'))) {
    ...
    articleInfo.forEach((article, idx) => {
      console.log(`${idx + 1}. ID: ${article.id}, Read: ${article.isRead}, Visible: ${article.visible}, Element: ${article.hasClickHandler}`);
    });
    ...
    // Click the article
    const firstArticle = page.locator(`[data-article-id="${firstArticleId}"]`).first();
    ...
    if (currentUrl.includes('/article/')) {
      ...
      const state4 = await captureSessionState(page, 'After Article Processing');
      ...
      const updatedArticleInfo = await page.evaluate((targetId) => {
        const article = document.querySelector(`[data-article-id="${targetId}"]`);
      ...
      if (updatedArticleInfo) {
        ...
        console.log(`Session preserved: ${updatedArticleInfo.hasSessionPreserved}`);
        console.log(`Class: ${updatedArticleInfo.className.substring(0, 100)}...`);
      } else {
      ...
      if (articleInfo.length > 1 && updatedArticleInfo?.isRead === 'true') {
        ...
        if (nextUnreadInfo) {
          ...
          const secondArticle = page.locator(`[data-article-id="${nextUnreadInfo.id}"]`).first();
          ...
          await page.waitForTimeout(3000);
          const state6 = await captureSessionState(page, 'After Second Article');
          
          // Navigate back again
          await page.goto(RSS_READER_URL);
          await waitForArticleToLoad(page);
          
          const state7 = await captureSessionState(page, 'Back After Second Article');
          ...
          const finalReadCount = await page.evaluate(() => {
            ...
            return {
              ...
              ids: Array.from(visibleReadArticles).map(a => a.getAttribute('data-article-id'))
            };
          });
          ...
          if (finalReadCount.visible > 2) {
            console.log('🐛 BUG CONFIRMED: More than 2 read articles are visible');

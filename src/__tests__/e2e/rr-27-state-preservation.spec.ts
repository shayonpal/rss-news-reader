...
test.describe("RR-27: Article List State Preservation", () => {
  ...
  test("should preserve auto-read articles in Unread Only mode after back navigation", async ({
    page,
  }) => {
  ...
  test("should not immediately hide article when clicked in Unread Only mode", async ({
    page,
  }) => {
  ...
  test("should restore exact scroll position on back navigation", async ({
    page,
  }) => {
    ...
    const scrollPosition = await page.evaluate(() => {
      const container =
        document.querySelector('[data-testid="article-list-container"]') ||
        document.body;
  ...
  test("should differentiate between auto-read and manually read articles", async ({
    page,
  }) => {
    await switchToUnreadOnlyMode(page);
    ...
    const manuallyReadArticle = page.locator(
      `[data-article-id="${manuallyReadId}"]`
    );
    await expect(manuallyReadArticle).not.toHaveClass(/session-preserved-read/);
  });

  test("should handle session storage expiry after 30 minutes", async ({
    page,
  }) => {
    await switchToUnreadOnlyMode(page);
  ...
  test("should work with prev/next navigation between articles", async ({
    page,
  }) => {
    await switchToUnreadOnlyMode(page);
    ...
    const nextButton = page
      ...
      .or(page.locator('[aria-label="Next article"]'));
  ...
  test("should handle rapid navigation without losing state", async ({
    page,
  }) => {
    await switchToUnreadOnlyMode(page);
...
test.describe("RR-27: Performance and Edge Cases", () => {
  test("should handle large number of articles without performance issues", async ({
    page,
  }) => {
    await page.goto(RSS_READER_URL);
    await waitForArticlesToLoad(page);

...
test.describe("iPhone Button Tappability Tests", () => {
  ...
  test("All interactive elements should be tappable with proper touch targets", async ({
    page,
  }) => {
    ...
    for (const selector of interactiveSelectors) {
      ...
      for (const element of elements) {
        ...
        if (box) {
          ...
          const touchTargetMet =
            box.width >= MIN_TOUCH_SIZE || box.height >= MIN_TOUCH_SIZE;
          if (!touchTargetMet) {
            const text = (await element.textContent()) || "unknown";
            console.warn(
              `Element "${text}" has insufficient touch target: ${box.width}x${box.height}`
            );
          }
          ...
          expect(
            isClickable,
            `Element should be enabled: ${selector}`
          ).toBeTruthy();
        }
      }
    }
  ...
  test("Sidebar navigation buttons should respond to tap", async ({ page }) => {
    ...
    const sidebarToggle = page
      .locator(
        '[data-testid="sidebar-toggle"], [aria-label*="menu"], button:has-text("☰")'
      )
      .first();
    ...
    const feedButtons = await page.$$(
      '[data-testid*="feed"], .feed-item, [role="treeitem"]'
    );
    for (const feed of feedButtons.slice(0, 3)) {
      // Test first 3 feeds
  ...
  test("Article list buttons should be tappable", async ({ page }) => {
    ...
    await page.waitForSelector(
      'article, [data-testid*="article"], .article-item',
      { timeout: 10000 }
    );
    ...
    if (articles.length > 0) {
      ...
      const articleContent = page.locator(
        '.article-content, [data-testid="article-content"], main article'
      );
      const isContentVisible = await articleContent
        .isVisible()
        .catch(() => false);
    ...
    // Test star button
    const starButtons = await page.$$(
      '[data-testid*="star"], [aria-label*="star"], button:has-text("⭐"), button:has-text("☆")'
    );
    for (const star of starButtons.slice(0, 2)) {
      // Test first 2 star buttons
      if (await star.isVisible()) {
        const initialState =
          (await star.getAttribute("aria-pressed")) || "false";
        await star.tap();
        await page.waitForTimeout(200);
  ...
  test("Header action buttons should respond to tap", async ({ page }) => {
    ...
    const syncButton = page
      .locator(
        '[data-testid*="sync"], [aria-label*="sync"], button:has-text("sync"), button:has-text("🔄")'
      )
      .first();
    ...
    const filterButtons = await page.$$(
      '[data-testid*="filter"], [aria-label*="filter"], button:has-text("All"), button:has-text("Unread"), button:has-text("Read")'
    );
  ...
  test("Touch gestures should work correctly", async ({ page }) => {
    ...
    if (articles.length > 0) {
      ...
      const articleList = page.locator(
        '[data-testid="article-list"], .article-list, main'
      );
    ...
    const refreshIndicator = page.locator(
      '[data-testid="refresh"], .refresh-indicator, .pull-to-refresh'
    );
  ...
  test("Modal and dropdown buttons should be tappable", async ({ page }) => {
    ...
    const menuButton = page
      .locator(
        '[data-testid*="menu"], [aria-label*="menu"], button:has-text("⚙"), button:has-text("Settings")'
      )
      .first();
    if (await menuButton.isVisible()) {
      ...
      const modal = page.locator(
        '[role="dialog"], .modal, .dropdown-menu, .popover'
      );
      ...
      if (isModalVisible) {
        ...
        const closeButton = modal
          .locator(
            '[data-testid*="close"], [aria-label*="close"], button:has-text("✕"), button:has-text("Close")'
          )
          .first();
  ...
  test("Accessibility: all buttons should have proper ARIA labels", async ({
    page,
  }) => {
    ...
    for (const button of buttons) {
      ...
      if (!hasAccessibleName) {
        ...
        console.warn(
          `Button missing accessible name: ${html.substring(0, 100)}`
        );
      }
  ...
  test("Touch target spacing should prevent mis-taps", async ({ page }) => {
    ...
    for (let i = 0; i < positions.length; i++) {
      for (let j = i + 1; j < positions.length; j++) {
        ...
        if (
          horizontalDistance < MIN_SPACING &&
          verticalDistance < MIN_SPACING
        ) {
          console.warn(
            `Touch targets too close: ${horizontalDistance}px horizontal, ${verticalDistance}px vertical`
          );
        }
      }
    }
  ...
  test("PWA installation banner should be tappable", async ({
    page,
    context,
  }) => {
    ...
    const installButton = page
      .locator(
        '[data-testid*="install"], button:has-text("Install"), button:has-text("Add to Home Screen")'
      )
      .first();
    ...
    if (await installButton.isVisible()) {
      ...
      const dialog = page.locator(
        '[role="dialog"], .install-prompt, .pwa-install'
      );

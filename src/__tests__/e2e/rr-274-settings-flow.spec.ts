...
test.describe('Sync Settings User Flow (RR-274)', () => {
  ...
  test('should persist settings across sessions', async ({ page }) => {
    ...
    await expect(page.locator('input[name="retentionCount"]')).toHaveValue('1000');
  });
  ...
  test('should show real-time validation errors', async ({ page }) => {
    ...
    await expect(page.locator('text=Must be between 10 and 5000')).toBeVisible();
    ...
    await expect(page.locator('text=Must be between 100 and 5000')).toBeVisible();
  ...
  test('should update statistics after retention', async ({ page }) => {
    ...
    const initialTotal = await statsSection.locator('[data-label="Total Articles"]').textContent();
    const initialUnread = await statsSection.locator('[data-label="Unread"]').textContent();
    const initialStarred = await statsSection.locator('[data-label="Starred"]').textContent();
    ...
    const newTotal = await statsSection.locator('[data-label="Total Articles"]').textContent();
    const newStarred = await statsSection.locator('[data-label="Starred"]').textContent();
  ...
  test('should handle concurrent operations gracefully', async ({ page, context }) => {
    ...
    expect(['Settings saved successfully', 'Settings were updated by another session']).toContain(toastText);
    ...
    const value2 = await page2.locator('input[name="maxArticles"]').inputValue();
  ...
  test('should display live article statistics', async ({ page }) => {
    ...
    await expect(statsSection.locator('[data-label="Total Articles"]')).toContainText(/\d+/);
    await expect(statsSection.locator('[data-label="Unread"]')).toContainText(/\d+/);
    await expect(statsSection.locator('[data-label="Starred"]')).toContainText(/\d+/);

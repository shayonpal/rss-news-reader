...
test.describe('RR-180: Complete Glass Morphing Integration', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('article, [data-testid="article-item"], .article-item, a[href*="/article"]', {
      timeout: 15000,
    });
  });

  test.describe('Glass Button Components', () => {
    test('should render glass buttons with proper dimensions', async ({ page }) => {
      // Navigate to article detail - click on first article link
      const article = page.locator('article, [data-testid="article-item"], .article-item, a[href*="/article"]').first();
      await article.click();
    ...
    test('should apply glass effects with backdrop blur', async ({ page }) => {
      ...
      const elementsWithBlur = await page.evaluate(() => {
        ...
        allElements.forEach(el => {
          ...
          if (className.indexOf('glass') !== -1 || className.indexOf('blur') !== -1 || 
  ...
  test.describe('Morphing Dropdown Animation', () => {
    test('should open dropdown with smooth animation', async ({ page }) => {
      ...
      const moreButton = page.locator('button').filter({
        has: page.locator('[aria-label*="More"], svg, .icon-more, .dots-icon')
      }).first();

      // Alternative: look for button with specific aria-label
      const altMoreButton = page.locator('button[aria-label*="More"], button[aria-label*="options"]').first();
      
      // Use whichever button is found
      const targetButton = await moreButton.count() > 0 ? moreButton : altMoreButton;
      ...
      if (await targetButton.count() > 0) {
        ...
        const dropdownVisible = await page.evaluate(() => {
          const menus = document.querySelectorAll('[role="menu"], .dropdown-menu, [data-morphing-dropdown], .menu-items');
    ...
    test('should show correct menu items', async ({ page }) => {
      ...
      if (await moreButton.count() > 0) {
        ...
        const menuItems = page.locator('[role="menuitem"], .menu-item, button').filter({
    ...
    test('should close dropdown on outside click', async ({ page }) => {
      ...
      if (await moreButton.count() > 0) {
        try {
          ...
          const pageResponsive = await page.evaluate(() => document.body !== null);
  ...
  test.describe('Article Actions Toolbar', () => {
    test('should display star and other action buttons', async ({ page }) => {
      ...
      const starButton = page.locator('button').filter({
        has: page.locator('[aria-label*="Star"], .star-icon, svg[class*="star"]')
      });
      ...
      if (await starButton.count() > 0) {
        ...
        const isStarred = await starButton.first().evaluate(el => {
          return el.classList.contains('starred') || 
    ...
    test('should handle AI Summary action', async ({ page }) => {
      ...
      if (await summaryButton.count() > 0) {
        ...
        const summaryIndicator = page.locator('[class*="summary"], [data-testid*="summary"], .loading');
  ...
  test.describe('Visual Rendering Quality', () => {
    test('should render glass effects correctly', async ({ page }) => {
      ...
      const glassElement = page.locator('[class*="glass"], [class*="blur"]').first();
      ...
      if (await glassElement.count() > 0) {
        const hasGlassEffect = await glassElement.evaluate(el => {
          ...
          return className.includes('glass') || 
  ...
  test('should meet performance benchmarks', async ({ page, context }) => {
    ...
    await context.addInitScript(() => {
      ...
      const measureFrame = () => {
        ...
        if (frameCount < 60) { // Measure first second
    ...
    if (perfData.animationFrames.length > 0) {
      ...
      const avgFrameTime = perfData.animationFrames.reduce((a, b) => a + b, 0) / 

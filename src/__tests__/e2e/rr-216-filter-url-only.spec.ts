...
test.describe("RR-216: Filter URL Updates", () => {
  ...
  test("Topic selection updates URL with tag parameter", async ({ page }) => {
    ...
    if (await topics.count() > 0) {
      ...
      const anyTopic = page.locator('.cursor-pointer').filter({ hasText: /Gaming|Tech|Apple|Google/ }).first();
  ...
  test("Feed selection updates URL with feed parameter", async ({ page }) => {
    ...
    const feedNames = ['The Verge', 'TechCrunch', '9to5Mac', 'MacRumors', 'Ars Technica'];
    let foundFeed = false;
    ...
    if (!foundFeed) {
      ...
      const anyFeed = page.locator('.cursor-pointer').filter({ hasText: /\d+/ }).first();
  ...
  test("All Articles clears URL parameters", async ({ page }) => {
    ...
    const topics = page.locator('text=Gaming').or(page.locator('text=AI Related News'));
    if (await topics.count() > 0) {

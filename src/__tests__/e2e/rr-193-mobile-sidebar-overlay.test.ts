...
test.describe("RR-193 Mobile Sidebar Overlay", () => {
  ...
  test("should display sidebar as full-width overlay on mobile", async ({
    page,
  }) => {
  ...
  test("should displace article listing when sidebar is open on mobile", async ({
    page,
  }) => {
  ...
  test("should close sidebar when backdrop is tapped on mobile", async ({
    page,
  }) => {
    // Open sidebar
    const sidebarToggle = page.getByTestId("sidebar-toggle-button");
  ...
  test("should maintain single scroll behavior on mobile", async ({ page }) => {
    ...
    if (hasScrollbar) {
      ...
      const nestedScrollable = await sidebar
  ...
  test("should handle mutex accordion on touch interactions", async ({
    page,
  }) => {
    // Open sidebar
    const sidebarToggle = page.getByTestId("sidebar-toggle-button");
  ...
  test("should maintain 60fps animation performance on mobile", async ({
    page,
  }) => {

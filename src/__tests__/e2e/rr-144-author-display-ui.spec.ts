...
async function findArticleWithAuthor(
  page: Page,
...
test.describe("RR-144: Author Display UI Validation", () => {
  ...
  test.describe("Article List Author Display", () => {
    test("should display author names in article cards when available", async ({
      page,
    }) => {
      ...
      if (articleWithAuthor) {
        const authorElement = articleWithAuthor.locator(
        ...
        const bulletElement = articleWithAuthor
          .locator(".hidden.sm\\:inline")
          .last();
    ...
    test("should apply truncation with max-width 150px and ellipsis", async ({
      page,
    }) => {
      const articleWithAuthor = await findArticleWithAuthor(page);
    ...
    test("should not display empty author elements when author is missing", async ({
      page,
    }) => {
    ...
    test("should handle special characters in author names", async ({
      ...
      const articleWithSpecialChars = await findArticleWithAuthor(
        page,
        specialCharPattern
      );

      if (articleWithSpecialChars) {
        const authorElement = articleWithSpecialChars.locator(
          ".max-w-\\[150px\\].truncate"
        );
        const authorText = await authorElement.textContent();
    ...
    test("should maintain responsive behavior on mobile screens", async ({
      page,
    }) => {
      // Set mobile viewport
    ...
    test("should maintain consistent styling across all article cards", async ({
      page,
    }) => {
      const articles = page.locator("article[data-article-id]");
      const articlesWithAuthors = articles.filter({
        has: page.locator(".max-w-\\[150px\\].truncate"),
      });
      ...
      if (count > 1) {
        ...
        for (let i = 0; i < Math.min(count, 5); i++) {
          const authorElement = articlesWithAuthors
            .nth(i)
            .locator(".max-w-\\[150px\\].truncate");
  ...
  test.describe("Article Detail Author Display", () => {
    test("should display author in article detail metadata section", async ({
      page,
    }) => {
      ...
      if (articleWithAuthor) {
        ...
        const authorElements = metadataSection
          .locator("span")
          .filter({ hasNotText: "•" });
    ...
    test("should include bullet separator after author in detail view", async ({
      page,
    }) => {
      const articleWithAuthor = await findArticleWithAuthor(page);
    ...
    test("should not display author section when author is missing in detail view", async ({
      page,
    }) => {
      const articleWithoutAuthor = await findArticleWithoutAuthor(page);
    ...
    test("should maintain author text readability in detail view", async ({
      page,
    }) => {
      const articleWithAuthor = await findArticleWithAuthor(page);
    ...
    test("should handle very long author names in detail view", async ({
      page,
    }) => {
  ...
  test.describe("Cross-browser and Responsive Testing", () => {
    test("should display authors correctly on tablet viewport", async ({
      page,
    }) => {
    ...
    test("should display authors correctly on desktop viewport", async ({
      page,
    }) => {
  ...
  test.describe("Accessibility Testing", () => {
    test("should provide accessible author information in article list", async ({
      page,
    }) => {
      const articleWithAuthor = await findArticleWithAuthor(page);
    ...
    test("should provide accessible author information in article detail", async ({
      page,
    }) => {
      const articleWithAuthor = await findArticleWithAuthor(page);
    ...
    test("should maintain keyboard navigation with author elements", async ({
      page,
    }) => {
  ...
  test.describe("Edge Cases and Error Handling", () => {
    test("should handle articles with empty string authors", async ({
      page,
    }) => {
    ...
    test("should handle articles with null/undefined authors gracefully", async ({
      page,
    }) => {
      ...
      if (articlesWithoutAuthors) {
        ...
        const authorElements = articlesWithoutAuthors.locator(
          ".max-w-\\[150px\\].truncate"
        );
        await expect(authorElements).toHaveCount(0);

        // Should not have orphaned bullet separators
        const metadataDiv = articlesWithoutAuthors.locator(
          ".flex.flex-wrap.items-center.gap-x-2.gap-y-1"
        );
    ...
    test("should maintain performance with many articles containing authors", async ({
      page,
    }) => {
  ...
  test.describe("Visual Consistency Testing", () => {
    test("should maintain consistent author styling in light mode", async ({
      page,
    }) => {
    ...
    test("should maintain consistent author styling in dark mode", async ({
      page,
    }) => {
      // Enable dark mode
      await page.evaluate(() => {

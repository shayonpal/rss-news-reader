...
test.describe("RR-154: HTML Entity Decoding User Experience", () => {
  ...
  test("should display properly decoded HTML entities in article titles", async ({
    page,
  }) => {
  ...
  test("should display properly decoded HTML entities in article content", async ({
    page,
  }) => {
    // Wait for articles to load
    await page.waitForSelector('[data-testid="article-list"]', {
  ...
  test("should handle articles with mixed HTML entities correctly", async ({
    page,
  }) => {
  ...
  test("should preserve URLs with query parameters in article links", async ({
    page,
  }) => {
    await page.waitForSelector('[data-testid="article-list"]', {
      timeout: 10000,
  ...
  test("should display article metadata correctly after HTML decoding", async ({
    page,
  }) => {
    await page.waitForSelector('[data-testid="article-list"]', {
      timeout: 10000,
  ...
  test("should handle article list scrolling with decoded entities", async ({
    page,
  }) => {
    await page.waitForSelector('[data-testid="article-list"]', {
      timeout: 10000,
  ...
  test("should search articles with properly decoded content", async ({
    page,
  }) => {
    await page.waitForSelector('[data-testid="article-list"]', {
      timeout: 10000,
  ...
  test("should display proper quotation marks and apostrophes in article previews", async ({
    page,
  }) => {
    await page.waitForSelector('[data-testid="article-list"]', {
      timeout: 10000,
    ...
    for (let i = 0; i < Math.min(5, articleCount); i++) {
      ...
      const previewElement = article.locator(
        '[data-testid="article-preview"], [data-testid="article-excerpt"]'
      );
  ...
  test("should handle read/unread status with decoded article titles", async ({
    page,
  }) => {
    await page.waitForSelector('[data-testid="article-list"]', {
      timeout: 10000,
  ...
  test("should display proper entities in feed titles and organization", async ({
    page,
  }) => {
    // Check if there's a sidebar or feed navigation
    const feedSidebar = page.locator(
      '[data-testid="feed-sidebar"], [data-testid="feed-list"]'
    );

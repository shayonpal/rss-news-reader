...
test.describe("RR-148: On-Demand Content Parsing E2E", () => {
  ...
  test.describe("Partial Feed Article Opening", () => {
    test("should trigger content parsing when opening partial feed article", async ({
      page,
    }) => {
      ...
      await page.route("**/api/articles/*/parse-content", async (route) => {
        await route.fulfill({
          ...
          body: JSON.stringify({
            success: true,
            content:
              "<p>This is the full extracted content from on-demand parsing</p>",
            cached: false,
      ...
      await expect(
        page.locator('[data-testid="content-loading"]')
      ).toBeVisible();

      // Wait for full content to load
      await expect(
        page.locator('[data-testid="article-content"]')
      ).toContainText("full extracted content");
    ...
    test("should display loading indicator during content parsing", async ({
      page,
    }) => {
      ...
      ).toContainText("Loading full content");
      ...
      await expect(
        page.locator('[data-testid="article-content"]')
      ).toContainText("Slowly loaded content");
      await expect(
        page.locator('[data-testid="content-loading"]')
      ).not.toBeVisible();
    });

    test("should show cached content immediately if already parsed", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="article-content"]')
      ).toContainText("cached full content");
    ...
    test("should fallback to RSS content when parsing fails", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="article-content"]')
      ).toContainText("Original RSS content");
  ...
  test.describe("User Experience Flow", () => {
    test("should provide seamless reading experience", async ({ page }) => {
        ...
        if (route.request().url().includes("/parse-content")) {
          await route.fulfill({
            ...
            body: JSON.stringify({
              success: true,
              content:
                "<div><h1>Full Article</h1><p>Complete article content with detailed information...</p></div>",
              cached: false,
            }),
          });
        } else {
      ...
      await expect(
        page.locator('[data-testid="article-content"]')
      ).toContainText("Complete article content");

      // Should maintain proper formatting
      await expect(
        page.locator('[data-testid="article-content"] h1')
      ).toContainText("Full Article");
    });

    test("should handle multiple rapid article opens gracefully", async ({
      page,
    }) => {
      let requestCount = 0;
    ...
    test("should work correctly on mobile viewport", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="article-content"]')
      ).toContainText("Mobile-optimized");
  ...
  test.describe("Performance and Reliability", () => {
    test("should complete parsing within performance target", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="article-content"]')
      ).toContainText("Performance-tested");
  ...
  test.describe("Accessibility and Usability", () => {
    test("should maintain accessibility during content loading", async ({
      page,
    }) => {
      await page.route("**/api/articles/*/parse-content", async (route) => {
      ...
      await expect(
        page.locator('[data-testid="article-content"]')
      ).toContainText("Accessible content");
    });

    test("should support keyboard navigation during parsing", async ({
      page,
    }) => {
      await page.route("**/api/articles/*/parse-content", async (route) => {
        await new Promise((resolve) => setTimeout(resolve, 1000));
      ...
      await expect(
        page.locator('[data-testid="article-content"]')
      ).toContainText("Keyboard-accessible");
    });
  });
});

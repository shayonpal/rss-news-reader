...
test.describe('RR-27: Comprehensive Mark-as-Read Scenarios', () => {
  test('should handle all 5 mark-as-read scenarios with session preservation', async ({ page }) => {
    ...
    await test.step('Manual article click preserves with context', async () => {
      ...
      await page.locator('[role="menuitem"]', { hasText: 'Unread only' }).click();
    ...
    await test.step('Mark All as Read marks ALL articles and preserves in session', async () => {
      ...
      const unreadCount = await page.locator('.text-muted-foreground').textContent();
      ...
      const finalCount = await page.locator('.text-muted-foreground').textContent();
  ...
  test('should maintain context across complex navigation flows', async ({ page }) => {
    await page.goto(RSS_READER_URL);

...
test.describe("RR-215: iOS Scrollable Header Mobile E2E", () => {
  ...
  test.describe("Header State Transitions", () => {
    test("should start in expanded state with large title", async ({
      page,
    }) => {
      ...
      await expect(
        navButton.locator('[data-testid="hamburger-icon"]')
      ).toBeVisible();
      await expect(navButton.locator('[data-testid="back-icon"]')).toHaveCSS(
        "opacity",
        "0"
      );
    });
    ...
    test("should transition to collapsed state on scroll", async ({ page }) => {
      ...
      await expect(
        navButton.locator('[data-testid="back-icon"]')
      ).toBeVisible();
      await expect(
        navButton.locator('[data-testid="hamburger-icon"]')
      ).toHaveCSS("opacity", "0");
    ...
    test("should show transitioning state in middle range", async ({
      page,
    }) => {
      const header = page.locator(
        '[data-testid="scrollable-header-container"]'
  ...
  test.describe("Touch Interactions", () => {
    test("should have 44px minimum touch target for nav button", async ({
      page,
    }) => {
      const navButton = page.locator('[data-testid="morphing-nav-button"]');
    ...
    test("should provide immediate visual feedback on tap", async ({
      page,
    }) => {
      const navButton = page.locator('[data-testid="morphing-nav-button"]');
  ...
  test.describe("Progressive Enhancement", () => {
    test("should fallback gracefully without backdrop-filter", async ({
      page,
    }) => {

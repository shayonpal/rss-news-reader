...
test.describe("RR-251: Ghost Button Violet Theme E2E", () => {
  ...
  test("Ghost navigation buttons should render with violet theme", async ({
    page,
  }) => {
  ...
  test("Dark mode toggle should change ghost button colors", async ({
    page,
  }) => {
  ...
  test("Ghost buttons should have proper hover states", async ({ page }) => {
    ...
    const initialBg = await prevButton.evaluate(
    ...
    const hoverBg = await prevButton.evaluate(
      (el) => window.getComputedStyle(el).backgroundColor
    );

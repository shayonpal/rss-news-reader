...
test.describe('RR-106: Freshness API Removal - E2E Tests', () => {
  ...
  test.describe('Health Endpoint Availability After Removal', () => {
    test('should verify remaining health endpoints are accessible', async ({ page }) => {
    ...
    test('should verify freshness endpoint returns 404 after removal', async ({ page }) => {
      ...
      if (response.status() === 404) {
        ...
        console.log(`Freshness endpoint still returns ${response.status()} (expected during removal process)`);
        expect([200, 503]).toContain(response.status());
      }
    });

    test('should verify sync server health endpoint still works', async ({ page }) => {
  ...
  test.describe('Application Core Functionality', () => {
    test('should verify main application loads without freshness dependency', async ({ page }) => {
    ...
    test('should verify article list functionality works', async ({ page }) => {
      ...
      const articleListExists = await page.locator('[data-testid="article-list"], .article-list, main').count();
      expect(articleListExists).toBeGreaterThan(0);
      
      // Should not show freshness-related UI elements
      const freshnessUI = await page.locator('text=/freshness|stale|degraded/i').count();
      expect(freshnessUI).toBe(0);
    });

    test('should verify sync functionality remains accessible', async ({ page }) => {
  ...
  test.describe('Monitoring Integration', () => {
    test('should verify monitoring endpoints exclude freshness', async ({ page }) => {
    ...
    test('should verify no freshness references in client-side monitoring', async ({ page }) => {
      await page.goto(APP_BASE_URL);
      await page.waitForLoadState('networkidle');
      ...
      if (freshnessRequests.length === 0) {
        ...
        console.log('Client still making freshness requests (expected during removal process):', freshnessRequests);
      }
    });
  });

  test.describe('Error Handling and Recovery', () => {
    test('should handle missing freshness endpoint gracefully', async ({ page }) => {
      // Simulate monitoring scripts trying to access removed endpoint
      const response = await page.request.get(`${APP_BASE_URL}/api/health/freshness`);
      ...
      if (response.status() === 404) {
        ...
        const appResponse = await page.request.get(`${APP_BASE_URL}/api/health/app`);
        expect([200, 503]).toContain(appResponse.status());
      } else {
        console.log('Freshness endpoint still accessible during removal process');
      }
    });

    test('should verify monitoring systems recover from freshness removal', async ({ page }) => {
  ...
  test.describe('Performance and Stability', () => {
    test('should verify application performance is not degraded', async ({ page }) => {
    ...
    test('should verify no resource leaks from freshness removal', async ({ page }) => {
      await page.goto(APP_BASE_URL);
      await page.waitForLoadState('networkidle');
      ...
      const consoleErrors = [];
      page.on('console', msg => {
        if (msg.type() === 'error' && msg.text().toLowerCase().includes('freshness')) {

...
const measurePerformance = async (fn: () => Promise<void>): Promise<PerformanceMetrics> => {
...
describe("RR-215: iOS Header Scroll Performance", () => {
  ...
  const mockScrollCoordinator = {
    ...
    updateScrollPosition(y: number) {
      this.scrollY = y;
      this.scrollState = y <= 44 ? "expanded" : 
                       y <= 150 ? "transitioning" : "collapsed";
  ...
  describe("Scroll Coordination Performance", () => {
    it("should handle 1000 scroll updates within 100ms", async () => {
      const scrollListener = () => {
        ...
        const titleScale = Math.max(0.5, 1 - mockScrollCoordinator.scrollY / 300);
        const headerHeight = Math.max(54, 120 - mockScrollCoordinator.scrollY * 0.4);
        const blurIntensity = Math.min(16, 8 + mockScrollCoordinator.scrollY / 20);
    ...
    it("should maintain smooth title scaling performance", async () => {
      const metrics = await measurePerformance(async () => {
        ...
        for (let scrollY = 0; scrollY <= 300; scrollY += 2) {
          ...
          const fontSize = scrollY <= 44 ? 34 : 
                          scrollY <= 150 ? 34 - (scrollY - 44) * 0.17 : 17;
    ...
    it("should not leak memory during extended scroll sessions", async () => {
      const metrics = await measurePerformance(async () => {
        ...
        for (let minute = 0; minute < 5; minute++) {
          for (let scroll = 0; scroll < 120; scroll++) { // 2 scrolls per second
    ...
    it("should handle rapid subscribe/unsubscribe cycles", async () => {
      const metrics = await measurePerformance(async () => {
        for (let cycle = 0; cycle < 50; cycle++) {
          ...
          const componentIds = [`header-${cycle}`, `nav-${cycle}`, `sidebar-${cycle}`];
  ...
  describe("Edge Case Performance", () => {
    ...
    it("should handle boundary scroll positions efficiently", async () => {
      const boundaryPositions = [
        0, 1, 43, 44, 45, // Expanded/transitioning boundary
        149, 150, 151, // Transitioning/collapsed boundary
        999, 1000, 10000, // Large scroll values
      ];
  ...
  describe("Integration Performance", () => {
    it("should perform well with RR-197 PerformanceMonitor integration", async () => {
      ...
      mockScrollCoordinator.subscribe("performance-monitor", performanceMonitor);

...
describe('RR-129: Cleanup Performance Tests', () => {
  ...
  afterAll(async () => {
    ...
    await supabase.from('deleted_articles').delete().like('inoreader_id', 'test-rr129-perf-%');
  });

  beforeEach(async () => {
    // Clear test data before each test
    await supabase.from('articles').delete().like('inoreader_id', 'test-rr129-perf-%');
    await supabase.from('feeds').delete().like('inoreader_id', 'test-rr129-perf-%');
    await supabase.from('deleted_articles').delete().like('inoreader_id', 'test-rr129-perf-%');
  ...
  const createTestData = async (articleCount: number, feedCount: number = 1) => {
    // Create feeds
  ...
  describe('Tracking Table Performance', () => {
    it('should insert tracking records in batches efficiently', async () => {
      ...
      for (const batchSize of batchSizes) {
        ...
        if (batchSize <= 100) {
          expect(insertTime).toBeLessThan(PERFORMANCE_TARGETS.TRACKING_INSERT_BATCH_100);
        } else if (batchSize <= 1000) {
          expect(insertTime).toBeLessThan(PERFORMANCE_TARGETS.TRACKING_INSERT_BATCH_1000);
        }
    ...
    it('should perform fast lookups in large tracking tables', async () => {
      ...
      for (const trackingCount of trackingCounts) {
        ...
        const lookupIds = Array.from({ length: 100 }, (_, i) => 
          `test-rr129-perf-lookup-${trackingCount}-${i * 10}`
        );
        ...
        if (trackingCount <= 1000) {
          expect(lookupTime).toBeLessThan(PERFORMANCE_TARGETS.TRACKING_LOOKUP_1000);
        } else if (trackingCount <= 10000) {
          expect(lookupTime).toBeLessThan(PERFORMANCE_TARGETS.TRACKING_LOOKUP_10000);
        }

        console.log(`Tracking lookup (${trackingCount} entries): ${lookupTime}ms`);
    ...
    it('should cleanup old tracking entries efficiently', async () => {
      ...
      const oldTimestamp = new Date(Date.now() - (120 * 24 * 60 * 60 * 1000)).toISOString(); // 120 days old
      const recentTimestamp = new Date(Date.now() - (30 * 24 * 60 * 60 * 1000)).toISOString(); // 30 days old
      ...
      // Insert test data
      await supabase.from('deleted_articles').insert([...oldEntries, ...recentEntries]);
      ...
      expect(cleanupTime).toBeLessThan(PERFORMANCE_TARGETS.TRACKING_CLEANUP_OLD);
  ...
  describe('Article Cleanup Performance', () => {
    it('should delete large numbers of articles efficiently', async () => {
      ...
      for (const deletionSize of deletionSizes) {
        ...
        // Performance assertions
        if (deletionSize <= 100) {
          expect(deleteTime).toBeLessThan(PERFORMANCE_TARGETS.CLEANUP_DELETE_100);
        } else if (deletionSize <= 1000) {
          expect(deleteTime).toBeLessThan(PERFORMANCE_TARGETS.CLEANUP_DELETE_1000);
        }
    ...
    it('should handle CASCADE deletion performance for large feeds', async () => {
      ...
      expect(error).toBeNull();
      expect(cascadeTime).toBeLessThan(PERFORMANCE_TARGETS.CASCADE_DELETE_LARGE_FEED);

      console.log(`CASCADE deletion (1 feed, ${articleCount} articles): ${cascadeTime}ms`);
  ...
  describe('Sync Filtering Performance', () => {
    it('should filter sync articles efficiently with large tracking sets', async () => {
      ...
      for (const filterSize of filterSizes) {
        ...
        const syncArticleIds = Array.from({ length: Math.floor(filterSize * 1.2) }, (_, i) => 
          `test-rr129-perf-filter-${filterSize}-${i}`
        );
        ...
        const deletedIds = new Set(deletedData?.map(d => d.inoreader_id) || []);
        ...
        console.log(`Sync filtering (${filterSize} tracking entries, ${syncArticleIds.length} articles): ${filterTime}ms`);
    ...
    it('should maintain performance with concurrent sync operations', async () => {
      ...
      const syncPromises = Array.from({ length: concurrentSyncs }, async (_, syncIndex) => {
        const syncArticleIds = Array.from({ length: articlesPerSync }, (_, i) => 
      ...
      results.forEach(result => {
        ...
        console.log(`Concurrent sync ${result.syncIndex}: ${result.syncTime}ms, ${result.resultCount} found`);
      });
  ...
  describe('Database Size Impact and Scalability', () => {
    it('should demonstrate storage efficiency improvements', async () => {
      ...
      for (const testSize of testSizes) {
        ...
        const reductionPercentage = ((initialCount - finalCount!) / initialCount) * 100;
        ...
        console.log(`Storage efficiency (${testSize} articles): ${reductionPercentage.toFixed(1)}% reduction, ${deletedCount} deleted in ${deleteTime}ms`);
    ...
    it('should scale linearly with database growth', async () => {
      ...
      for (const scaleSize of scaleSizes) {
        ...
        console.log(`Scale test ${scaleSize}: ${totalTime}ms total, ${timePerArticle.toFixed(2)}ms per article`);
      ...
      console.log(`Scale factor: ${scaleFactor}x size, ${timeIncrease.toFixed(2)}x time`);
    });
  });
  ...
  describe('Memory and Resource Usage', () => {
    it('should maintain stable memory usage during large operations', async () => {
      ...
      console.log(`Memory usage - Initial: ${(initialMemory.heapUsed / 1024 / 1024).toFixed(1)}MB, Final: ${(finalMemory.heapUsed / 1024 / 1024).toFixed(1)}MB, Increase: ${heapIncrease.toFixed(1)}MB`);
  ...
  describe('Performance Regression Detection', () => {
    it('should maintain consistent performance across multiple runs', async () => {
      ...
      const avgTime = performanceMeasurements.reduce((a, b) => a + b, 0) / testRuns;
      ...
      console.log(`Performance consistency - Avg: ${avgTime.toFixed(0)}ms, Min: ${minTime}ms, Max: ${maxTime}ms, Variation: ${variation.toFixed(1)}%`);
    });
    ...
    it('should meet overall performance targets for RR-129', async () => {
      ...
      const syncArticleIds = Array.from({ length: 200 }, (_, i) => 
        `test-rr129-perf-article-${i}`
      );
      ...
      console.log(`✅ RR-129 Performance targets met - processed ${deletedCount} articles in ${fullCycleTime}ms`);
    });
  });
});
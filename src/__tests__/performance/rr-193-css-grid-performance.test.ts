...
test.describe('RR-193 CSS Grid Performance', () => {
  ...
  test('should achieve 60fps during sidebar animations', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      performance.measure('accordion-toggle', 'accordion-toggle-start', 'accordion-toggle-end');
    });
  ...
  test('should have minimal layout thrashing with CSS Grid', async ({ page }) => {
  ...
  test('should handle large feed lists without performance degradation', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      performance.measure('scroll-test', 'scroll-test-start', 'scroll-test-end');
    });
  ...
  test('should maintain responsive performance across viewport sizes', async ({ page }) => {
    const viewports = [
    ...
    for (const viewport of viewports) {
      ...
      const layoutMetrics = await page.evaluate(() => {
        ...
        return recent.length >= 2 ? recent[1].startTime - recent[0].startTime : 0;
      });
  ...
  test('should have no memory leaks during accordion interactions', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      (window as any).finalMemory = (performance as any).memory?.usedJSHeapSize || 0;
    });

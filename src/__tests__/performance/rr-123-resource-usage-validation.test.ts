...
describe("RR-123: Performance and Resource Usage Validation", () => {
  ...
  describe("Memory Usage Validation", () => {
    ...
    it("MUST have stable memory usage (no memory leaks)", async () => {
      ...
      console.log(
        `Average memory increase per test run: ${Math.round(avgIncrease / 1024 / 1024)}MB`
      );
    });
    ...
    it("MUST release memory after test completion", async () => {
      ...
      console.log(
        `Retained memory after tests: ${Math.round(retainedMemory / 1024 / 1024)}MB`
      );
    });
  });
  ...
  describe("Process Count Validation", () => {
    it("MUST NOT spawn more than 4 vitest processes", async () => {
      ...
      const avgProcesses =
        processCountLog.reduce((a, b) => a + b, 0) / processCountLog.length;
    ...
    it("MUST prevent process accumulation during multiple test runs", async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        console.log(
          `Iteration ${i + 1}: ${beforeTest} -> ${afterTest} processes`
        );
      }

      // Should not accumulate processes
      const avgProcessIncrease =
        processReadings.reduce((a, b) => a + b, 0) / processReadings.length;
  ...
  describe("System Performance Validation", () => {
    ...
    it("MUST maintain low CPU usage during tests", async () => {
      ...
      if (cpuReadings.length > 0) {
        ...
        console.log(
          `Average CPU: ${avgCpu.toFixed(1)}%, Max CPU: ${maxCpu.toFixed(1)}%`
        );
      }
    });
    ...
    it("MUST not impact PM2 service performance", async () => {
      ...
      const servicesBefore = beforeServices.stdout
        .split("\n")
        .filter((line) => line.includes("online")).length;

      // Monitor PM2 memory usage
      const { stdout: beforeMemory } = await execAsync(
        'pm2 monit --json | grep -o \'"memory":[0-9]*\' || echo "0"'
      );
      ...
      const servicesAfter = afterServices.stdout
        .split("\n")
        .filter((line) => line.includes("online")).length;
  ...
  describe("Resource Limit Compliance", () => {
      ...
      const monitor = setInterval(async () => {
        try {
          const { stdout } = await execAsync(
            'ps aux | grep -E "vitest.*worker|vitest.*test" | grep -v grep | wc -l'
          );
    ...
    it("MUST respect memory limits per process", async () => {
      ...
      const memoryMonitor = setInterval(async () => {
        try {
          const { stdout } = await execAsync(
            "ps aux | grep vitest | grep -v grep | awk '{print $6}' || echo \"0\""
          );
          const memories = stdout
            .split("\n")
      ...
      if (processMemories.length > 0) {
        ...
        const avgProcessMemory =
          processMemories.reduce((a, b) => a + b, 0) / processMemories.length;
        ...
        console.log(
          `Max process memory: ${Math.round(maxProcessMemory / 1024 / 1024)}MB`
        );
        console.log(
          `Avg process memory: ${Math.round(avgProcessMemory / 1024 / 1024)}MB`
        );
      }
    });
  });

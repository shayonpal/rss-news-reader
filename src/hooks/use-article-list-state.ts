...
export function useArticleListState({
  ...
  useEffect(() => {
    ...
    if (state && !articleListStateManager.isStateExpired()) {
      ...
      console.log(
        `🔧 Hook initialized with ${preserved.length} preserved articles`
      );
    }
  }, []);
  ...
  const saveStateBeforeNavigation = useCallback(() => {
    ...
    const state: Partial<ListState> = {
      ...
      readStates: articleArray.reduce(
        (acc, article) => {
      ...
      autoReadArticles: [
        ...new Set([
          ...
          ...articleArray
            .filter(
              (a) => a.isRead && sessionPreservedArticles.current.has(a.id)
            )
            .map((a) => a.id),
        ]),
      ],
  ...
  const shouldShowArticle = useCallback(
    (article: Article) => {
      ...
      if (readStatusFilter === "unread") {
        ...
        if (currentState && !articleListStateManager.isStateExpired()) {
          ...
          const isAutoRead = currentState.autoReadArticles?.includes(
            article.id
          );
          const isManuallyReadInSession =
            currentState.manualReadArticles?.includes(article.id);
  ...
  useEffect(() => {
    ...
    if (!state && sessionPreservedArticles.current.size > 0) {
      console.log(
        `🔧 Detected external state clearing on articles change, clearing sessionRef (had ${sessionPreservedArticles.current.size} articles)`
      );
      sessionPreservedArticles.current = new Set();
    }
  }, [articles]);

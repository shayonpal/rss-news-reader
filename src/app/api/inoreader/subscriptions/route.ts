  ...
  try {
    ...
    if (accessToken && expiresAt) {
      ...
      if (expiresAtTime < Date.now()) {
        ...
        if (refreshToken) {
          const refreshResponse = await fetch(`${request.nextUrl.origin}/api/auth/inoreader/refresh`, {
            method: 'POST',
            headers: {
    ...
    if (!process.env.NEXT_PUBLIC_INOREADER_CLIENT_ID || !process.env.INOREADER_CLIENT_SECRET) {

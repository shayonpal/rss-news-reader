...
function validateEncryptionKey(): void {
  if (!ENCRYPTION_KEY) {
    console.warn(
      "[Preferences] TOKEN_ENCRYPTION_KEY not set - API keys will not be encrypted"
    );
    return;
  }
  ...
  if (!/^[0-9a-fA-F]{64}$/.test(ENCRYPTION_KEY)) {
    console.error(
      "[Preferences] TOKEN_ENCRYPTION_KEY must be a 64-character hex string"
    );
  }
}
    ...
    .object({
      ...
      summaryWordCount: z
        ...
        .refine(
          ...
          {
            message:
              "Word count must be between 10-500 for minimum and 20-1000 for maximum, with min ≤ max",
          }
        ),
      summaryStyle: z

...
export async function GET() {
  try {
    ...
    const { data: allLogs, error: logsError } = await supabase.from(
      "fetch_logs"
    ).select(`
        id,
        fetch_type,
    ...
    const overallStats = {
      today: {
        auto: { total: 0, successful: 0, failed: 0 },
        manual: { total: 0, successful: 0, failed: 0 },
      },
      thisMonth: {
        auto: { total: 0, successful: 0, failed: 0 },
        manual: { total: 0, successful: 0, failed: 0 },
      ...
      if (log.status !== "attempt") {
        ...
        if (period === "today") {
          ...
          if (log.status === "success") {
            ...
            if (log.duration_ms) {
              feedStats.thisMonth[log.fetch_type].durations.push(
                log.duration_ms
              );
            }
          } else if (log.status === "failure") {
    ...
    const { data: recentFailuresData, error: recentFailuresError } =
      await supabase
        .from("fetch_logs")
        .select(
          `
    ...
    const feeds = Array.from(feedStatsMap.values())
      .filter(
        (feed) => feed.lifetime.auto.total + feed.lifetime.manual.total > 0
      )
      .sort((a, b) => {
    ...
    const recentFailures: RecentFailure[] =
      recentFailuresData?.map((row: any) => ({

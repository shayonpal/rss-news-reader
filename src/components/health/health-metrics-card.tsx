...
export function HealthMetricsCard({ metrics }: HealthMetricsCardProps) {
  ...
  const successRate = metrics.totalChecks > 0
    ? ((metrics.totalChecks - metrics.failedChecks) / metrics.totalChecks * 100).toFixed(1)
    : '100.0';
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <MetricCard
        ...
        color={metrics.avgResponseTime < 1000 ? 'green' : metrics.avgResponseTime < 3000 ? 'yellow' : 'red'}
      />
      <MetricCard
        icon={<AlertCircle className="h-5 w-5" />}
        label="Failed Checks"
        value={metrics.failedChecks.toString()}
        color={metrics.failedChecks === 0 ? 'green' : metrics.failedChecks < 5 ? 'yellow' : 'red'}
      />
    </div>
  );
}
...
function MetricCard({ icon, label, value, color }: MetricCardProps) {
  const colorClasses = {
    ...
    yellow: 'text-yellow-600 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-950/20',

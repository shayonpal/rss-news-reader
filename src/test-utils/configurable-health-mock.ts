...
import type { UISystemHealth, HealthStatus, ServiceHealth, ComponentHealthCheck } from "@/types/health";
...
export class ConfigurableHealthCheckMock {
  ...
  setServiceStatus(serviceName: string, status: HealthStatus, message?: string): void {
    const service = this.mockHealth.services.find(s => s.name === serviceName);
    if (service) {
      service.status = status;
  ...
  setCheckStatus(serviceName: string, checkName: string, status: HealthStatus, message?: string): void {
    const service = this.mockHealth.services.find(s => s.name === serviceName);
    if (service) {
  ...
  simulateServerEnvironment(): void {
    ...
    const cacheService = this.mockHealth.services.find(s => s.name === "cache");
    if (cacheService) {
    ...
    const networkService = this.mockHealth.services.find(s => s.name === "network");
    if (networkService) {
  ...
  simulateDatabaseFailure(): void {
    this.setServiceStatus("database", "unhealthy", "Database connection failed");
  }
  ...
  setMockHealthFunction(fn: () => Promise<UISystemHealth> | UISystemHealth): void {
